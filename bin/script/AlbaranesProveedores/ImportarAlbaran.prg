/*
COLUMNA A---------UNIDADES COMPRADAS
COLUMNA B---------CODIGO ARTICULO
COLUMNA C---------EAN 13
...
CLASS TImportarExcelAlbaranes FROM TImportarExcel
   ...
   METHOD SetResources()      INLINE ( SetResources( fullcurdir() + "Script\AlbaranesProveedores\ImportarAlbaran.dll" ) )
...
METHOD procesaLinea()
   ...
   for n := 2 to 65536
      ...
      cCodArt     := cValToChar( ::oExcel:oExcel:ActiveSheet:Range( "B" + lTrim( str( n ) ) ):Value )
      ...
      if !empty( cCodArt )
         ...
         */
         ...
         hSet( hLine, "NombreArticulo", cValToChar( ::oExcel:oExcel:ActiveSheet:Range( "D" + lTrim( str( n ) ) ):Value ) )

         /*
         Código barras---------------------------------------------------------
         */

         cCodBar        := cValToChar( ::oExcel:oExcel:ActiveSheet:Range( "C" + lTrim( str( n ) ) ):Value )

         nPos           := at( ".", cCodBar )
         if nPos != 0
            cCodBar     := SubStr( cCodBar, 1, nPos - 1 )
         end if
         hSet( hLine, "CodigoBarras", Padr( cCodBar, 20 ) )

         /*
         Unidades compradas----------------------------------------------------
         */

         hSet( hLine, "Unidades", ::oExcel:oExcel:ActiveSheet:Range( "A" + lTrim( str( n ) ) ):Value )

         /*
         PVP-------------------------------------------------------------------
         */

         hSet( hLine, "PVenta", ::oExcel:oExcel:ActiveSheet:Range( "E" + lTrim( str( n ) ) ):Value )

         /*
         Costo-----------------------------------------------------------------
         */

         hSet( hLine, "PCosto", ::oExcel:oExcel:ActiveSheet:Range( "I" + lTrim( str( n ) ) ):Value )

         /*
         Familia---------------------------------------------------------------
         */

         hSet( hLine, "cCodFamilia", "" )
         hSet( hLine, "cNomFamilia", ::oExcel:oExcel:ActiveSheet:Range( "F" + lTrim( str( n ) ) ):Value )
...
METHOD addProduct( hLine )
   ...
   local nRecAntCodeBar       := ( D():ArticulosCodigosBarras( ::nView ) )->( recno() )
   local nOrdAntCodeBar       := ( D():ArticulosCodigosBarras( ::nView ) )->( OrdSetFocus( "cArtBar" ) )
   ...
   if !( D():Articulos( ::nView ) )->( dbSeek( hGet( hLine, "CodigoArticulo" ) ) )
      ...
      ( D():Articulos( ::nView ) )->Familia     := hGet( hLine, "cCodFamilia" )
      ...
      if dbLock( D():Articulos( ::nView ) )
         ...
         ( D():Articulos( ::nView ) )->pVenta1     := ( hGet( hLine, "PVenta" ) / 1.21 )
         ( D():Articulos( ::nView ) )->pVtaIva1    := hGet( hLine, "PVenta" )  
         if !Empty( hGet( hLine, "cCodFamilia" ) )
            ( D():Articulos( ::nView ) )->Familia  := hGet( hLine, "cCodFamilia" )
         end if
         ( D():Articulos( ::nView ) )->( dbUnLock() )
      end if
   ...
   if !( D():ArticulosCodigosBarras( ::nView ) )->( dbSeek( hGet( hLine, "CodigoArticulo" ) + hGet( hLine, "CodigoBarras" ) ) )
      
      ( D():ArticulosCodigosBarras( ::nView ) )->( dbAppend() )
      
      ( D():ArticulosCodigosBarras( ::nView ) )->cCodArt    := hGet( hLine, "CodigoArticulo" )
      ( D():ArticulosCodigosBarras( ::nView ) )->cCodBar    := hGet( hLine, "CodigoBarras" )
      ( D():ArticulosCodigosBarras( ::nView ) )->nTipBar    := 1
...
METHOD addFamilia( hLine )
   ...
   if !( D():Familias( ::nView ) )->( dbSeek( Upper( hGet( hLine, "cNomFamilia" ) ) ) )
      ...
      hSet( hLine, "cCodFamilia", cNewCodFam )

   else

      hSet( hLine, "cCodFamilia", ( D():Familias( ::nView ) )->cCodFam )
