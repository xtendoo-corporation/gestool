   ...
   DEFINE DIALOG oDlg RESOURCE "ImportarICG"
      ...
      */
      ...
      REDEFINE BUTTON oBtnAceptar;
         ID       IDOK ;
         OF       oDlg ;
         ACTION   ( Ejecutar( aFichero, oDlg, oInforme ) )
      ...
      oDlg:AddFastKey( VK_F5, {|| Ejecutar( aFichero, oDlg, oInforme ) } )
   ...
   */
   ...
   if lIncidencia
      ...
      */
      ...
      //hSet( hashDatabaseList, "mail", "dario@gestool.es" )
...
Static Function IcgDetAlbPrv( cSerDoc, cSufDoc, cDesLin, nUntLin, nPvpLin, nDtoLin, cRefLin, dFecDoc )
   ...
   if !dbSeekInOrd( cRefLin, "Codigo", D():Articulos( nView ) )
      ...
      if ( Round( ( D():Articulos( nView ) )->pCosto, 2 ) != ( Round( ( nPvpLin ) - ( nPvpLin * nDtoLin / 100 ), 2 ) ) )
         ...
         cInforme             += "......Familia " + ( D():Articulos( nView ) )->Familia + ", " + alltrim( retFamilia( ( D():Articulos( nView ) )->Familia, D():Familias( nView ) ) ) + CRLF
         cInforme             += "......Precio nuevo " + alltrim( str( round( ( nPvpLin ) - ( nPvpLin * nDtoLin / 100 ), 2 ) ) )  + CRLF
         cInforme             += "......Precio anterior " + alltrim( str( round( ( D():Articulos( nView ) )->pCosto, 2 ) ) ) + " con fecha " + dtoc( ( D():Articulos( nView ) )->dFecChg ) + CRLF
         cInforme             += CRLF
         lIncidencia          := .t.
      end if
   end if
