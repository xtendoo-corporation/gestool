...
METHOD New( nView )
   ...
   */
   ...
   ::cFicheroExcel            := "E:\Xtendoo\CECOTRANS\clientes.xls"
...
METHOD importarCampos()
   ...
   ( D():Clientes( ::nView ) )->Cod             := RJust( ::getExcelString( "M" ), "0", RetNumCodCliEmp() )
   ...
   if !empty( ::getExcelString( "H" ) )
      ( D():Clientes( ::nView ) )->CodPostal    := ::getExcelString( "H" )
   end if 
   ...
   if !empty( ::getExcelString( "K" ) )
      ( D():Clientes( ::nView ) )->cMeiInt      := ::getExcelString( "K" )
   end if
