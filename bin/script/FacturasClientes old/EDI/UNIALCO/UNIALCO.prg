...
METHOD writeReferenciaAlbaran()
   ...
   cLine          += __separator__
   cLine          += AllTrim( ::getFecha( RetFld( ( D():FacturasClientes( ::nView ) )->cNumAlb, D():AlbaranesClientes( ::nView ), "dFecAlb" ) ) )
...
METHOD writeReceptorFactura()
   ...
   if ( D():Clientes( ::nView ) )->( dbSeek( ( D():FacturasClientes( ::nView ) )->cCodCli ) )
      cLine       += AllTrim( ( D():Clientes( ::nView ) )->cCodEdi ) + __separator__    //Codigo EDI
      cLine       += AllTrim( ( D():FacturasClientes( ::nView ) )->cNomCli ) + __separator__                                              //Nombre
...
METHOD writeDivisa()
   ...
   cLine          += "EUR" + __separator__    //Condiciones de pago
   cLine          += "4"                     //Fecha de vencimiento
...
METHOD writeVencimiento()
   ...
   local cLine    := "PAT" + __separator__
   
   cLine          += "35" + __separator__   //Moneda
   cLine          += ::getFecha( ( D():FacturasClientes( ::nView ) )->dFecFac + 30 ) + __separator__  //Fecha de vencimiento
   cLine          += ::getNumero( ::sTotalFactura:TotalDocumento() ) //Importe de la factura
...
METHOD writeCabeceraLinea()
   ...
   cLine          += AllTrim( getCustomExtraField( "042", "Artículos", ( D():Articulos( ::nView ) )->Codigo ) ) + __separator__
   cLine          += "EN" 
