...
Function afterSave( cNumeroFactura, nMode, nView, oStock )
	      
   local oActualizaCostos

   if nMode != APPD_MODE
      Return nil
   end if

   oActualizaCostos   := tActualizaCostos():New( cNumeroFactura, nView, oStock )
...
METHOD New( cNumeroFactura, nView, oStock ) CLASS tActualizaCostos

   ::nView                    := nView

   ::oStock                   := oStock

   ::cNumeroFactura           := cNumeroFactura
   ...
   */
   ...
   for each hLine in ::aLinesFacturaProveedor
      ...
      if ( D():Articulos( ::nView ) )->( dbSeek( hGet( hLine, "codigo" ) ) )
         ...
         nUnidades   := nStockAct + hGet( hLine, "unidades" )
   ...
   RECOVER USING oError
      ...
      msgStop( ErrorMessage( oError ), "Error en el script aftersave"  )
