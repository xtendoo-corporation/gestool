...
METHOD AddLineasFactura() CLASS CreaFacturaCliente 
   ...
   */
   ...
   while !( ::dbfTmpLin )->( Eof() )
      ...
      ( D():FacturasClientesLineas( ::nView ) )->nPreUnit   := 0 //( ::dbfTmpLin )->nPreDiv
      ( D():FacturasClientesLineas( ::nView ) )->nCanEnt    := 1 //( ::dbfTmpLin )->nCanPed
      ( D():FacturasClientesLineas( ::nView ) )->nUniCaja   := 1 //( ::dbfTmpLin )->nUniCaja
