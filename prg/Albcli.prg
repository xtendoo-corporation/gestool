...
#define _MENUITEM_               "albaranes_de_clientes"
...
#define _LVALIDADO               115
#define _LPREPARADO              116
#define _CIDNUMCOM               117
...
STATIC FUNCTION GenAlbCli( nDevice, cCaption, cCodigoDocumento, cPrinter, nCopies )
   ...
   /*
   Ejecuto script antes de imprimir--------------------------------------------
   */
...
STATIC FUNCTION EdtRec( aTmp, aGet, dbf, oBrw, hHash, bValid, nMode )
   ...
   */
   ...
   cSay[ 10]                  := UsuariosModel():getNombreWhereCodigo( aTmp[ _CCODUSR ] )
   ...
   */
   ...
   DEFINE DIALOG oDlg ;
      ...
      */
      ...
      REDEFINE GET aGet[ _CCODUSR ] VAR aTmp[ _CCODUSR ];
         ID       125 ;
         WHEN     ( .f. ) ;
         VALID    ( oSay[ 10 ]:cText( UsuariosModel():getNombreWhereCodigo( aTmp[ _CCODUSR ] ) ), .t.  ) ;
         OF       oFld:aDialogs[2]
      ...
      */
      ...
      REDEFINE GET aGet[ _CNUMPED ] VAR aTmp[ _CNUMPED ] ;
         ...
         VALID    ( cPedCli( aGet, aTmp, oBrwLin, nMode ), RecalculaTotal( aTmp ), SetDialog( aGet, oSayDias, oSayTxtDias ) );
         WHEN     ( lWhen .and. ( dbfTmpLin )->( ordKeyCount() ) == 0 ) ;
   ...
   ACTIVATE DIALOG   oDlg ;
      ON INIT        ( initEdtRec( aTmp, aGet, oDlg, oSayDias, oSayTxtDias, hHash ) );
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "LFACCLI" TITLE LblTitle( nMode ) + "líneas a albaranes de clientes"
      ...
      */
      ...
      REDEFINE GET aGet[ _CALMLIN ] VAR aTmp[ _CALMLIN ] ;
         ID       300 ;
         WHEN     ( !aTmp[ _LCONTROL ] .and. nMode != ZOOM_MODE ) ;         
         VALID    ( cAlmacen( aGet[ _CALMLIN ], , oSayAlm ), if( !uFieldEmpresa( "lNStkAct" ), StocksModel():lPutStockActual( aTmp[ _CREF ], aTmp[ _CALMLIN ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ], oStkAct ), .t. ) ) ;
         BITMAP   "LUPA" ;
      ...
      TBtnBmp():ReDefine( 322, "gc_businessman_16",,,,,{|| THistoricoProveedor():run( aTmp[ _CREF ], aGet[ _NCOSDIV ], aGet[ _CCODPRV ], aGet[ _CIDNUMCOM ] ) }, oFld:aDialogs[1], .f., , .f.,  )
      ...
      */
      ...
      REDEFINE CHECKBOX aGet[ _LPREPARADO ] VAR aTmp[ _LPREPARADO ]  ;
         ID       370 ;
         WHEN     ( Auth():isSuperAdmin() .and. nMode != ZOOM_MODE ) ;
         OF       oFld:aDialogs[ 2 ]
      ...
      REDEFINE GET aGet[ _CIDNUMCOM ] VAR aTmp[ _CIDNUMCOM ] ;
         ID       390 ;
         WHEN     ( .f. ) ;
         OF       oFld:aDialogs[2]
   ...
   if nMode != ZOOM_MODE
      /*if uFieldEmpresa( "lGetLot")
         oDlg:AddFastKey( VK_RETURN,   {|| if( !Empty( aGet[ _CREF ] ), ( aGet[ _CVALPR1 ]:lValid(), aGet[ _CVALPR2 ]:lValid(), MsgInfo( "PASO POR lGetLot" ) ), ), oBtn:SetFocus(), oBtn:Click() } )
      end if */
      ...
      */
      ...
      if ( D():Articulos( nView ) )->( dbSeek( ( dbfTmpLin )->cRef ) )
         ...
         */
         ...
         ( dbfTmpLin )->nCosDiv     := nCosto( nil, D():Articulos( nView ), dbfKit, , , , aTmpAlb[ _CCODCLI ] )
         ...
         */
         ...
         if !empty( hAtipica )
            ...
            if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
               ( dbfTmpLin )->nPreUnit    := nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpAlb[ _CDIVALB ], aTmpAlb[ _LIVAINC ], D():Articulos( nView ), D():Get( "Divisas", nView ), dbfKit, D():Get( "TIva", nView ), , , oNewImp )
            end if
...
Static Function ChangeTarifa( aTmp, aGet, aTmpAlb )
   ...
   if !empty( aTmpAlb[ _CCODTAR ] )
      nPrePro  := RetPrcTar( aTmp[ _CREF ], aTmpAlb[ _CCODTAR ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], dbfTarPreL, aTmp[ _NTARLIN ] )
   end if
...
Static Function DataReport( oFr )
   ...
   */
   ...
   oFr:setUserDataSet( "Impuestos albarán",;
            ...
            */
            ...
            if uFieldempresa( "lDtoCliLin" )
               ...
               if !empty( aGet ) .and. !empty( aGet[ _NDTO ] )
                  aGet[ _NDTO ]:cText( retFld( aTmpAlb[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" ) )
                  aTmp[ _NDTO ]     := retFld( aTmpAlb[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               else
                  aTmp[ _NDTO ]     := retFld( aTmpAlb[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               end if         
            ...
            //if Empty( aTmpAlb[ _CNUMPED ] ) //Para que no modifique cuando venga de un pedido (TEFESA)
               ...
               if !empty( hAtipica )
                  ...
                  if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
                     ...
                     aTmp[ _NPREUNIT ]    := nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpAlb[ _CDIVALB ], aTmpAlb[ _LIVAINC ], D():Articulos( nView ), D():Get( "Divisas", nView ), dbfKit, D():Get( "TIva", nView ) , , , oNewImp ) 
                     
                     if empty(aGet)
                        aGet[ _NPREUNIT ]:cText( nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpAlb[ _CDIVALB ], aTmpAlb[ _LIVAINC ], D():Articulos( nView ), D():Get( "Divisas", nView ), dbfKit, D():Get( "TIva", nView ) , , , oNewImp ) )
         ...
         if ( lChgCodArt ) .or. ( lChgLotArt )
            ...
            do case
               case uFieldEmpresa( "nCosVta" ) < 2
                  ...
                  if nCosPro == 0
                     nCosPro        := nCosto( aTmp[ _CREF ], D():Articulos( nView ), dbfKit, .f., , D():Get( "Divisas", nView ), aTmpAlb[ _CCODCLI ] )
                  end if
...
STATIC FUNCTION EndTrans( aTmp, aGet, oBrw, oBrwInc, nMode, oDlg )
   ...
   */
   ...
   if lLimitDaysPendiente( aTmp[ _CCODCLI ] )
   ...
   while !( dbfTmpLin )->( eof() )
      ...
      if !( dbfTmpLin )->lKitChl
         ...
         AtipicasModel():SaveUltimoPrecioVenta( {  "cCodCli" => aTmp[ _CCODCLI ] ,;
   ...
   */
   ...
   //********************Por aquí pierde tiempo lo de boni*******************//
...
Return ( printReportAlbCli( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
   ...
   if OpenFiles()
      ...
      while !( D():Get( "AlbCliT", nView ) )->( eof() )
         ...
         */
         ...
         /*
         Revisamos los estados de los albaranes--------------------------------
         */
...
FUNCTION nTotAlbCli( cAlbaran, cAlbCliT, cAlbCliL, cIva, cDiv, aTmp, cDivRet, lPic, lExcCnt, lNeto )
   ...
   */
   ...
   /*if nSbrAtp <= 1 .and. nDtoAtp != 0
      ...
      aTotalAtp[1]   := _NBASIVA1 * nDtoAtp / 100
      aTotalAtp[2]   := _NBASIVA2 * nDtoAtp / 100
      aTotalAtp[3]   := _NBASIVA3 * nDtoAtp / 100
   ...
   if nDtoEsp  != 0
      ...
      aTotalDto[1]   := _NBASIVA1 * nDtoEsp / 100
      aTotalDto[2]   := _NBASIVA2 * nDtoEsp / 100
      aTotalDto[3]   := _NBASIVA3 * nDtoEsp / 100
   ...
   IF nDtoPP   != 0
      ...
      aTotalDPP[1]   := _NBASIVA1 * nDtoPP / 100
      aTotalDPP[2]   := _NBASIVA2 * nDtoPP / 100
      aTotalDPP[3]   := _NBASIVA3 * nDtoPP / 100
   ...
   IF nDtoUno != 0
      ...
      aTotalUno[1]   := _NBASIVA1 * nDtoUno / 100
      aTotalUno[2]   := _NBASIVA2 * nDtoUno / 100
      aTotalUno[3]   := _NBASIVA3 * nDtoUno / 100
   ...
   IF nDtoDos != 0
      ...
      aTotalDos[1]   := _NBASIVA1 * nDtoDos / 100
      aTotalDos[2]   := _NBASIVA2 * nDtoDos / 100
      aTotalDos[3]   := _NBASIVA3 * nDtoDos / 100
