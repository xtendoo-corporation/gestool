...
#define _MENUITEM_                  "albaranes_de_proveedores"
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "PEDPRV" TITLE LblTitle( nMode ) + "albaranes de proveedores" 
      ...
      */
      ...
      with object ( oBrwIva:AddCol() )
         :cHeader          := "R.E."
         :bStrData         := {|| if( len( aTotIva ) > 0, if( ( hhaskey( aTotIva[ oBrwIva:nArrayAt ], "impre" ) .and. hGet( aTotIva[ oBrwIva:nArrayAt ], "impre" ) != nil .and. aTmp[ _LRECARGO ] ), Trans( hGet( aTotIva[ oBrwIva:nArrayAt ], "impre" ), cPirDiv ), "" ), "" ) }
         :nWidth           := 76
         :nDataStrAlign    := 1
         :nHeadStrAlign    := 1
      end with
...
static FUNCTION nTotUnd( uDbf )
   ...
   if ValType( uDbf ) == "A"
      nTotUnd  := NotBulto( uDbf[ __NBULTOS ] ) * NotCaja( uDbf[ _NCANENT ] ) * uDbf[ _NUNICAJA ]
   else
   ...
   */
   ...
   while ( D():AlbaranesProveedoresLineas( nView ) )->( dbSeek( ( D():AlbaranesProveedores( nView ) )->cSerAlb + Str( ( D():AlbaranesProveedores( nView ) )->nNumAlb ) + ( D():AlbaranesProveedores( nView ) )->cSufAlb ) ) .and. !( D():AlbaranesProveedoresLineas( nView ) )->( eof() )
      ...
      if !Empty( ( D():AlbaranesProveedoresLineas( nView ) )->cCodPed )
         ...
         if PedidosProveedoresLineasModel():ExisteLinea( ( D():AlbaranesProveedoresLineas( nView ) )->cCodPed,;
                                                         ( D():AlbaranesProveedoresLineas( nView ) )->cRef,;
                                                         ( D():AlbaranesProveedoresLineas( nView ) )->cCodPr1,;
                                                         ( D():AlbaranesProveedoresLineas( nView ) )->cCodPr2,;
                                                         ( D():AlbaranesProveedoresLineas( nView ) )->cValPr1,;
                                                         ( D():AlbaranesProveedoresLineas( nView ) )->cValPr2,;
                                                         ( D():AlbaranesProveedoresLineas( nView ) )->cLote )
         ...
         IF ( D():PedidosProveedoresLineas( nView ) )->( dbSeek( aAlbaranes[ nItem, 3] ) ) .AND. aAlbaranes[ nItem, 1]
            ...
            while ( ( D():PedidosProveedoresLineas( nView ) )->cSerPed + Str( ( D():PedidosProveedoresLineas( nView ) )->nNumPed ) + ( D():PedidosProveedoresLineas( nView ) )->cSufPed == aAlbaranes[ nItem, 3 ] )
               ...
               if aAlbaranes[ nItem, 2 ] == 2
                  ...
                  nTotPed              := nTotNPedPrv( D():PedidosProveedoresLineas( nView ) )
                  nTotRec              := AlbaranesProveedoresLineasModel():nUnidadesRecibidas( aAlbaranes[ nItem, 3 ],;
                                                                                                ( D():PedidosProveedoresLineas( nView ) )->cRef,;
                                                                                                ( D():PedidosProveedoresLineas( nView ) )->cCodPr1,;
                                                                                                ( D():PedidosProveedoresLineas( nView ) )->cCodPr2,;
                                                                                                ( D():PedidosProveedoresLineas( nView ) )->cValPr1,;
...
RETURN ( printReportAlbPrv( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
...
FUNCTION nTotAlbPrv( nAlbaran, cAlbPrvT, cAlbPrvL, cIva, cDiv, aTmp, cDivRet, lPic )
   ...
   IF aTmp != NIL
      ...
      nRouDiv        := nRouDiv( aTmp[ _CDIVALB ], cDiv )
...
FUNCTION SynAlbPrv( cPath )
   ...
   USE ( cPatEmp() + "ALBPROVT.Dbf" ) NEW VIA ( cDriver() )    ALIAS ( cCheckArea( "ALBPROVT", @cAlbPrvT ) )
   SET ADSINDEX TO ( cPatEmp() + "ALBPROVT.CDX" ) ADDITIVE

   USE ( cPatEmp() + "ALBPROVL.Dbf" ) NEW VIA ( cDriver() )   ALIAS ( cCheckArea( "ALBPROVL", @cAlbPrvL ) )
   SET ADSINDEX TO ( cPatEmp() + "ALBPROVL.CDX" ) ADDITIVE

   USE ( cPatEmp() + "ALBPRVS.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "ALBPRVS", @cAlbPrvS ) )
   SET ADSINDEX TO ( cPatEmp() + "ALBPRVS.CDX" ) ADDITIVE

   USE ( cPatEmp() + "ALBPRVI.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "ALBPRVI", @cAlbPrvI ) )
   SET ADSINDEX TO ( cPatEmp() + "ALBPRVI.CDX" ) ADDITIVE

   USE ( cPatEmp() + "FAMILIAS.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "FAMILIAS", @cFamilia ) )
   SET ADSINDEX TO ( cPatEmp() + "FAMILIAS.CDX" ) ADDITIVE

   USE ( cPatEmp() + "ARTICULO.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "ARTICULO", @cArticulo ) )
   SET ADSINDEX TO ( cPatEmp() + "ARTICULO.CDX" ) ADDITIVE

   USE ( cPatEmp() + "PROVART.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "PROVART", @cArtPrv ) )
   SET ADSINDEX TO ( cPatEmp() + "PROVART.CDX" ) ADDITIVE

   USE ( cPatDat() + "TIVA.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "TIVA", @cIva ) )
   SET ADSINDEX TO ( cPatDat() + "TIVA.CDX" ) ADDITIVE

   USE ( cPatDat() + "DIVISAS.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "DIVISAS", @cDiv ) )
   SET ADSINDEX TO ( cPatDat() + "DIVISAS.CDX" ) ADDITIVE

   USE ( cPatEmp() + "PEDPROVT.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "PEDPROVT", @cPedPrvT ) )
   SET ADSINDEX TO ( cPatEmp() + "PEDPROVT.CDX" ) ADDITIVE

   USE ( cPatEmp() + "PEDPROVL.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "PEDPROVL", @cPedPrvL ) )
   SET ADSINDEX TO ( cPatEmp() + "PEDPROVL.CDX" ) ADDITIVE

   USE ( cPatEmp() + "ArtDiv.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "ArtDiv", @cArtDiv ) )
...
FUNCTION RollBackAlbPrvFromFacPrv( cNumAlb, nView )
   ...
   if ( D():AlbaranesProveedoresLineas( nView ) )->( dbSeek( cNumAlb ) )
      ...
      while ( D():AlbaranesProveedoresLineas( nView ) )->cSerAlb + Str( ( D():AlbaranesProveedoresLineas( nView ) )->nNumAlb ) + ( D():AlbaranesProveedoresLineas( nView ) )->cSufAlb == cNumAlb .and.;
         !( D():AlbaranesProveedoresLineas( nView ) )->( Eof() )
...
Function CreateAlbPrvQrCode( cText, cFileName )
   ...
   //MsgInfo( hb_strtoutf8( cText ), cText )
