...
#define fldAtipicas                 oFld:aDialogs[3]
#define fldTactil                   oFld:aDialogs[4] 
#define fldObservaciones            oFld:aDialogs[5]
#define fldAdicionales              oFld:aDialogs[6] 
#define fldImagenes                 oFld:aDialogs[7]
#define fldPropiedades              oFld:aDialogs[8]
#define fldLogistica                oFld:aDialogs[9]
#define fldStocks                   oFld:aDialogs[10] 
#define fldContabilidad             oFld:aDialogs[11]
#define fldOfertas                  oFld:aDialogs[12]
#define fldEscandallos              oFld:aDialogs[13]
#define fldWeb                      oFld:aDialogs[14]
#define fldDescripciones            oFld:aDialogs[15]
#define fldDocumentos               oFld:aDialogs[16]
#define fldAsociados                oFld:aDialogs[17]
#define fldKit                      oFld:aDialogs[18]
...
STATIC FUNCTION OpenFiles( lExt, cPath )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPatEmp() + "PROVEE.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "PROVEE", @dbfProv ) )
      ...
      USE ( cPatEmp() + "PARKIT.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "PARTIK", @dbfParKit ) )
      SET ADSINDEX TO ( cPatEmp() + "PARKIT.CDX" ) ADDITIVE

      USE ( cPatEmp() + "ARTASC.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "ARTASC", @dbfArtAsc ) )
      SET ADSINDEX TO ( cPatEmp() + "ARTASC.CDX" ) ADDITIVE
...
Function Articulo( oMenuItem, oWnd, bOnInit )
   ...
   */
   ...
   DEFINE BTNSHELL RESOURCE "gc_package_" GROUP OF oWndBrw ;
      NOBORDER ;
      ACTION   ( BrwStkArt( nView ) ) ;
      TOOLTIP  "Informe stoc(k)" ;
      HOTKEY   "K" ;
      LEVEL    ACC_ZOOM

   if getPvProfString( "Gamma", "ViewGamma", ".F.", cIniAplication() ) == ".T."

      DEFINE BTNSHELL RESOURCE "gamma_" GROUP OF oWndBrw ;
         NOBORDER ;
         ACTION   ( ImportGamma():New():getStockProduct( AllTrim( ( D():Articulos( nView ) )->Codigo ) ) ) ;
         TOOLTIP  "Gamma" ;
         HOTKEY   "G" ;
         LEVEL    ACC_ZOOM
   ...
   DEFINE BTNSHELL RESOURCE "IMP" GROUP OF oWndBrw ;
      NOBORDER ;
      ACTION   ( TInfArtFam():New( "Listado de artículos" ):Play( .f., D():Articulos( nView ), dbfDiv, dbfArtKit, D():TiposIva( nView ), D():Familias( nView ), oWndBrw ) );
      TOOLTIP  "Lis(t)ado";
      HOTKEY   "T" ;
      LEVEL    ACC_IMPR
   ...
   DEFINE BTNSHELL RESOURCE "gc_document_empty_chart_" OF oWndBrw ;
      NOBORDER ;
      ACTION   ( runFastGallery( "Articulos" ) ) ;
      TOOLTIP  "Rep(o)rting";
      HOTKEY   "O" ;
      LEVEL    ACC_IMPR
   ...
   DEFINE BTNSHELL RESOURCE "WordPress_" OF oWndBrw ;
      NOBORDER ;
      ACTION   ( msgRun( "Subiendo artículo", "Espere por favor...", {|| ImportLatress():New():addOneArticulo( dbHash( D():Articulos( nView ) ) ) } ) );
      TOOLTIP  "Subir (w)eb" ;
      HOTKEY   "W";
      LEVEL    ACC_EDIT
   ...
   DEFINE BTNSHELL oRotor RESOURCE "ROTOR" GROUP OF oWndBrw ;
      ...
      if Auth():isSuperAdmin()
         ...
         DEFINE BTNSHELL RESOURCE "WORDPRESS_" OF oWndBrw ;
            ACTION   ( if( !Empty( ( D():Articulos( nView ) )->cIdWP ), ImportLatress():New():desactivateArticulo( ( D():Articulos( nView ) )->cIdWP ), ) );
            TOOLTIP  "Desactivar en WP" ;
            FROM     oRotor ;
            ALLOW    EXIT ;
            LEVEL    ACC_EDIT

         DEFINE BTNSHELL RESOURCE "WORDPRESS_" OF oWndBrw ;
            ACTION   ( if( !Empty( ( D():Articulos( nView ) )->cIdWP ), ImportLatress():New():activateArticulo( ( D():Articulos( nView ) )->cIdWP ), ) );
            TOOLTIP  "Activar en WP" ;
            FROM     oRotor ;
            ALLOW    EXIT ;
            LEVEL    ACC_EDIT

         DEFINE BTNSHELL RESOURCE "WORDPRESS_" OF oWndBrw ;
            ACTION   ( if( !Empty( ( D():Articulos( nView ) )->cIdWP ), ImportLatress():New():ActStockArticulo( ( D():Articulos( nView ) )->Codigo, ( D():Articulos( nView ) )->cIdWP ), ) );
            TOOLTIP  "Actualiza stock" ;
            FROM     oRotor ;
            ALLOW    EXIT ;
            LEVEL    ACC_EDIT
...
STATIC FUNCTION EdtRec( aTmp, aGet, cArticulo, oBrw, bWhen, bValid, nMode )
   ...
   */
   ...
   REDEFINE GET aGet[ ( D():Articulos( nView ) )->( fieldpos( "cCodFab" ) ) ] ;
      ...
      ID       ( 235 ) ;
      IDTEXT   ( 236 ) ;
      VALID    ( oFabricantes:Existe( aGet[ ( D():Articulos( nView ) )->( fieldpos( "cCodFab" ) ) ], aGet[ ( D():Articulos( nView ) )->( fieldpos( "cCodFab" ) ) ]:oHelpText ) ) ;
      ON HELP  ( oFabricantes:Buscar( aGet[ ( D():Articulos( nView ) )->( fieldpos( "cCodFab" ) ) ] ) ) ;
      WHEN     ( nMode != ZOOM_MODE ) ;
      BITMAP   "LUPA" ;
      OF       fldGeneral
   ...
   */
   ...
   REDEFINE GET aGet[ ( D():Articulos( nView ) )->( fieldpos( "cExtFld" ) ) ] ;
         VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "cExtFld" ) ) ];
         ID       235 ;
         IDTEXT   236 ;
         VALID    ( oCamposExtra:Existe( aGet[ ( D():Articulos( nView ) )->( fieldpos( "cExtFld" ) ) ], aGet[ ( D():Articulos( nView ) )->( fieldpos( "cExtFld" ) ) ]:oHelpText, "cNombre" ) ) ;
         BITMAP   "LUPA" ;
         ON HELP  ( oCamposExtra:Buscar( aGet[ ( D():Articulos( nView ) )->( fieldpos( "cExtFld" ) ) ], "Clientes" ) );
         OF       fldAdicionales
   ...
   REDEFINE CHECKBOX aTmp[ ( D():Articulos( nView ) )->( fieldpos( "lDonativ" ) ) ];
         ID       140 ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         OF       fldAdicionales
   ...
   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "nIncPrc1" ) ) ] ;
         VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "nIncPrc1" ) ) ] ;
         ID       292 ;
         WHEN     ( nMode != ZOOM_MODE );
         PICTURE  "@E 999,999.999999" ;
         OF       fldAdicionales

   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "nIncPrc2" ) ) ] ;
         VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "nIncPrc2" ) ) ] ;
         ID       293 ;
         WHEN     ( nMode != ZOOM_MODE );
         PICTURE  "@E 999,999.999999" ;
         OF       fldAdicionales

   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "nIncPrc3" ) ) ] ;
         VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "nIncPrc3" ) ) ] ;
         ID       294 ;
         WHEN     ( nMode != ZOOM_MODE );
         PICTURE  "@E 999,999.999999" ;
         OF       fldAdicionales
   ...
   */
   ...
   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "NRENMIN" ) ) ] ;
      ...
      */
      ...
      REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "cTimChg" ) ) ] ;
         VAR         aTmp[ ( D():Articulos( nView ) )->( fieldpos( "cTimChg" ) ) ] ;
         ID          198 ;
         WHEN        ( .f. ) ;
         OF          fldLogistica
   ...
   */
   ...
   REDEFINE GET aTmp[( D():Articulos( nView ) )->( fieldpos( "NBULENT" ) ) ] ;
      ID       181 ;
      SPINNER ;
      WHEN     ( nMode != ZOOM_MODE ) ;
      PICTURE  cPicUnd ;
   ...
   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "nUndPal" ) ) ] ;
      VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "nUndPal" ) ) ] ;
      ID       401 ;
      WHEN     ( nMode != ZOOM_MODE );
      PICTURE  "@E 999,999.999999" ;
      OF       fldLogistica
   ...
   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "M2Palet" ) ) ] ;
      VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "M2Palet" ) ) ] ;
      ID       402 ;
      WHEN     ( nMode != ZOOM_MODE );
      PICTURE  "@E 999,999.999999" ;
      OF       fldLogistica

   REDEFINE GET   aGet[ ( D():Articulos( nView ) )->( fieldpos( "M2Pieza" ) ) ] ;
      VAR      aTmp[ ( D():Articulos( nView ) )->( fieldpos( "M2Pieza" ) ) ] ;
      ID       403 ;
      WHEN     ( nMode != ZOOM_MODE );
      PICTURE  "@E 999,999.999999" ;
      OF       fldLogistica 
   ...
   */
   ...
   REDEFINE GET aTmp[ ( D():Articulos( nView ) )->( fieldpos( "cIdWP" ) ) ] ;
         ID       211 ;
         WHEN     ( .F. );
         OF       fldWeb

   TBtnBmp():ReDefine( 162, "WordPress_16",,,,,{|| msgRun( "Subiendo artículo", "Espere por favor...", {|| ImportLatress():New():addOneArticulo( dbHash( D():Articulos( nView ) ) ) } ) }, fldWeb, .f., , .f.,  )
   ...
   */
   ...
   /*
   Caja de atipicas de clientes------------------------------------------------
   */

   REDEFINE BITMAP oBmpAtipicas ;
      ID       500 ;
      RESOURCE "gc_symbol_euro_48" ;
      TRANSPARENT ;
      OF       fldAtipicas

   REDEFINE BUTTON oBtnEditAtp;
      ID       510 ;
      OF       fldAtipicas;
      WHEN     ( nMode != ZOOM_MODE ) ;
      ACTION   ( if( nMode != ZOOM_MODE, WinEdtRec( oBrwAtp, bEdtAtp, dbfTmpAtp, aTmp, nMode ), ) )

   oBrwAtp                    := IXBrowse():New( fldAtipicas )
   ...
   */
   ...
   REDEFINE BUTTON aBtn[15] ;
         ...
         OF       fldAsociados;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( WinAppRec( oBrwAsc, bEdtAsc, dbfTmpAsc, , , aTmp ) )

   REDEFINE BUTTON aBtn[16] ;
         ID       502 ;
         OF       fldAsociados;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( dbDelRec( oBrwAsc, dbfTmpAsc ) )

   oBrwAsc                 := IXBrowse():New( fldAsociados )

   oBrwAsc:bClrSel         := {|| { CLR_BLACK, Rgb( 229, 229, 229 ) } }
   oBrwAsc:bClrSelFocus    := {|| { CLR_BLACK, Rgb( 167, 205, 240 ) } }
   ...
   REDEFINE BUTTON aBtn[11] ;
         ...
         OF       fldKit;
         WHEN     ( nMode != ZOOM_MODE .AND. aTmp[ ( D():Articulos( nView ) )->( fieldpos( "LKITART" ) ) ] ) ;
         ACTION   ( WinAppRec( oBrwEsc, bEdtParKit, dbfTmpParKit, , , aTmp ),  Eval( oBrwEsc:bValid ) )

   REDEFINE BUTTON ;
         ID       503 ;
         OF       fldKit;
         WHEN     ( nMode != ZOOM_MODE .AND. aTmp[ ( D():Articulos( nView ) )->( fieldpos( "LKITART" ) ) ] ) ;
         ACTION   ( WinDupRec( oBrwEsc, bEdtParKit, dbfTmpParKit, , , aTmp ),  Eval( oBrwEsc:bValid ) )

   REDEFINE BUTTON aBtn[12] ;
         ID       501 ;
         OF       fldKit;
         WHEN     ( nMode != ZOOM_MODE .AND. aTmp[ ( D():Articulos( nView ) )->( fieldpos( "LKITART" ) ) ] ) ;
         ACTION   ( WinEdtRec( oBrwEsc, bEdtParKit, dbfTmpParKit, , , aTmp ),  Eval( oBrwEsc:bValid ) )

   REDEFINE BUTTON aBtn[13] ;
         ID       502 ;
         OF       fldKit;
         WHEN     ( nMode != ZOOM_MODE .AND. aTmp[ ( D():Articulos( nView ) )->( fieldpos( "LKITART" ) ) ] ) ;
         ACTION   ( dbDelRec( oBrwEsc, dbfTmpParKit ), Eval( oBrwEsc:bValid ) )

   oBrwEsc                 := IXBrowse():New( fldKit )

   oBrwEsc:bClrSel         := {|| { CLR_BLACK, Rgb( 229, 229, 229 ) } }
   oBrwEsc:bClrSelFocus    := {|| { CLR_BLACK, Rgb( 167, 205, 240 ) } }
   ...
   //oBrwEsc:bRClicked       := {| nRow, nCol, nFlags | oBrwEsc:RButtonDown( nRow, nCol, nFlags ) }
   ...
   */
   ...
   if nMode != ZOOM_MODE
      ...
      fldAtipicas:AddFastKey( VK_F3, {|| oBtnEditAtp:Click() } )
   ...
   ACTIVATE DIALOG oDlg ;
      ...
      VALID       (  KillTrans( oMenu, oBmpTemporada, oBmpEstado, oBmpGeneral, oBmpPrecios, oBmpDescripciones, oBmpPropiedades, oBmpLogistica, oBmpStocks, oBmpContabilidad, oBmpOfertas, oBmpEscandallos, oBmpWeb, oBmpUbicaciones, oBmpImagenes, oBmpTactil, oBmpAdicionales, oBmpAtipicas, oBmpDocumentos, oBmpAsociados ) )
...
Static Function BeginTrans( aTmp, nMode )
   ...
   */
   ...
   if nMode != APPD_MODE .and. ( dbfParKit )->( dbSeek( cCodArt ) )
   ...
   */
   ...
   if nMode != APPD_MODE .and. ( dbfArtAsc )->( dbSeek( cCodArt ) )
   ...
   */
   ...
   if nMode != APPD_MODE .and. ( D():ArticuloDocumentos( nView ) )->( dbSeek( cCodArt ) )
   ...
   */
   ...
   if nMode != APPD_MODE .and. nMode != DUPL_MODE .and. ( D():Atipicas( nView ) )->( dbSeek( cCodArt ) )
      
      while ( D():Atipicas( nView ) )->cCodArt == cCodArt .and. !( D():Atipicas( nView ) )->( eof() )
      
         if ( ( Empty( ( D():Atipicas( nView ) )->dFecIni ) .and. Empty( ( D():Atipicas( nView ) )->dFecFin ) ) .or.;
            ( ( D():Atipicas( nView ) )->dFecIni <= GetSysDate() .and. ( D():Atipicas( nView ) )->dFecFin >= GetSysDate() ) )
...
Static Function EndTrans( aTmp, aGet, oSay, oDlg, aTipBar, cTipBar, nMode, oImpComanda1, oImpComanda2, lActualizaWeb )
   ...
   BEGIN SEQUENCE
      ...
      */
      ...
      /*
      Pasamos a definitivo las atipicas de clientes----------------------------
      */
...
STATIC FUNCTION EdtDet( aTmp, aGet, dbfArtPrv, oBrw, bWhen, bValid, nMode )
   ...
   TBtnBmp():ReDefine( 121, "gc_cabinet_open_16",,,,,{|| if( nMode != APPD_MODE .and. nMode != DUPL_MODE, InfoCompras():run( aTmp[ ( dbfTmpPrv )->( fieldPos( "CCODPRV" ) ) ], aTmp[ ( dbfTmpPrv )->( fieldPos( "cCodArt" ) ) ] ), MsgInfo( "Función desactivada añadiendo" ) ) }, oDlg, .f., , .f.,  )
   ...
   REDEFINE GET aGet[ ( dbfTmpPrv )->( fieldPos( "NSBRCOS" ) ) ];
      VAR      aTmp[ ( dbfTmpPrv )->( fieldPos( "NSBRCOS" ) ) ];
      SPINNER ;
      ID       300 ;
      PICTURE  cPinDiv ;
      WHEN     ( nMode != ZOOM_MODE );
   ...
   REDEFINE GET aGet[ ( dbfTmpPrv )->( fieldPos( "NBANDEJ" ) ) ];
      VAR      aTmp[ ( dbfTmpPrv )->( fieldPos( "NBANDEJ" ) ) ];
      SPINNER ;
      ID       310 ;
      PICTURE  cPinDiv ;
      WHEN     ( nMode != ZOOM_MODE );
   ...
   REDEFINE GET aGet[ ( dbfTmpPrv )->( fieldPos( "NUNDBAN" ) ) ];
      VAR      aTmp[ ( dbfTmpPrv )->( fieldPos( "NUNDBAN" ) ) ];
      SPINNER ;
      ID       320 ;
      PICTURE  cPinDiv ;
      WHEN     ( nMode != ZOOM_MODE );
...
STATIC FUNCTION EdtVta( aTmp, aGet, dbfTmpVta, oBrw, bWhen, bValid, nMode, aArt )
   ...
   */
      ...
      REDEFINE GET aGet[ ( dbfTmpVta )->( fieldpos( "cCodWp" ) ) ] ;
         VAR      aTmp[ ( dbfTmpVta )->( fieldpos( "cCodWp" ) ) ] ; 
         ID       700 ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         OF       oFld:aDialogs[1] ;
...
STATIC FUNCTION EdtAtipica( aTmp, aGet, dbfTmpAtp, oBrw, aTmpArt, nModeArt, nMode )
   ...
   DEFINE DIALOG oDlg RESOURCE "ARTATIPICAS" TITLE LblTitle( nMode ) + "tarifas de clientes"
      ...
      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "cCodCli" ) ) ] ;
         VAR      aTmp[ ( dbfTmpAtp )->( fieldPos( "cCodCli" ) ) ] ;
         ID       100 ;
         WHEN     ( .f. );
         OF       oDlg

      REDEFINE GET oGetNomCli VAR cGetNomCli ;
         ID       110 ;
         WHEN     ( .f. );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDtoArt" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDtoArt" ) ) ];
         ID       130 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDtoArt" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDtoArt" ) ) ] <= 100 ) )  ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDtoDiv" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDtoDiv" ) ) ];
         ID       135 ;
         SPINNER ;
         PICTURE  cPouDiv() ;
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDprArt" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDprArt" ) ) ];
         ID       601 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDprArt" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDprArt" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE CHECKBOX aTmp[ ( dbfTmpAtp )->( fieldPos( "lComAge" ) ) ] ;
         ID       151 ;
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nComAge" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nComAge" ) ) ];
         ID       150 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nComAge" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nComAge" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg


      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcCom" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcCom" ) ) ];
         ID       120 ;
         SPINNER  ;
         PICTURE  cPinDiv() ;
         OF       oDlg
      ...
      */
      ...
      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt" ) ) ];
         ID       121 ;
         SPINNER  ;
         PICTURE  cPouDiv ;
         WHEN     ( !aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] );
         ON CHANGE( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], , aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva1" ) ) ] ) );
         VALID    ( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], , aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva1" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt2" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt2" ) ) ];
         ID       124 ;
         SPINNER  ;
         PICTURE  cPouDiv ;
         WHEN     ( !aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] );
         ON CHANGE( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt2" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva2" ) ) ] ) );
         VALID    ( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt2" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva2" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt3" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt3" ) ) ];
         ID       125 ;
         SPINNER  ;
         PICTURE  cPouDiv ;
         WHEN     ( !aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] );
         ON CHANGE( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt3" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva3" ) ) ] ) );
         VALID    ( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt3" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva3" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt4" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt4" ) ) ];
         ID       126 ;
         SPINNER  ;
         PICTURE  cPouDiv ;
         WHEN     ( !aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] );
         ON CHANGE( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt4" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva4" ) ) ] ) );
         VALID    ( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt4" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva4" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt5" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt5" ) ) ];
         ID       127 ;
         SPINNER  ;
         PICTURE  cPouDiv ;
         WHEN     ( !aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] );
         ON CHANGE( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt5" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva5" ) ) ] ) );
         VALID    ( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt5" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva5" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt6" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt6" ) ) ];
         ID       128 ;
         SPINNER  ;
         PICTURE  cPouDiv ;
         WHEN     ( !aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] );
         ON CHANGE( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt6" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva6" ) ) ] ) );
         VALID    ( CalIvaAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPrcArt6" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva6" ) ) ] ) );
         OF       oDlg
      ...
      */
      ...
      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva1" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva1" ) ) ] ;
         ID       300 ;
         SPINNER ;
         PICTURE  cPouDiv ;
         WHEN     ( aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] ) ;
         ON CHANGE( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva1" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], , aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt" ) )] ) );
         VALID    ( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva1" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], , aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt" ) )] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva2" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva2" ) ) ] ;
         ID       310 ;
         PICTURE  cPouDiv ;
         SPINNER ;
         WHEN     ( aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] ) ;
         ON CHANGE( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva2" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt2" ) ) ] ) );
         VALID    ( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva2" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt2" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva3" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva3" ) ) ] ;
         ID       320 ;
         SPINNER ;
         PICTURE  cPouDiv ;
         WHEN     ( aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] ) ;
         ON CHANGE( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva3" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt3" ) ) ] ) );
         VALID    ( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva3" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt3" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva4" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva4" ) ) ] ;
         ID       330 ;
         SPINNER ;
         PICTURE  cPouDiv ;
         WHEN     ( aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] ) ;
         ON CHANGE( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva4" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt4" ) ) ] ) );
         VALID    ( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva4" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt4" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva5" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva5" ) ) ] ;
         ID       340 ;
         SPINNER ;
         PICTURE  cPouDiv ;
         WHEN     ( aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] ) ;
         ON CHANGE( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva5" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt5" ) ) ] ) );
         VALID    ( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva5" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt5" ) ) ] ) );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nPreIva6" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva6" ) ) ] ;
         ID       350 ;
         SPINNER ;
         PICTURE  cPouDiv ;
         WHEN     ( aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ] ) ;
         On CHANGE( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva6" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt6" ) ) ] ) );
         VALID    ( CalBasAtp( aTmp[ ( dbfTmpAtp )->( fieldPos( "nPreIva6" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "lIvaInc" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "TipoIva" ) ) ], aTmpArt[ ( D():Articulos( nView ) )->( fieldpos( "cCodImp" ) ) ], aGet[ ( dbfTmpAtp )->( fieldPos( "nPrcArt6" ) ) ] ) );
         OF       oDlg
      ...
      */
      ...
      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDto1" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto1" ) ) ];
         ID       400 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto1" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto1" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDto2" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto2" ) ) ];
         ID       410 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto2" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto2" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDto3" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto3" ) ) ];
         ID       420 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto3" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto3" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDto4" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto4" ) ) ];
         ID       430 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto4" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto4" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDto5" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto5" ) ) ];
         ID       440 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto5" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto5" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "nDto6" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto6" ) ) ];
         ID       450 ;
         SPINNER ;
         VALID    ( ( aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto6" ) ) ] >= 0 .AND. aTmp[ ( dbfTmpAtp )->( fieldPos( "nDto6" ) ) ] <= 100 ) ) ;
         PICTURE  "@E 999.99";
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "dFecIni" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "dFecIni" ) ) ];
         ID       160 ;
         SPINNER ;
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAtp )->( fieldPos( "dFecFin" ) ) ] VAR aTmp[ ( dbfTmpAtp )->( fieldPos( "dFecFin" ) ) ];
         ID       170 ;
         SPINNER ;
         OF       oDlg
      ...
      REDEFINE BUTTON ;
         ID       IDOK ;
         OF       oDlg ;
         ACTION   ( EndEdtAtipicas( aGet, aTmp, dbfTmpAtp, oBrw, nMode, oDlg ) )
...
STATIC FUNCTION EdtParKit( aTmp, aGet, dbfTmpParKit, oBrw, bWhen, bValid, nMode, aTmpArt )
   ...
   DEFINE DIALOG oDlg RESOURCE "ARTPARKIT" TITLE LblTitle( nMode ) + "escandallos"

      REDEFINE GET aGet[ ( dbfTmpParKit )->( fieldpos( "cNomKit" ) ) ];
         VAR      aTmp[ ( dbfTmpParKit )->( fieldpos( "cNomKit" ) ) ];
         PICTURE  "@!" ;
         ID       100 ;
         OF       oDlg
      ...
      REDEFINE BUTTON ;
         ID       500 ;
         OF       oDlg;
         ACTION   ( WinAppRec( oBrwEsc, bEdtKit, dbfTmpKit, , , aTmp ), oBrwEsc:Select(0), oBrwEsc:Select(1), oBrwEsc:Refresh() )

      REDEFINE BUTTON ;
         ID       501 ;
         OF       oDlg;
         ACTION   ( WinEdtRec( oBrwEsc, bEdtKit, dbfTmpKit, , , aTmp ), oBrwEsc:Select(0), oBrwEsc:Select(1), oBrwEsc:Refresh() )

      REDEFINE BUTTON ;
         ID       502 ;
         OF       oDlg;
         ACTION   ( dbDelRec( oBrwEsc, dbfTmpKit ), oBrwEsc:Select(0), oBrwEsc:Select(1), oBrwEsc:Refresh() )
      ...
      REDEFINE BUTTON oBtnOk ;
         ID       IDOK ;
         OF       oDlg ;
         ACTION   ( lPreSaveParKit( aGet, aTmp, dbfTmpParKit, oBrw, nMode, oDlg ) )
   ...
   ACTIVATE DIALOG   oDlg ;
      ON INIT        ( oBrwEsc:MakeTotals(), oBrwEsc:Load() );
      CENTER
...
STATIC FUNCTION ChgPrc( oWndBrw )
   ...
   REDEFINE BUTTON ;
      ID       IDOK;
		OF 		oDlg ;
      ACTION   ( mkChgPrc( cFam, cGetTip, cTipIva, lCosto, lTarifa1, lTarifa2, lTarifa3, lTarifa4, lTarifa5, lTarifa6, lPesVol, nRad, nPctInc, nUndInc, lRnd, nDec, lMargenAjuste, cMargenAjuste, oComBox, cArtOrg, cArtDes, oMtr, oDlg, oWndBrw, lPrcMin ))
   ...
   oDlg:AddFastKey( VK_F5, {|| mkChgPrc( cFam, cGetTip, cTipIva, lCosto, lTarifa1, lTarifa2, lTarifa3, lTarifa4, lTarifa5, lTarifa6, lPesVol, nRad, nPctInc, nUndInc, lRnd, nDec, lMargenAjuste, cMargenAjuste, oComBox, cArtOrg, cArtDes, oMtr, oDlg, oWndBrw, lPrcMin ) } )
...
function CalPosTactil( oMeter, lMessage )
   ...
   USE ( cPatEmp() + "ARTICULO.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "ARTICULO", @cArticulo ) )
   SET ADSINDEX TO ( cPatEmp() + "ARTICULO.CDX" ) ADDITIVE
...
function SynArt( cPath )
   ...
   USE ( cPatEmp() + "ARTICULO.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "ARTICULO", @dbfArt ) )
   SET ADSINDEX TO ( cPatEmp() + "ARTICULO.CDX" ) ADDITIVE

   USE ( cPatEmp() + "ARTKIT.Dbf" ) NEW VIA ( cDriver() )      EXCLUSIVE ALIAS ( cCheckArea( "ARTTIK", @dbfArtKit ) )
   SET ADSINDEX TO ( cPatEmp() + "ARTKIT.CDX" ) ADDITIVE

   USE ( cPatEmp() + "PARKIT.Dbf" ) NEW VIA ( cDriver() )      EXCLUSIVE ALIAS ( cCheckArea( "PARTIK", @dbfParKit ) )
   SET ADSINDEX TO ( cPatEmp() + "PARKIT.CDX" ) ADDITIVE

   USE ( cPatEmp() + "PROVART.Dbf" ) NEW VIA ( cDriver() )     EXCLUSIVE ALIAS ( cCheckArea( "PROVART", @dbfArtPrv ) )
   ...
   USE ( cPatEmp() + "ArtCodebar.Dbf" ) NEW VIA ( cDriver() )  EXCLUSIVE ALIAS ( cCheckArea( "CODEBAR", @dbfCodebar ) )
   SET ADSINDEX TO ( cPatEmp() + "ArtCodebar.Cdx" ) ADDITIVE

   USE ( cPatEmp() + "ArtImg.Dbf" ) NEW VIA ( cDriver() )      EXCLUSIVE ALIAS ( cCheckArea( "ArtImg", @dbfImg ) )
   SET ADSINDEX TO ( cPatEmp() + "ArtImg.Cdx" ) ADDITIVE
   ...
   if ::lSuccesfullSend
      ...
      */
      ...
      BEGIN SEQUENCE
         ...
         USE ( cPatEmp() + "Articulo.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "ARTICULO", @dbfArt ) )
         SET ADSINDEX TO ( cPatEmp() + "Articulo.Cdx" ) ADDITIVE
...
Function IsArticulo( cPath )
   ...
   if !lExistIndex( cPath + "Articulo.Cdx"   )  .or. ;
      ...
      !lExistIndex( cPath + "ArtImg.Cdx"     )  .or. ;
      !lExistIndex( cPath + "ArtDoc.Cdx"     )
...
FUNCTION rxArticulo( cPath, cDriver )
   ...
   if !lExistTable( cPath + "Articulo.Dbf"   ) .or. ;
      ...
      !lExistTable( cPath + "ParKit.Dbf"     ) .or. ;
      !lExistTable( cPath + "ArtAsc.Dbf"     ) .or. ;
      !lExistTable( cPath + "ArtCodebar.Dbf" ) .or. ;
      !lExistTable( cPath + "ArtLbl.Dbf"     ) .or. ;
      !lExistTable( cPath + "ArtImg.Dbf"     ) .or. ;
      !lExistTable( cPath + "ArtAlm.Dbf"     ) .or. ;
      !lExistTable( cPath + "ArtDoc.Dbf"     )
   ...
   */
   ...
   /*
   Parent Kit_______________________________________________________________
   */
   ...
   /*
   Asociados___________________________________________________________________
   */
   ...
   */
   ...
   /*
   Documentos de articulos_____________________________________________________
   */
...
Static Function ShowComentario( aTmp, nMode )
   ...
   if nMode != APPD_MODE .and. aTmp[ ( D():Articulos( nView ) )->( fieldpos( "lMosCom" ) ) ] .and. !Empty( aTmp[ ( D():Articulos( nView ) )->( fieldpos( "mComent" ) ) ] )
   ...
   if lKitArt
      ...
      if ( dbfArtKit )->( dbSeek( cCodArt ) )
         ...
         while ( dbfArtKit )->cCodKit == cCodArt .and. !( dbfArtKit )->( eof() )
            ...
            if Empty( cCodCli )                    .or.;
               ( Empty( ArticulosModel():getField( 'cExtFld', 'Codigo', ( dbfArtKit )->cRefKit ) )  .or. ;
               ( Empty( AllTrim( getCustomExtraField( ArticulosModel():getField( 'cExtFld', 'Codigo', ( dbfArtKit )->cRefKit ), "Clientes", cCodCli ) ) ) .or. AllTrim( getCustomExtraField( ArticulosModel():getField( 'cExtFld', 'Codigo', ( dbfArtKit )->cRefKit ), "Clientes", cCodCli ) ) == "si" ) )
...
Function BrwSelArticulo( oGetCodigo, oGetNombre, lCodeBar, lAppend, lEdit, oBtnSaveLine, oGetLote, oGetCodPrp1, oGetCodPrp2, oGetValPrp1, oGetValPrp2, oGetFecCad, cCodAlm )
   ...
   /*
   Filtramos para que no entren los obsoletos----------------------------------
   */
   ...
   */
   ...
   if IsReport()
      ...
      if !uFieldEmpresa( "lNStkAct" )
         if getPvProfString( "Gamma", "ViewGamma", ".F.", cIniAplication() ) == ".T."
            DEFINE DIALOG oDlg RESOURCE "HELPENTRYDUPSTKGAMMA"   TITLE "Seleccionar artículos"
         else 
   ...
   //SetBrwOpt( "BrwArticulo", if( ( D():Articulos( nView ) )->( OrdSetFocus() ) == "Codigo", 1, 2 ) )
...
Static Function ChangeBrwArt( oBrwStock, oBmpImage, oBrw, cCodAlm )
   ...
   oTimerBrw             := TTimer():New( 300, {|| LoadBrwArt( oBrwStock, oBmpImage, cCodAlm ) }, )  //900
...
FUNCTION RetFamArt( cCodArt, uArt )
   ...
   if ( uArt ) == NIL
      USE ( cPatEmp() + "ARTICULO.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "ARTICULO", @uArt ) )
      SET ADSINDEX TO ( cPatEmp() + "ARTICULO.CDX" ) ADDITIVE
      lClose      := .t.
   end if
...
Function PrintReportArticulo( nDevice, nCopies, cPrinter, dbfDoc )
   ...
   DEFAULT nCopies      := 1
   DEFAULT cPrinter     := ImpresoraDefectoUsuario()
...
Static Function EdtImg( aTmp, aGet, dbfTmpImg, oBrw, aArt, bValid, nMode )
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "Imagenes" TITLE LblTitle( nMode ) + "imagenes de artículos"
      ...
      REDEFINE GET   oImgRmt ;
         VAR         aTmp[ ( dbfTmpImg )->( FieldPos( "cRmtArt" ) ) ] ;
         ID          140 ;
         WHEN        ( nMode != ZOOM_MODE ) ;
         OF          fldGeneral

         //oImgRmt:bHelp := {|| ImgArtWeb( aTmp[ ( dbfTmpImg )->( FieldPos( "cImgArt" ) ) ] ) }
      ...
      //oDlg:bStart    := {|| oImgArt:SetFocus(), MsgInfo( AllTrim( aTmp[ ( dbfTmpImg )->( FieldPos( "cRmtArt" ) ) ] ) ) , oImgArt:LoadFromURL( AllTrim( aTmp[ ( dbfTmpImg )->( FieldPos( "cRmtArt" ) ) ] ) ) }
...
Static Function EdtDoc( aTmp, aGet, dbf, oBrw, bWhen, bValid, nMode, aTmpLin )
   ...
   DEFINE DIALOG oDlg RESOURCE "DOCUMENTOS" TITLE LblTitle( nMode ) + "documento de artículo"
...
STATIC FUNCTION EdtAsc( aTmp, aGet, dbfTmpAsc, oBrw, bWhen, bValid, nMode, aTmpArt )
   ...
   DEFINE DIALOG oDlg RESOURCE "ARTASC" TITLE LblTitle( nMode ) + "asociado"

      REDEFINE GET aGet[ ( dbfTmpAsc )->( fieldpos( "cRefAsc" ) ) ] ;
         VAR      aTmp[ ( dbfTmpAsc )->( fieldpos( "cRefAsc" ) ) ];
         PICTURE  "@!" ;
         ID       100 ;
         COLOR    CLR_GET ;
         BITMAP   "LUPA" ;
         VALID    cArticulo( aGet[ ( dbfTmpAsc )->( fieldpos( "cRefAsc" ) ) ], nil, aGet[ ( dbfTmpAsc )->( fieldpos( "cDesAsc" ) ) ] );
         ON HELP  BrwSelArticulo( aGet[ ( dbfTmpAsc )->( fieldpos( "cRefAsc" ) ) ], nil, .f., .f., .f. );
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAsc )->( fieldpos( "cDesAsc" ) ) ];
         VAR      aTmp[ ( dbfTmpAsc )->( fieldpos( "cDesAsc" ) ) ];
         PICTURE  "@!" ;
         WHEN     ( .F. ) ;
         ID       110 ;
         OF       oDlg

      REDEFINE GET aGet[ ( dbfTmpAsc )->( fieldpos( "nUndAsc" ) ) ];
         VAR      aTmp[ ( dbfTmpAsc )->( fieldpos( "nUndAsc" ) ) ];
         ID       120 ;
         SPINNER ;
         MIN      1 ;
         MAX   9999 ;
      ...
      REDEFINE BUTTON ;
         ID       IDOK ;
         OF       oDlg ;
         ACTION   ( lPreSaveAsc( aGet, aTmp, dbfTmpAsc, D():Articulos( nView ), oBrw, nMode, oDlg, aTmpArt ) )
   ...
   oDlg:AddFastKey( VK_F5, {|| lPreSaveAsc( aGet, aTmp, dbfTmpAsc, D():Articulos( nView ), oBrw, nMode, oDlg, aTmpArt ) } )
...
Static function informeEscandallo()
   ...
   DEFINE DIALOG oDlg RESOURCE "ART_INFO_KIT"
      ...
      REDEFINE SAY oSay PROMPT "Informe escandallo: " + AllTrim( ( dbfTmpKit )->cRefKit ) + " - " + AllTrim( ArticulosModel():getField( 'Nombre', 'Codigo', ( dbfTmpKit )->cRefKit ) ) ;
         ID       150 ;
         OF       oDlg
