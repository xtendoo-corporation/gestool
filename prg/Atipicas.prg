...
METHOD DefineFiles( cPath, cDriver, lUniqueName, cFileName ) CLASS TAtipicas
   ...
   DEFINE TABLE oDbf FILE ( cFileName ) CLASS ( cFileName ) ALIAS ( cFileName ) PATH ( cPath ) VIA ( cDriver ) COMMENT "Atipicas de clientes y grupos"
      ...
      FIELD NAME "cDesEsp"    TYPE "C" LEN 150 DEC 0 COMMENT "Descripción especial"                         OF oDbf
      FIELD NAME "dUltPrc"    TYPE "D" LEN  8  DEC 0 COMMENT "Fecha ultimo cambio precio"                   OF oDbf
      FIELD NAME "cTimUPr"    TYPE "C" LEN  6  DEC 0 COMMENT "Hora último cambio precio"                    OF oDbf
      FIELD NAME "nTarifa"    TYPE "N" LEN  1  DEC 0 COMMENT "Tarifa a aplicar en atipica por familia"      OF oDbf
...
METHOD SaveResource() CLASS TAtipicas
...
METHOD SaveDetails() CLASS TAtipicas
...
METHOD DeleteDetails() CLASS TAtipicas

RETURN ( Self )

//--------------------------------------------------------------------------//

METHOD ChangeNaturaleza() CLASS TAtipicas
...
METHOD Expandir( lSet ) CLASS TAtipicas
...
METHOD ButtonAppend( Id, oDialog ) CLASS TAtipicas

   REDEFINE BUTTON  ;
      ID       ( Id ) ;
      OF       ( oDialog ) ;
      WHEN     ( ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) ) ) ;
...
METHOD ButtonEdit( Id, oDialog ) CLASS TAtipicas
...
METHOD ButtonDel( Id, oDialog ) CLASS TAtipicas
...
METHOD Browse( Id, oDialog ) CLASS TAtipicas
...
METHOD LoadAtipica() CLASS TAtipicas
...
METHOD SetPrimeraPropiedad( cCodigoPropiedad, cValorPropiedad ) CLASS TAtipicas
...
METHOD SetSegundaPropiedad( cCodigoPropiedad, cValorPropiedad ) CLASS TAtipicas
...
METHOD CalculaRentabilidad() CLASS TAtipicas    
...
METHOD CalculaBase( oPrecioIva, oPrecioBase ) CLASS TAtipicas
...
METHOD CalculaIva( oPrecioBase, oPrecioIva ) CLASS TAtipicas
...
METHOD PrecioBase( nTarifa ) CLASS TAtipicas
...
METHOD PrecioIva( nTarifa ) CLASS TAtipicas
...
CLASS ArticulosAtipicas
   ...
   METHOD run( cCodigoArticulo )

   METHOD initSay()

   METHOD initArrayAtipicas()
...
METHOD run( cCodigoArticulo ) CLASS ArticulosAtipicas
...
METHOD initSay() CLASS ArticulosAtipicas
...
METHOD initArrayAtipicas() CLASS ArticulosAtipicas
...
METHOD Resource() CLASS ArticulosAtipicas

   DEFINE DIALOG ::oDialog RESOURCE "ARTATIPICAS" TITLE "Atípicas de clientes"
