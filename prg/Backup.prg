...
METHOD SqlSafe()
   ...
   if !File( cPath( AllTrim( ::cMySQLDump ) ) + "mysqldump.exe" )
      msgStop( "Error al crear la copia sql" )
      Return ( Self )
   end if
   ...
   /*
   Hago una copia en Safe------------------------------------------------------
   */

   /*cCommandExec            += cPatConfig() + 'CopiaGestoolSql.bat '
   cCommandExec            += '"' + cPath( AllTrim( ::cMySQLDump ) ) + '" '
   cCommandExec            += '"' + cPath( AllTrim( ::cDir ) ) + 'gestoolsql.sql" '
   cCommandExec            += '"' + ::cIpMySQL + '" '
   cCommandExec            += '"' + ::cUserMySQL + '" '
   cCommandExec            += '"' + ::cPasswordMySQL + '" '
   cCommandExec            += '"' + ::cDatabaseMySQL + '" '

   nHndReport              := winExec( cCommandExec, 1 )
