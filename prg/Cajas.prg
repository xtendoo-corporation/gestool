...
STATIC FUNCTION lOpenFiles()
   ...
   BEGIN SEQUENCE
      ...
      /*if !lExistTable( cPatDat() + "Cajas.Dbf" )    .or. ;
         !lExistTable( cPatDat() + "CajL.Dbf" )     .or. ;
         mkCajas()
      end if

      if !lExistIndex( cPatDat() + "Cajas.Cdx" )    .or. ;
         !lExistIndex( cPatDat() + "CajL.Cdx" )     .or. ;
         rxCajas()
      end if*/
...
FUNCTION Cajas( oMenuItem, oWnd )
   ...
   DEFAULT  oMenuItem   := "cajon_portamonedas"
   DEFAULT  oWnd        := oWnd()
...
STATIC FUNCTION EdtRec( aTmp, aGet, dbfCajT, oBrw, bWhen, bValid, nMode )
   ...
   REDEFINE FOLDER oFld ;
      ...
      */
      ...
      // Caja almacén repacionado----------------------------------------------

      REDEFINE GET   aGet[ ( dbfCajT )->( FieldPos( "cCodAlm" ) ) ];
         VAR         aTmp[ ( dbfCajT )->( FieldPos( "cCodAlm" ) ) ];
         ID          310 ;
         IDTEXT      311 ;
         BITMAP      "Lupa" ;
         ON HELP     ( BrwAlmacen( aGet[ ( dbfCajT )->( FieldPos( "cCodAlm" ) ) ], aGet[ ( dbfCajT )->( FieldPos( "cCodAlm" ) ) ]:oHelpText ) ) ;
         WHEN        ( nMode != ZOOM_MODE ) ;
         VALID       ( cAlmacen( aGet[ ( dbfCajT )->( FieldPos( "cCodAlm" ) ) ], , aGet[ ( dbfCajT )->( FieldPos( "cCodAlm" ) ) ]:oHelpText ) );
         OF          oFld:aDialogs[ 1 ]
      ...
      */
      ...
      REDEFINE BUTTON ;
         ID       500 ;
			OF 		oDlg ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( SavRec( aTmp, aGet, oBrw, oDlg, nMode ) )
   ...
   if nMode != ZOOM_MODE
      oDlg:AddFastKey( VK_F5, {|| SavRec( aTmp, aGet, oBrw, oDlg, nMode ) } )
   end if
...
Static FUNCTION InitBrwBigCaj( oDlg, oImgCaj, oLstCaj, dbfCaj )
   ...
   oImgCaj:AddMasked( TBitmap():Define( "gc_cash_register_50" ),   Rgb( 255, 0, 255 ) )
...
FUNCTION SelectCajas()
   ...
   if oDlg:nResult == IDOK
      ...
      setKey( VK_F12, {|| Application():openDirectCajon() } )

      //Caso de que tenga un almacén seleccionado en la caja caso Canario
...
FUNCTION SynCajas( cPath )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPath + "CajL.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "CajL", @dbfCajL ) )
      SET ADSINDEX TO ( cPath + "CajL.Cdx" ) ADDITIVE
