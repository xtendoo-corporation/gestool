...
CLASS TCamposExtra FROM TMant
   ...
   METHOD Buscar( oGet, cField, oGetField, cType )
   ...
   while !( cDbf )->( Eof() )
      ...
      if !empty( aCampos )
         ...
         if hhaskey( aCampos, cTipoCampo ) .and. hGet( aCampos, cTipoCampo )
            ...
            aAdd( aCamposExtra, {   "código"       => ( cDbf )->cCodigo,;
                                    "descripción"  => ( cDbf )->cNombre,;
                                    "tipo"         => ( cDbf )->nTipo,;
                                    "longitud"     => ( cDbf )->nLongitud,;
                                    "decimales"    => ( cDbf )->nDecimales,;
                                    "lrequerido"   => ( cDbf )->lRequerido,;
                                    "valores"      => hb_deserialize( ( cDbf )->mDefecto ),;
                                    "valor"        => cValor,;
                                    "clave"        => space( 30 ),;
                                    "claveinterna" => "" } )
...
METHOD Syncronize() CLASS TCamposExtra
...
METHOD Buscar( oGet, cType ) CLASS TCamposExtra
...
CLASS CamposExtraModel FROM ADSBaseModel
   ...
   METHOD getExtraFieldForType( cType )

   METHOD getExtraFields()
...
METHOD getExtraFieldForType( cType ) CLASS CamposExtraModel
...
METHOD getExtraFields() CLASS CamposExtraModel
