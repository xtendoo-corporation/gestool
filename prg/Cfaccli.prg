...
FUNCTION CntFacCli( lSimula, lPago, lExcCnt, lMessage, oTree, nAsiento, aSimula, dbfFacCliT, dbfFacCliL, dbfFacCliP, dbfAntCliT, dbfCli, dbfDiv, dbfArt, dbfFPago, dbfIva, oNewImp, oBrw, cCodEmp, cCodPro )
   ...
   */
   ...
   for each uIva in newIva
      ...
      if isNum( hGet( uIva, "porcentajeiva" ) ) //uIva[ 3 ] )
   ...
   if lSimula .or. !lErrorFound
      ...
      */
      ...
      for each uIva in newIva
         ...
         if ( len( uIva ) >= 12 ) //.and. ( hGet( uIva, "impiva" ) != 0 .or. uFieldEmpresa( "lConIva" ) ) /*******Comentado para que salga el asiento con iva 0*******/

            //nBaseImp    := uIva[ 2 ] - uIva[ 10 ] 
            nBaseImp    := hGet( uIva, "neto" ) //no metidos los gastos
            ...
            aAsiento    := MkAsiento(  nAsiento, ;                          
                                       cCodDiv, ;                          
                                       dFecha, ;                          
                                       hGet( uIva, "cSubCtaIva" ),;           // Cuenta de impuestos                          
                                       cCtaCli,;                              // Contrapartida                          
                                       ,;                                     // Ptas. Debe                          
                                       cConcepto,;                          
                                       hGet( uIva, "impiva" ) ,;// - uIva[ 11 ],;               // Ptas. Haber                          
                                       cFactura,;                          
                                       nBaseImp,;                             // Base Imponible                          
                                       hGet( uIva, "porcentajeiva" ),;                          
                                       if( lRecargo, hGet( uIva, "porcentajere" ), 0 ),;         // Tipo de recargo                         
                                       ,;                          
                                       cProyecto,;                          
                                       cClave,;                          
                                       ,;                          
         ...
         */
         ...
         /*VERLO CUANDO TENGAMOS LOS GASTOS*/
      ...
      /*
      Asiento impuesto especial 0.20€/litro
      */
      ...
      if ConfiguracionesEmpresaModel():getLogic( 'lDtoLinAfterTotal', .f. )
         ...
         aadd( aSimula, MkAsiento(  nAsiento,;
                                    cCodDiv,;
                                    dFecha,;
                                    Padr( AllTrim( ConfiguracionesEmpresaModel():getValue( 'cuenta_descuento_especial', "" ) ), 12 ),;
                                    ,;
                                    nTotalDtoLineal,;
                                    "Impuesto especial 0.2€/L",;
                                    ,;
                                    cFactura,;
                                    ,;
                                    ,;
   ...
   */
   ...
   if lPago .and. !lErrorFound .and. ( dbfFacCliP )->( dbSeek( nFactura ) )
      ...
      while ( ( dbfFacCliP )->cSerie + str( ( dbfFacCliP )->nNumFac ) + ( dbfFacCliP )->cSufFac == nFactura ) .and. !( dbfFacCliP )->( eof() )
         ...
         //he metido lsimula antes estaba a .t.
...
Function DlgCntTicket( oBrw, oNewImp, nView )
   ...
   DEFINE DIALOG oDlg RESOURCE "SelectRango" TITLE "Contabilizar tickets"
   ...
   REDEFINE BTNBMP ;
      ...
      RESOURCE "Up16" ;
      NOBORDER ;
      ACTION   ( oDocIni:cText( Val( dbFirst( D():Tikets( nView ), "cNumTik", , cSerIni, "cNumTik" ) ) ) )
   ...
   REDEFINE BTNBMP ;
      ...
      RESOURCE "Down16" ;
      NOBORDER ;
      ACTION   ( oDocFin:cText( Val( dbLast( D():Tikets( nView ), "cNumTik", , cSerIni, "cNumTik" ) ) ) )
   ...
   */
   ...
   REDEFINE APOLOMETER oMtrInf ;
      ...
      PROMPT   "Proceso" ;
      ID       200;
      TOTAL    ( D():Tikets( nView ) )->( Lastrec() ) ;
      OF       oDlg

   REDEFINE BUTTON ;
      ID       IDOK ;
      OF       oDlg ;
      ACTION   ( ContabilizaTicket( nRad, cSerIni + str( nDocIni, 10 ) + cSufIni, cSerFin + str( nDocFin, 10 ) + cSufFin, lFechas, dDesde, dHasta, lChk1, lChk2, oBrw, oMtrInf, oTree, oDlg, oBtnCancel, oNewImp, nView ) )
   ...
   oDlg:AddFastKey( VK_F5, {|| ContabilizaTicket( nRad, cSerIni + str( nDocIni, 10 ) + cSufIni, cSerFin + str( nDocFin, 10 ) + cSufFin, lFechas, dDesde, dHasta, lChk1, lChk2, oBrw, oMtrInf, oTree, oDlg, oBtnCancel, oNewImp, nView ) } )
   ...
   */
   ...
   if nRad == 1
      ...
      for each nPos in ( oBrw:oBrw:aSelected )
         ...
         if lFechas .or.( ( D():Tikets( nView ) )->dFecTik >= dDesde .and. ( D():Tikets( nView ) )->dFecTik <= dHasta ) .and.;
            ( D():Tikets( nView ) )->cTipTik == "1"
      ...
      if ( D():Tikets( nView ) )->( dbSeek( cNumIni, .t. ) )
         ...
         while !Empty( ( D():Tikets( nView ) )->( OrdKeyVal() ) )    .and.;
               ( D():Tikets( nView ) )->( OrdKeyVal() ) >= cNumIni   .and.;
               ( D():Tikets( nView ) )->( OrdKeyVal() ) <= cNumFin   .and.;
               ( D():Tikets( nView ) )->( !eof() )
   ...
   /*
   Escribimos los ficheros de Sage 50------------------------------------------
   */
...
FUNCTION CntFacRec( lSimula, lPago, lExcCnt, lMessage, oTree, nAsiento, aSimula, dbfFacRecT, dbfFacRecL, dbfFacCliP, dbfCli, dbfDiv, dbfArt, dbfFPago, dbfIva, oNewImp, oBrw, cCodEmp, cCodPro )
   ...
   */
   ...
   for each uIva in newIva
      ...
      if isNum( hGet( uIva, "porcentajeiva" ) )  // isNum( uIva[ 3 ] )
   ...
   if lSimula .or. !lErrorFound
      ...
      */
      ...
      for each uIva in newIva
         ...
         if ( len( uIva ) >= 12 )  //( len( uIva ) >= 12 ) .and. ( uIva[ 8 ] != 0 .or. uFieldEmpresa( "lConIva" ) )
