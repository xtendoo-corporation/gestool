...
STATIC FUNCTION EdtBig( aTmp, aGet, dbfCli, oBrw, bWhen, bValid, nMode )
   ...
   DEFINE DIALOG  oDlg ;
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _TITULO ] ) )
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _DOMICILIO ] ) )
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _POBLACION ] ) )
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _CODPOSTAL ] ) )

      REDEFINE GET aGet[ _PROVINCIA ] ;
         VAR      aTmp[ _PROVINCIA ]  ;
         ID       190 ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         OF       oDlg
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _PROVINCIA ] ) )
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _TELEFONO ] ) )
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _CMEIINT ] ) )  
      ...
      REDEFINE BUTTONBMP;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32";
         ACTION   ( VirtualKey( .f., aGet[ _MCOMENT ] ) )  
      ...
      */
      ...
      REDEFINE BUTTON ;
         ...
         OF       oDlg ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( MsgRun(  "Guardando cliente", "Espere por favor...", {|| SavClient( aTmp, aGet, oDlg, oBrw, nMode ) } ) )
      ...
      if nMode != ZOOM_MODE
         oDlg:AddFastKey( VK_F5, {|| MsgRun(  "Guardando cliente", "Espere por favor...", {|| SavClient( aTmp, aGet, oDlg, oBrw, nMode ) } ) } )
      end if
   ...
   DEFINE DIALOG  oDlg ;
      ...
      */
      ...
      REDEFINE GET aGet[ _CCODPAI ] VAR aTmp[ _CCODPAI ] ;
         ...
         COLOR    CLR_GET ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         VALID    ( oPais:GetPais( aTmp[ _CCODPAI ], oSay[ 6 ] ) ) ;
      ...
      */
      ...
      REDEFINE CHECKBOX aGet[ _LULTVTA ] VAR aTmp[ _LULTVTA ] ;
         ID       400 ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         OF       fldImpuestos

      REDEFINE CHECKBOX aGet[ _LFACPDT ] VAR aTmp[ _LFACPDT ] ;
         ID       410 ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         OF       fldImpuestos

      REDEFINE GET aGet[ _NDFACPDT ] VAR aTmp[ _NDFACPDT ];
         ID       420 ;
         SPINNER ;
         MIN      0 ;
         MAX      999 ;
         PICTURE  "999" ;
         WHEN     ( aTmp[ _LFACPDT ] .and. nMode != ZOOM_MODE ) ;
         OF       fldImpuestos
      ...
      */
      ...
      REDEFINE BUTTON  ;
         ID       500 ;
         OF       fldTarifa ;
         WHEN     ( ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) ) .and. nMode != ZOOM_MODE );
      ...
      */
      ...
      if ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) ) .and. nMode != ZOOM_MODE
...
STATIC FUNCTION EdtAtp( aTmp, aGet, dbfTmpAtp, oBrw, aTmpCli, aGetCli, nMode )
   ...
   DEFINE DIALOG oDlg RESOURCE "CLIATP" TITLE LblTitle( nMode ) + "tarifas de clientes"
      ...
      REDEFINE COMBOBOX aGet[ _aNTIPATP ] VAR cNaturaleza ;
         ITEMS    aNaturaleza ;
         ID       90 ;
         ON CHANGE( ChangeNaturaleza( aGet, aTmp, oSayPr1, oSayPr2, oSayVp1, oSayVp2, oGetArticulo, oGetFamilia, oCosto, nMode, oSayLabels, , oBtnEscandallo, oSayTar ) ) ;
      ...
      REDEFINE GET aGet[ _aCCODART ] VAR aTmp[ _aCCODART ];
         ...
         VALID    ( IsCliAtp( aGet, aTmp, oGetArticulo, ( D():Atipicas( nView ) ), nMode, oSayPr1, oSayPr2, oSayVp1, oSayVp2, oCosto, aTmpCli ) ) ;
         OF       oFld:aDialogs[1]
      ...
      oBtnEscandallo    := TBtnBmp():ReDefine( 141, "BMPKIT",,,,,{|| if( aTmp[ _aNTIPATP ] <= 1, infoEscandallo( aTmp[ _aCCODART ], aTmpCli[ _COD ] ), ) }, oFld:aDialogs[1], .f., , .f.,  )
      ...
      REDEFINE GET aGet[ _aCVALPR1 ] VAR aTmp[ _aCVALPR1 ];
         ...
         VALID    ( if( lPrpAct( aGet[ _aCVALPR1 ], oSayVp1, aTmp[ _aCCODPR1 ], dbfProL ),;
                    IsCliAtp( aGet, aTmp, oGetArticulo, ( D():Atipicas( nView ) ), nMode, oSayPr1, oSayPr2, oSayVp1, oSayVp2, oCosto, aTmpCli ),;
                    .f. ) ) ;
         OF       oFld:aDialogs[1]
      ...
      REDEFINE CHECKBOX aGet[ _aLPRCCOM ] VAR aTmp[ _aLPRCCOM ] ;
         ID       122 ;
         ON CHANGE( lChangeCostoParticular( aGet, aTmp, oCosto, nMode, aTmpCli ) );
         WHEN     ( aTmp[ _aNTIPATP ] <= 1 .and. nMode != ZOOM_MODE ) ;
         OF       oFld:aDialogs[ 1 ]

      aGet[ _aNTARIFA ]  := comboTarifa():Build( { "idCombo" => 610, "uValue" => aTmp[ _aNTARIFA ] } )
      aGet[ _aNTARIFA ]:Resource( oFld:aDialogs[ 1 ] )

      REDEFINE SAY oSayTar;
         ID       611 ;
         OF       oFld:aDialogs[1]
      ...
      */
      ...
      REDEFINE GET aGet[ _aCDESESP ] VAR aTmp[ _aCDESESP ];   
         ID       260 ;
         WHEN     ( nMode != ZOOM_MODE );
         OF       oFld:aDialogs[2]

      REDEFINE GET aGet[ _aDULTPRC ] VAR aTmp[ _aDULTPRC ];   
         ID       261 ;
         WHEN     ( .f. );
         OF       oFld:aDialogs[2]

      REDEFINE GET aGet[ _aCTIMUPR ] VAR aTmp[ _aCTIMUPR ];   
         ID       262 ;
         WHEN     ( .f. );
         PICTURE  "@R 99:99:99" ;
         OF       oFld:aDialogs[2]
...
static function infoEscandallo( cCodArt, cCodCli )
   ...
   DEFINE DIALOG oDlg RESOURCE "ART_INFO_KIT"
      ...
      REDEFINE SAY oSay PROMPT "Informe escandallo: " + AllTrim( cCodArt ) + " - " + AllTrim( ArticulosModel():getField( 'Nombre', 'Codigo', cCodArt ) ) ;
         ID       150 ;
         OF       oDlg
      ...
      oBrw:bClrStd                  := {|| { if( hGet( aEscandallos[ oBrw:nArrayAt ], "lGray" ), CLR_GRAY, CLR_BLACK ), GetSysColor( COLOR_WINDOW ) } }
...
Static Function LoadInfoEscandallo( cCodArt, cCodCli )
   ...
   for each a in hKit
      ...
      lGray                := !( Empty( ArticulosModel():getField( 'cExtFld', 'Codigo', hGet( a, "CREFKIT" ) ) )  .or. ;
                              ( Empty( AllTrim( getCustomExtraField( ArticulosModel():getField( 'cExtFld', 'Codigo', hGet( a, "CREFKIT" ) ), "Clientes", cCodCli ) ) ) .or. AllTrim( getCustomExtraField( ArticulosModel():getField( 'cExtFld', 'Codigo', hGet( a, "CREFKIT" ) ), "Clientes", cCodCli ) ) == "si" ) )
...
STATIC FUNCTION lChangeCostoParticular( aGet, aTmp, oCosto, nMode, aTmpCli )
   ...
   if aTmp[ _aLPRCCOM ]
      ...
      aGet[ _aNPRCCOM ]:Hide()
      if nMode != APPD_MODE
         oCosto:cText( nCosto( nil, D():Articulos( nView ), dbfArtKit,,,, aTmpCli[ _COD ] ) )
      end if
   end if
   ...
   DEFINE DIALOG oDlg RESOURCE "Banco" TITLE LblTitle( nMode ) + "banco de cliente"
      ...
      REDEFINE GET aGet[ ( dbfTmpBnc )->( FieldPos( "cEntBnc" ) ) ] ;
         ...
         VALID    (  lCalcDC( aTmp[ ( dbfTmpBnc )->( FieldPos( "cEntBnc" ) ) ],;
                              ...
                              aTmp[ ( dbfTmpBnc )->( Fieldpos( "cPaisIBAN" ) ) ] ),;
   ...
   if nMode == APPD_MODE
      ...
      if dbSeekInOrd( cCodArt, "Codigo", D():Articulos( nView ) )
         ...
         */
         ...
         if nPreCom == 0
            nPreCom        := nCosto( nil, D():Articulos( nView ), dbfArtKit, , , , aTmpCli[ _COD ] )
         end if
...
Static Function ChangeNaturaleza( aGet, aTmp, oSayPr1, oSayPr2, oSayVp1, oSayVp2, oGetArticulo, oGetFamilia, oCosto, nMode, oSayLabels, lInit, oBtnEscandallo, oSayTar )
   ...
   if aGet[ _aNTIPATP ]:nAt == 1
      ...
      if !Empty( aGet[ _aNTARIFA ] )
         aGet[ _aNTARIFA ]:oControl:Hide()
      end if
...
static function SaveEdtAtp( aGet, aTmp, dbfTmpAtp, oBrw, oDlg, nMode )
   ...
   */
   ...
   if !Empty( aGet[ _aNTARIFA ] )
      aTmp[ _aNTARIFA ]  := aGet[ _aNTARIFA ]:getTarifa()
   end if

   aTmp[ _aCNOMART ]    := RetArticulo( aTmp[ _aCCODART ] )
   aTmp[ _aCNOMFAM ]    := RetFamilia( aTmp[ _aCCODFAM ] )
...
Static function getSelectedAtipica( hValue, hSelectedAtipica )
   ...
   /*MsgInfo( hb_valToExp( hCurrentAtipica ), "hCurrentAtipica" )
   MsgInfo( hb_valToExp( hSelectedAtipica ), "hCurrentAtipica" )   
