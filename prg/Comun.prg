...
#define HB_CURLUSESSL_NONE          0
#define HB_CURLUSESSL_TRY           1
#define HB_CURLUSESSL_CONTROL       2
#define HB_CURLUSESSL_ALL           3
#endif
...
FUNCTION lStartCheck()
   ...
   // Abrimos el Timmer para la pda del Rocio
...
FUNCTION Test()
   ...
   LOCAL hCurl, cURL, cXML, cResponse := "", nStatus := 0
   LOCAL aHeaders, nErr

   /* Endpoint PRE VeriFactu SOAP (ajústalo si usas otro) */
   cURL := "https://prewww1.aeat.es/wlpl/TIKE-CONT/ws/SistemaFacturacion/VerifactuSOAP"

   /* Construimos el sobre SOAP */
   cXML := cXmlPruebas()

   hCurl := curl_easy_init()

   IF hCurl == NIL
      ? "Error al inicializar libcurl"
      RETURN NIL
   ENDIF
   ...
   /* TLS / Certificado de cliente (PFX/P12) */
   curl_easy_setopt(hCurl, HB_CURLOPT_USE_SSL, .T.)
   curl_easy_setopt(hCurl, HB_CURLOPT_SSLCERT, "C:/cer/cert.pem" )
   curl_easy_setopt(hCurl, HB_CURLOPT_SSLCERTTYPE, "PEM" )
   //curl_easy_setopt(hCurl, HB_CURLOPT_KEYPASSWD, "Momax0808" )

   curl_easy_setopt(hCurl, HB_CURLOPT_SSLKEY, "C:/cer/cer_key_encrypted.pem")
   curl_easy_setopt(hCurl, HB_CURLOPT_SSLKEYTYPE, "PEM")

   curl_easy_setopt(hCurl, HB_CURLOPT_HTTP_VERSION, 2)   // CURL_HTTP_VERSION_1_1
   curl_easy_setopt(hCurl, HB_CURLOPT_SSLVERSION, 6)     // CURL_SSLVERSION_TLSv1_2

   curl_easy_setopt(hCurl, HB_CURLOPT_POSTFIELDSIZE, Len(cXML))

   /* En PRE puedes desactivar verificación.
      En PRODUCCIÓN: pon VERIFYPEER=.T. y VERIFYHOST=2 y CAINFO correcto. */
   ...
   /* (Opcional) Traza para depurar */
   curl_easy_setopt(hCurl, HB_CURLOPT_VERBOSE, .T.)
   curl_easy_setopt(hCurl, HB_CURLOPT_STDERR, "c:\cer\curl_log.txt")

   /* Ejecutar petición: muchas builds de Harbour devuelven el body como retorno */
   cResponse := curl_easy_perform(hCurl)

   /* Si tu build no devuelve el body en perform, usa WRITEFUNCTION/WRITEDATA.
      Este ejemplo asume retorno directo como string. */

   /* Código HTTP */
   curl_easy_getinfo(hCurl, HB_CURLINFO_RESPONSE_CODE, @nStatus)
...
ENDIF
   ...
   /* Si hubo error de libcurl */
   /*nErr := curl_easy_errno(hCurl)
   IF nErr != 0
      ? "Error libcurl:", curl_easy_strerror(nErr)
   ENDIF
   ...
   /*LOCAL hCurl, cURL, cXML, cResponse, nStatus

   // URL del endpoint (puede ser AEAT o Verifacti)
   cURL := "https://prewww1.aeat.es/wlpl/TIKE-CONT/ws/SistemaFacturacion/VerifactuSOAP"

   // Cargar el contenido XML desde archivo
   cXML := cXmlPruebas()
   ...
   IF hCurl != NIL
      ...
      curl_easy_setopt(hCurl, HB_CURLOPT_HTTPHEADER, { ;
         "Content-Type: application/xml", ;º
         "Accept: application/xml" ;
      })
      curl_easy_setopt(hCurl, HB_CURLOPT_USE_SSL, .t. )
      curl_easy_setopt(hCurl, HB_CURLOPT_SSLCERT, "C:/cer.pfx" )
      curl_easy_setopt(hCurl, HB_CURLOPT_SSLKEY, "Momax0808" )
      curl_easy_setopt(hCurl, HB_CURLOPT_SSLCERTTYPE, "PFX")
      curl_easy_setopt(hCurl, HB_CURLOPT_SSL_VERIFYPEER, .F.) // Desactivar verificación del certificado SSL (no recomendado para producción)
      curl_easy_setopt(hCurl, HB_CURLOPT_SSL_VERIFYHOST, 0) // Desactivar verificación del host (no recomendado para producción)
   ...
   oWebService:setUrl( "https://prewww1.aeat.es/wlpl/TIKE-CONT/ws/SistemaFacturacion/VerifactuSOAP" )
...
function cXmlPruebas()

   LOCAL c := ''

   c += '<?xml version="1.0" encoding="UTF-8"?>' + hb_eol()
   c += '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" ' + ;
        'xmlns:ver="https://www2.agenciatributaria.gob.es/static_files/common/internet/dep/aplicaciones/es/verifactu/ws/SistemaFacturacion/VerifactuV1.xsd">' + hb_eol()
   c += '  <soapenv:Header/>' + hb_eol()
   c += '  <soapenv:Body>' + hb_eol()
   c += '    <ver:EnvioFactura>' + hb_eol()
   c += '      <ver:Cabecera>' + hb_eol()
   c += '        <ver:Version>1.0</ver:Version>' + hb_eol()
   c += '        <ver:TipoComunicacion>ALTA</ver:TipoComunicacion>' + hb_eol()
   c += '        <ver:Software>' + hb_eol()
   c += '          <ver:Nombre>Xtendoo</ver:Nombre>' + hb_eol()
   c += '          <ver:Version>1.0.0</ver:Version>' + hb_eol()
   c += '          <ver:IdProveedor>ESB16890287</ver:IdProveedor>' + hb_eol()
   c += '        </ver:Software>' + hb_eol()
   c += '      </ver:Cabecera>' + hb_eol()
   c += '      <ver:Registro>' + hb_eol()
   c += '        <ver:Factura>' + hb_eol()
   c += '          <ver:Emisor>' + hb_eol()
   c += '            <ver:NIF>ES75558136P</ver:NIF>' + hb_eol()
   c += '            <ver:NombreRazon>Darío Cruz Mauro</ver:NombreRazon>' + hb_eol()
   c += '          </ver:Emisor>' + hb_eol()
   c += '          <ver:DatosFactura>' + hb_eol()
   c += '            <ver:FechaExpedicion>2025-08-27</ver:FechaExpedicion>' + hb_eol()
   c += '            <ver:HoraExpedicion>10:30:00</ver:HoraExpedicion>' + hb_eol()
   c += '            <ver:TipoFactura>F1</ver:TipoFactura>' + hb_eol()
   c += '            <ver:NumSerieFacturaEmisor>DEM-0001</ver:NumSerieFacturaEmisor>' + hb_eol()
   c += '            <ver:DescripcionOperacion>Prueba VeriFactu Harbour</ver:DescripcionOperacion>' + hb_eol()
   c += '          </ver:DatosFactura>' + hb_eol()
   c += '          <ver:Importes>' + hb_eol()
   c += '            <ver:BaseImponible>100.00</ver:BaseImponible>' + hb_eol()
   c += '            <ver:TipoImpositivo>21.00</ver:TipoImpositivo>' + hb_eol()
   c += '            <ver:CuotaRepercutida>21.00</ver:CuotaRepercutida>' + hb_eol()
   c += '            <ver:ImporteTotal>121.00</ver:ImporteTotal>' + hb_eol()
   c += '          </ver:Importes>' + hb_eol()
   c += '          <ver:Cliente>' + hb_eol()
   c += '            <ver:TipoPersona>J</ver:TipoPersona>' + hb_eol()
   c += '            <ver:NIF>ES44244263Z</ver:NIF>' + hb_eol()
   c += '            <ver:NombreRazon>Lorena Espina Díaz</ver:NombreRazon>' + hb_eol()
   c += '          </ver:Cliente>' + hb_eol()
   c += '          <!-- Incluye aquí huella/hash y firma si procede según tu flujo -->' + hb_eol()
   c += '        </ver:Factura>' + hb_eol()
   c += '      </ver:Registro>' + hb_eol()
   c += '    </ver:EnvioFactura>' + hb_eol()
   c += '  </soapenv:Body>' + hb_eol()
   c += '</soapenv:Envelope>' + hb_eol()
...
function checkOrtography( cText )
   ...
   finalText      := StrTran(finalText,Chr(13),Chr(13)+Chr(10))  //No recuerdo por qué puse esta linea

   If Empty( finalText ) // si cancelamos en el chequeo de ortografia, nos devuelve una cadena vacia
      finalText   := cText
   Endif
...
FUNCTION SFtpUpload()
   ...
   //produkcyjne srodowisko TEXTHIDDEN(1) zamaskowana zmiennavvvvvv
   ...
   //cUploadFolder := "var" + Chr( 47 ) + "www" + Chr( 47 ) + "prestashop" + Chr( 47 ) + "img" + Chr( 47 )

   cUploadFolder := UrlAllEncode( "var/www/prestashop/img/" )

   cSftpURL := "sftp://" + UrlAllEncode( UserLogin ) + ":" + ;
               UrlAllEncode( UserPass ) + "@" + cUrl + Chr( 47 ) + cUploadFolder + cFileNoPath( cFile )
   ...
   curl_easy_setopt( curlHandle, HB_CURLOPT_APPEND, 1 )
   ...
   curl_easy_setopt( curlHandle, HB_CURLOPT_VERBOSE, 1 )
   curl_easy_setopt( curlHandle, HB_CURLOPT_FAILONERROR, 1 )
   curl_easy_setopt( curlHandle, HB_CURLOPT_ERRORBUFFER, cErrorB )
   curl_easy_setopt( curlHandle, HB_CURLOPT_FTP_CREATE_MISSING_DIRS, 1 )
...
FUNCTION ControlAplicacion()
   ...
   DEFINE DIALOG oDlg RESOURCE "GetSerialNo" TITLE "Sistema de protección " + __GSTROTOR__ + __GSTVERSION__
   ...
   */
   ...
   REDEFINE RADIO oLicencia ;
      VAR         nLicencia ;
      ID          100, 200 ;
      ON CHANGE   ( ChangeLicenciaMode( nLicencia, oSerialHd, oSerialUsr, oSayPerpetua, oSayDemo ) ) ;
      OF          oDlg
   ...
   */
   ...
   REDEFINE BUTTON ;
      ID          IDOK ; 
      OF          oDlg ;
      ACTION      ( ExitDialog( oDlg, nLicencia, nSerialHD, nSerialUSR, oSerialUsr ) )
...
FUNCTION About()
   ...
   ACTIVATE DIALOG oDlg ;
      ON INIT     (  oTree:SetImageList( oImgLst ),;
                     ...
                     oTree:Add( "Dario Cruz Mauro",                     1 ),;
                     oTree:Add( "C. Real, 58",                          2 ),;
                     oTree:Add( "21700 - La Palma del Condado - Huelva",2 ),;
                     oTree:Add( "655.94.50.25",                         3 ),;
                     oTree:Add( "655.94.50.27",                         3 ),;
                     oTree:Add( "mcalero@gestool.es",                   4 ),;
                     oTree:Add( "dario@gestool.es",                     4 ) ) ;
      CENTER
...
FUNCTION CreateAcceso( oWnd )
   ...
   oItem:bAction        := {|| UsuariosController():New():activateUserNavigatorView() }
   oItem:cId            := "usuarios"
   ...
   oItem:bAction        := {|| RolesController():New():activateUserNavigatorView() }
   ...
   oItem:bAction        := {|| PermisosController():New():activateUserNavigatorView() }
   ...
   if IsNil( cNameVersion )
      ...
      do case
         case File( FullCurDir() + "scmmrc" )
            ...
            cNameVersion      := "Profesional" //"PrestaShop 1.7"
...
FUNCTION aEmpresa( cCodigoEmpresa, lAdministrador )
   ...
   DEFAULT lAdministrador  := .f.

   setArrayEmpresa( EmpresasModel():scatter( cCodigoEmpresa ) )
   
   //setArrayEmpresa( arrayEmpresa( cCodigoEmpresa, nView ) )
...
FUNCTION ImgArtWeb( cDir )
   ...
   DEFINE DIALOG oDlg RESOURCE "IMGARTWEB"
...
FUNCTION lCheckPerpetuoMode( nSerialUSR )
   ...
   local cFileIni       
   local oSerialHD
   local nSerialHD      := Val( AllTrim( Str( Abs( nSerialHD() ) ) + getExeDate() ) ) // Abs( nSerialHD() )
   local aSerialCRC     := {}
   local nSerialCRC     := 0
