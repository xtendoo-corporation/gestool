...
STATIC FUNCTION EdtRec( aTmp, aGet, dbfImpTik, oBrw, bWhen, bValid, nMode )
   ...
   DEFINE DIALOG oDlg RESOURCE "balanza_sql" TITLE LblTitle( nMode ) + " balanzas"

      // Grupo general---------------------------------------------------------

      REDEFINE BITMAP oBmpGeneral ;
         ID       900 ;
         RESOURCE "gc_balance_48" ;
         TRANSPARENT ;
         OF       oDlg
      ...
      REDEFINE BUTTON ;
         ID       210;
         OF       oDlg ;
         ACTION   ( TestBalanza( cBitsSec, cBitsDatos, cBitsParada, cBitsParidad, cPort ) )
...
FUNCTION mkImpTik( cPath, lAppend, cPathOld, oMeter )
   ...
   DEFINE DATABASE oImpTik FILE "IMPTIK.DBF" PATH ( cPath ) ALIAS "IMPTIK" VIA ( cDriver() ) COMMENT "Impresoras de tickets"
      ...
      FIELD NAME "NBUFER"        TYPE "N" LEN  3  DEC 0 COMMENT "Bufer de lectura"                       OF oImpTik
...
CLASS TCommPort
   ...
   METHOD nGramos()
   METHOD cPeso()
   METHOD getPeso( nPrecio )
   METHOD checksumXor( cString )
   METHOD getFormatPrecio( nPrecio )
   METHOD getFormatPeso( cString )
   ...
   METHOD CloseConexion()           INLINE ( CLOSECOMM( ::nHandleConexion ) )
...
METHOD New( cPort, nBitsSec, nBitsParada, nBitsDatos, cBitsParidad, cEntubamiento, nBufer ) CLASS TCommPort
...
METHOD Write( nPrecio ) CLASS TCommPort
