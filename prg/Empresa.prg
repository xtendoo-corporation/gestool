...
STATIC FUNCTION EditConfig( aTmp, aGet, dbfEmp, oBrw, nSelFolder, bValid, nMode )
   ...
   */
   ...
   DEFINE DIALOG  oDlg ;
      ...
      REDEFINE GET nDecimalesRecargo ;
         ID       141;
         PICTURE  "9" ;
         SPINNER ;
         MIN      2 ;
      ...
      REDEFINE CHECKBOX aGet[ _LDTOCLILIN ] ;
         VAR      aTmp[ _LDTOCLILIN ] ;
         ID       154 ;
         OF       fldGeneral

      REDEFINE CHECKBOX aGet[ _LADDATP ] ;
         VAR      aTmp[ _LADDATP ] ;
         ID       155       ;
         OF       fldGeneral
      ...
      REDEFINE CHECKBOX aGet[ _LMSGPROMO ] VAR aTmp[ _LMSGPROMO ] ;
         ID       194 ;
         OF       fldTPV
      ...
      REDEFINE CHECKBOX aGet[ _LCBARTPV ] VAR aTmp[ _LCBARTPV ] ;
         ID       240 ;
         OF       fldTPV
      ...
      REDEFINE CHECKBOX lBrowseSql;
         ID       650;
         OF       fldValores

      REDEFINE CHECKBOX lBusConte;
         ID       660;
         OF       fldValores

      REDEFINE CHECKBOX lDtoLinealDespuesTotal;
         ID       654;
         OF       fldValores

      REDEFINE CHECKBOX lPasPrepare;
         ID       651;
         OF       fldValores

      REDEFINE RADIO aGet[ _NCOSVTA ] VAR aTmp[ _NCOSVTA ] ;
         ID       310, 320, 330 ;
         OF       fldValores
      ...
      REDEFINE CHECKBOX aGet[ _LLOAULTLOT ] VAR aTmp[ _LLOAULTLOT ] ;
         ID       550 ;
         OF       fldArticulos   
      ...
      REDEFINE GET aGet[ _CDIRSAFE ] VAR aTmp[ _CDIRSAFE ] ;
            ID       200 ;
            BITMAP   "Folder" ;
            ON HELP  ( aGet[ _CDIRSAFE ]:cText( Padr( cGetDir32( "Seleccione directorio", Rtrim( aTmp[ _CDIRSAFE ] ), .t. ), 200 ) ) ) ;
            OF       fldContadores 
      ...
      REDEFINE GET nLenSubcta ;
         ID       120;
         PICTURE  "99" ;
         SPINNER ;
         MIN      1 ;
         MAX      12 ;
         WHEN     ( oCmbContabilidad:nAt == 5 );
         OF       fldContabilidad

      REDEFINE GET cCtaDescuentoEspecial ;
         ID       121 ;
         OF       fldContabilidad
      ...
      REDEFINE GET cDireccionFtpImagenes ;
         ID       410 ;
         OF       fldComunicaciones

      REDEFINE GET cUsuarioFtpImagenes ;
         ID       420 ;
         OF       fldComunicaciones

      REDEFINE GET cClaveFtpImagenes ;
         ID       430 ;
         OF       fldComunicaciones
...
FUNCTION SetEmpresa( cCodEmp, oBrw, lAdministrator )
   ...
   */
   ...
   /*cPatEmp( EmpresasModel():getCodigoGrupoCliente( cCodEmp ), .t. )

   cPatEmp( EmpresasModel():getCodigoGrupoProveedor( cCodEmp ), .t. )

   cPatEmp( EmpresasModel():getCodigoGrupoArticulo( cCodEmp ), .t. )

   cPatEmp( EmpresasModel():getCodigoGrupoAlmacen( cCodEmp ), .t. )*/
   ...
   if lChDir( cNamePath( cPath ) ) .or. makeDir( cNamePath( cPath ) ) != -1
      ...
      */
      ...
      if aImportacion:lStockIni .and. cPathOld != nil
         ...
         //StocksModel():StockInit( cPath, cPathOld, oMsg, cCodEmpOld, cCodEmpNew )
      end if
...
FUNCTION SynEmpresa( cPath )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPath + "Empresa.Dbf" ) NEW VIA ( cDriver() ) ALIAS ( cCheckArea( "Empresa", @dbfEmpresa ) )
      SET ADSINDEX TO ( cPath + "Empresa.Cdx" ) ADDITIVE

      USE ( cPath + "Delega.Dbf" ) NEW VIA ( cDriver() ) EXCLUSIVE ALIAS ( cCheckArea( "Delega", @dbfDelega ) )
      SET ADSINDEX TO ( cPath + "Delega.Cdx" ) ADDITIVE

      USE ( cPath + "ConfEmp.Dbf" ) NEW VIA ( cDriver() ) EXCLUSIVE ALIAS ( cCheckArea( "ConfEmp", @dbfConfig ) )
      SET ADSINDEX TO ( cPath + "ConfEmp.Cdx" ) ADDITIVE
