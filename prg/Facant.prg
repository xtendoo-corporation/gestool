...
#define _MENUITEM_           "facturas_de_anticipos"
...
FUNCTION FacAntCli( oMenuItem, oWnd, cCodCli )
   ...
   */
   ...
   DEFINE SHELL oWndBrw FROM 0, 0 TO 22, 80 ;
	  ...
	  oWndBrw:bChgIndex    := {|| if( RolesModel():getRolFiltrarVentas( Auth():rolUuid() ), CreateFastFilter( cFiltroUsuario, dbfAntCliT, .f., , cFiltroUsuario ), CreateFastFilter( "", dbfAntCliT, .f. ) ) }
...
STATIC FUNCTION EdtRec( aTmp, aGet, dbfAntCliT, oBrw, cCodCli, bValid, nMode, cSerAnt )
   ...
   */
   ...
   cSay[ 8 ]               := UsuariosModel():getNombreWhereCodigo( aTmp[ _CCODUSR ] )
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "AntCli" TITLE LblTitle( nMode ) + "anticipos a clientes"
      ...
      */
      ...
      REDEFINE GET aGet[ _CCODUSR ] VAR aTmp[ _CCODUSR ];
         ID       115 ;
         WHEN     ( .f. ) ;
         VALID    ( oSay[ 8 ]:cText( UsuariosModel():getNombreWhereCodigo( aTmp[ _CCODUSR ] ) ), .t.  ) ;
         OF       oFld:aDialogs[1]
   ...
   oDlg:bStart := {|| StartEdtRec( aGet, nMode, oDlg ) }
...
Static Function StartEdtRec( aGet, nMode, oDlg )
...
Function PrintReportAntCli( nDevice, nCopies, cPrinter, dbfDoc )
   ...
   local oFr
   local cFilePdf       := cPatOut() + "FacturasAnticiposCliente" + StrTran( ( dbfAntCliT )->cSerAnt + Str( ( dbfAntCliT )->nNumAnt ) + ( dbfAntCliT )->cSufAnt, " ", "" ) + ".Pdf"
