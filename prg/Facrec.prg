...
#define _MENUITEM_           "rectificativas_de_clientes"
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "FacCli" TITLE LblTitle( nMode ) + "facturas rectificativas"
      ...
      */
      ...
      oGetTarifa:setWhen( {|| nMode != ZOOM_MODE .and. ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) ) } )
        ...
        */
        ...
        if !empty( hAtipica )
            ...
            if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
                aGet[ _NPREUNIT ]:cText( nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpFac[ _CDIVFAC ], aTmpFac[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, dbfIva, , , oNewImp ) )
            end if
...
Static Function lValidLote( aTmp, aGet, oStkAct )
   ...
   if !empty( aGet[ _DFECCAD ] )
      aGet[ _DFECCAD ]:cText( StocksModel():getFechaCaducidad( aTmp[ _CREF ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] ) )
   end if
...
Return ( printReportFacRec( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
