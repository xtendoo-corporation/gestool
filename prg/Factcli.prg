...
#define fldVeryfactu            oFld:aDialogs[8]
...
//DELETE   ( WinDelRec( oWndBrw:oBrw, D():FacturasClientes( nView ), {|| QuiFacCli() } ) );
      ...
      with object ( oWndBrw:AddXCol() )
         :cHeader          := "Número"
         :cSortOrder       := "nNumFac"
         :bEditValue       := {|| if( ( D():FacturasClientes( nView ) )->nNumFac == 0, "", D():FacturasClientesIdTextShort( nView ) ) }
         :nWidth           := 80
   ...
   if RolesModel():getRolCambiarEstado( Auth():rolUuid() )
      ...
      DEFINE BTNSHELL RESOURCE "gc_document_text_earth_" OF oWndBrw ;
         NOBORDER ;
         ACTION   ( aGetSelRec( oWndBrw, {|lChk1, lChk2, oTree| CreateFileFacturae( oTree, lChk1, lChk2 ) }, "Exportar facturas electrónicas a Facturae v 3.2", .t., "Firmar digitalmente (necesita runtime de Java)", .t., "Enviar por correo electrónico" ) ) ;
...
STATIC FUNCTION EdtRec( aTmp, aGet, dbf, oBrw, hHash, bValid, nMode )
   ...
   local hBmpGeneral       := {  { "Resource" => "gc_document_text_user2_48",    "Dialog" => 1 },;
                                 ...
                                 { "Resource" => "gc_veryfactu_48",                 "Dialog" => 8 } }
     ...
     */
     ...
     REDEFINE GET aGet[ _NPCTRET ] VAR aTmp[ _NPCTRET ] ;
      ...
      REDEFINE GET aGet[ _CSERIE ] VAR aTmp[ _CSERIE ] ;
         ...
         WHEN     ( nMode == APPD_MODE .OR. nMode == DUPL_MODE .OR. !aTmp[ _LVALIDA ] );
   ...
   if nTotalUnidades != 0
      ...
      do case
         case ( nStockActual - nTotalUnidades ) < 0
            ...
            if lNotVta
               if lMsg
                  msgStop( "No hay stock suficiente, tenemos " + Alltrim( Trans( nStockActual, MasUnd() ) ) + " unidad(es) disponible(s)," + CRLF + "en almacén " + AllTrim( cCodigoAlmacen ) + " de este lote o propiedades." + CRLF + " Hay " + AllTrim( Trans( nStockAct, MasUnd() ) ) + " unidades totales seleccionando otras opciones."  )
               end if
               return .f.
            end if
            ...
            if lMsgVta
               if lMsg
                  Return ApoloMsgNoYes( "No hay stock suficiente, tenemos " + Alltrim( Trans( nStockActual, MasUnd() ) ) + " unidad(es) disponible(s)," + CRLF + " en almacén " + AllTrim( cCodigoAlmacen ) + " de este lote o propiedades." + CRLF + " Hay " + AllTrim( Trans( nStockAct, MasUnd() ) ) + " unidades totales seleccionando otras opciones." , "¿Desea continuar?" )
               else
                  Return .f.
               end if
            end if
...
Function CreateFacCliQrCode()
	...
	cText 		:= "Número: " + ( D():FacturasClientes( nView ) )->cSerie + AllTrim( Str( ( D():FacturasClientes( nView ) )->nNumFac ) ) + CRLF
