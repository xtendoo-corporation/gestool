...
#define _MENUITEM_               "facturas_de_proveedores"
...
STATIC FUNCTION EdtDet( aTmp, aGet, dbf, oBrw, aTmpFac, cCodArtEnt, nMode )
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "LALBPRV" TITLE LblTitle( nMode ) + "líneas a facturas de proveedores"
      ...
      aGet[ _CREF ]:bValid       := {|| LoaArt( cCodArt, aGet, aTmp, aTmpFac, oFld, oSayPr1, oSayPr2, oSayVp1, oSayVp2, oBmp, oBrwPrp, oDlg, oSayLote, oBeneficioSobre, oTotal, nMode ) }
            ...
            */
            ...
            if ( D():Articulos( nView ) )->lLote
               ...
               */
               ...
               if empty( dFechaCaducidad )
                  dFechaCaducidad   := StocksModel():getFechaCaducidad( cCodArt, aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] )
               end if 
...
FUNCTION nTotFacPrv( cFactura, cFacPrvT, cFacPrvL, cIva, cDiv, cFacPrvP, aTmp, cDivRet, lPic )
   ...
   if aTmp != nil
      ...
      nRouDiv        := nRouDiv( aTmp[ _CDIVFAC ], cDiv )
   ...
   */
   ...
   if lFacGas
      ...
      aAdd( aTotIva, {  "porcentajeiva" => NPCTIVA1,;
                        "logrecargo"   => lRecargo,;
                        "porcentajere" => NPCTREQ1,;
                        "bruto"        => NBRTIVA1,;
                        "neto"         => NBASIVA1,;
                        "impiva"       => NIMPIVA1,;
                        "impre"        => NIMPREQ1,;
      ...
      aAdd( aTotIva, {  "porcentajeiva" => NPCTIVA2,;
                        "logrecargo"   => lRecargo,;
                        "porcentajere" => NPCTREQ2,;
                        "bruto"        => NBRTIVA2,;
                        "neto"         => NBASIVA2,;
                        "impiva"       => NIMPIVA2,;
                        "impre"        => NIMPREQ2,;
      ...
      aAdd( aTotIva, {  "porcentajeiva" => NPCTIVA3,;
                        "logrecargo"   => lRecargo,;
                        "porcentajere" => NPCTREQ3,;
                        "bruto"        => NBRTIVA3,;
                        "neto"         => NBASIVA3,;
                        "impiva"       => NIMPIVA3,;
                        "impre"        => NIMPREQ3,;
...
Function SynFacPrv( cPath )
   ...
   USE ( cPatEmp() + "FacPrvS.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "FacPrvS", @dbfFacPrvS ) )
   SET ADSINDEX TO ( cPatEmp() + "FacPrvS.CDX" ) ADDITIVE

   USE ( cPatEmp() + "FacPRVI.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "FacPRVI", @dbfFacPrvI ) )
   ...
   USE ( cPatEmp() + "ARTDIV.Dbf" ) NEW VIA ( cDriver() )    EXCLUSIVE ALIAS ( cCheckArea( "ARTDIV", @dbfArtPrv ) )
               ...
               while ( tmpFacPrvT )->( !eof() )
                  ...
                  */
                  ...
                  if ::validateRecepcion( tmpFacPrvT, cFacPrvT )
                     ...
                     //MsgInfo( ( tmpFacPrvL )->( dbSeek( ( tmpFacPrvT )->cSerFac + Str( ( tmpFacPrvT )->nNumFac ) + ( tmpFacPrvT )->cSufFac ) ), "Seek" )
...
RETURN( PropiedadesLineasModel():getNombre( cCodPrp1, cValPrp1 ) )
   ...
   DEFAULT uFacPrvL  := D():FacturasProveedoresLineas( nView )
   DEFAULT uTblPro   := D():PropiedadesLineas( nView )

   ?( uFacPrvL )->cCodPr1 + ( uFacPrvL )->cValPr1
...
RETURN( PropiedadesLineasModel():getNombre( cCodPrp2, cValPrp2 ) )
   ...
   ?( uFacPrvL )->cCodPr2 + ( uFacPrvL )->cValPr2
...
Return ( printReportFacPrv( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
