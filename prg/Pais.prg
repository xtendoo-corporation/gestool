...
METHOD DefineFiles( cPath, cDriver )
   ...
   DEFINE DATABASE ::oDbf FILE "PAIS.DBF" CLASS "Pais" PATH ( cPath ) VIA ( cDriver ) COMMENT "Paises"
      ...
      FIELD NAME "cCodIso"    TYPE "C" LEN  3  DEC 0  COMMENT "ISO"                        HIDE COLSIZE 80  OF ::oDbf
...
CLASS PaisModel FROM ADSBaseModel

   METHOD getTableName()                     INLINE ::getDatosTableName( "Pais" )

   METHOD getNombre( cCodigoPais )

END CLASS

//---------------------------------------------------------------------------//

METHOD getNombre( cCodigoPais ) CLASS PaisModel

   local cStm
   local cSql  := "SELECT cNomPai "                      + ;
                     "FROM " + ::getTableName() + " "             + ;
                     "WHERE cCodPai = " + quoted( cCodigoPais ) 
