...
#define _MENUITEM_                 "pedidos_de_clientes"
...
#define _LFIRMA            		 103
#define _NPREPARE            	 104 	//N 	1 		0 ( 0-1 No preparado, 2 parcial y no albaranar, 3 parcial y albaranar, 4 Preparado )
...
FUNCTION PedCli( oMenuItem, oWnd, cCodCli, cCodArt, cCodPre, lPedWeb )
   ...
   */
   ...
   if RolesModel():getRolCambiarEstado( Auth():rolUuid() )
    ...
    DEFINE BTNSHELL oPrepare RESOURCE "GC_BARCODE_SCANNER_" GROUP OF oWndBrw ;
      	NOBORDER ;
      	MENU     This:Toggle() ;
      	TOOLTIP  "Preparar pedido" ;
      	MESSAGE  "Preparar el pedido seleccionado" ;
      	ACTION   ( TPrepareOrder():run( nView ) ) ;
      	HOTKEY   "R";

      	DEFINE BTNSHELL RESOURCE "GC_BARCODE_SCANNER_" OF oWndBrw ;
            ALLOW    EXIT ;
            ACTION   ( SelSend( oWndBrw:oBrw ) );
            TOOLTIP  "Para preparar" ;
            FROM     oPrepare ;

        DEFINE BTNSHELL RESOURCE "GC_BARCODE_SCANNER_" OF oWndBrw ;
            ALLOW    EXIT ;
            ACTION   ( TPrepareOrder():Info( nView, oWndBrw ) );
            TOOLTIP  "Info preparación" ;
            FROM     oPrepare ;
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "PEDCLI" TITLE LblTitle( nMode ) + "Pedidos de clientes"
      ...
      */
	  ...
	  REDEFINE COMBOBOX aGet[ _NPREPARE ] ;
         VAR      aTmp[ _NPREPARE ] ;
         ID       590 ;
         WHEN     ( nMode != ZOOM_MODE );
         ITEMS    ( aPrepare ) ;
   ...
   oDlg:bStart 		:= {|| StartEdtRec( aTmp, aGet, oDlg, nMode, cCodArt, cCodPre, oBrwLin, cCodCli ) }
	...
	ACTIVATE DIALOG oDlg;
      ON INIT  (  InitEdtRec( aTmp, aGet, oDlg, oBrwLin, oBrwInc ), SetDialog( aGet, oSayGetRnt, oGetRnt ) );
...
STATIC FUNCTION EdtDet( aTmp, aGet, dbf, oBrw, lTotLin, cCodArtEnt, nMode, aTmpPed )
   ...
   */
   ...
   nTotRes              	:= AlbaranesClientesLineasModel():nUnidadesEntregadas( cNumPed, aTmp[ _CREF ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] )
   nTotRes              	+= FacturasClientesLineasModel():nUnidadesEntregadas( cNumPed, aTmp[ _CREF ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] )   
   ...
   */
   ...
   cTablaLineasPedidosAlbaran := AlbaranesClientesLineasModel():lineasUnidadesEntregadas( aTmp[ _CSERPED ] + Str( aTmp[ _NNUMPED ] ) + aTmp[ _CSUFPED ], aTmp[ _CREF ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] )
   cTablaLineasPedidosFactura := FacturasClientesLineasModel():lineasUnidadesEntregadas( aTmp[ _CSERPED ] + Str( aTmp[ _NNUMPED ] ) + aTmp[ _CSUFPED ], aTmp[ _CREF ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] )
      ...
      */
      ...
      REDEFINE GET aGet[ _CLOTE ] ;
         ...
         OF 		oFld:aDialogs[1]
   		...
   		aGet[ _CLOTE ]:bValid   := {|| 	StocksModel():lPutStockActual( aTmp[ _CREF ], aTmp[ _CALMLIN ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ], oStkAct ),;
      ...
      */
      ...
      if nMode != ZOOM_MODE
         ...
         if uFieldEmpresa( "lGetLot")
            oDlg:AddFastKey( VK_RETURN,   {|| if( !Empty( aGet[ _CREF ] ), aGet[ _CREF ]:lValid(), ), oBtn:SetFocus(), oBtn:Click() } )
         end if 
      ...
      */
      ...
      if ( D():Articulos( nView ) )->( dbSeek( ( dbfTmpLin )->cRef ) )
         ...
         */
         ...
         ( dbfTmpLin )->nCosDiv  := nCosto( nil, D():Articulos( nView ), dbfKit, , , , aTmpPed[ _CCODCLI ] )
         ...
         */
         ...
         if !Empty( hAtipica )
            ...
            if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
               ( dbfTmpLin )->nPreDiv  := nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), D():Divisas( nView ), dbfKit, D():TiposIva( nView ), , , oNewImp )
            end if
...
Static Function ChangeTarifa( aTmp, aGet, aTmpPed )
   ...
   if !Empty( aTmpPed[ _CCODTAR ] )
      nPrePro  := RetPrcTar( aTmp[ _CREF ], aTmpPed[_CCODTAR], aTmp[_CCODPR1], aTmp[_CCODPR2], aTmp[_CVALPR1], aTmp[_CVALPR2], dbfTarPreL, aTmp[ _NTARLIN ] )
   end if
...
Static Function DataReport( oFr )
   ...
   */
   ...
   oFr:setUserDataSet( "Impuestos pedido",;
...
STATIC FUNCTION SetDlgMode( aTmp, aGet, nMode, oStkAct, oSayPr1, oSayPr2, oSayVp1, oSayVp2, oTotal, aTmpPed, oFld, oRentLin, oBrwAlbCli, oBrwAlbPrv, oBrwFacCli )
   ...
   */
   ...
   if ( empty( aTmp[ _NPREDIV ] ) .or. RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) ) .and. nMode != ZOOM_MODE
...
STATIC FUNCTION appendAsociado( uTmpLin, aTmpPed )
   ...
   /*
   Guardamos los productos asociados------------------------------------------------
   */
      ...
      while ( D():Asociado( nView ) )->cCodArt == cCodArt .and. !( D():Asociado( nView ) )->( eof() )
         ...
         if ( D():Articulos( nView ) )->( dbSeek( ( D():Asociado( nView ) )->cRefAsc ) )
            ...
            */
            ...
            ( dbfTmpLin )->nPreDiv := nRetPreArt( nTarLin, aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), D():Get( "Divisas", nView ), dbfKit, D():Get( "TIva", nView ), , , oNewImp )
      ...
      */
      ...
      if ( D():Articulos( nView ) )->( dbSeek( cCodArt ) ) .or. ( D():Articulos( nView ) )->( dbSeek( Upper( cCodArt ) ) )
            ...
            */
            ...
            do case
               case uFieldEmpresa( "nCosVta" ) < 2
                  ...
                  if nCosPro == 0
                     nCosPro        := nCosto( aTmp[ _CREF ], D():Articulos( nView ), dbfKit, .f., , D():Get( "Divisas", nView ), aTmpPed[ _CCODCLI ] )
                  end if
            ...
            */
            ...
            if uFieldempresa( "lDtoCliLin" )
               ...
               if !empty( aGet ) .and. !empty( aGet[ _NDTO ] )
                  aGet[ _NDTO ]:cText( retFld( aTmpPed[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" ) )
                  aTmp[ _NDTO ]     := retFld( aTmpPed[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               else
                  aTmp[ _NDTO ]     := retFld( aTmpPed[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               end if         
            ...
            if !empty( hAtipica )
               ...
               if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
                  aGet[ _NPREDIV ]:cText( nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), D():Divisas( nView ), dbfKit, D():TiposIva( nView ), , , oNewImp ) )
               end if 
         ...
         */
         ...
         if Empty( aTmp[ _NPREDIV ] ) .or. ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) )
   ...
   IF ( dbfPreCLiT )->( dbSeek( cNumPre ) )
      ...
      IF ( dbfPreCLiT )->lEstado
         ...
         */
         ...
         aTmp[ _NREGIVA   ]   := ( dbfPreCliT )->nRegIva
...
FUNCTION imprimePedidoCliente( cNumeroPedido, cFormatoDocumento, cPrinter, nCopies )
   ...
   if OpenFiles( .t. )
      ...
      if dbSeekInOrd( cNumeroPedido, "nNumPed", D():PedidosClientes( nView ) )
         ...
         genPedCli( IS_PRINTER, nil, cFormatoDocumento, cPrinter, nCopies )
...
function nEstadoUnidadesEntregadasPedidosClientes( cTmpLin )
 	...
 	nUnidadesRecibidas   		:= AlbaranesClientesLineasModel():nUnidadesEntregadas( 	( dbfTmpLin )->cSerPed + Str( ( dbfTmpLin )->nNumPed ) + ( dbfTmpLin )->cSufPed,;
 																						( dbfTmpLin )->cRef,;
 																						( dbfTmpLin )->cCodPr1,;
 																						( dbfTmpLin )->cCodPr2,;
 																						( dbfTmpLin )->cValPr1,;
 	...
 	nUnidadesRecibidas   		+= FacturasClientesLineasModel():nUnidadesEntregadas( 	( dbfTmpLin )->cSerPed + Str( ( dbfTmpLin )->nNumPed ) + ( dbfTmpLin )->cSufPed,;
 																						( dbfTmpLin )->cRef,;
 																						( dbfTmpLin )->cCodPr1,;
 																						( dbfTmpLin )->cCodPr2,;
 																						( dbfTmpLin )->cValPr1,;
...
Return ( printReportPedCli( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
...
Function nTotalUnidadesPedidasClientes( nMes, nYear, cCodCli )
	...
	local cSql  		:= "SELECT cabecerapedidos.cSerPed, "                                  	+ ;
		   						"cabecerapedidos.nNumPed, "                                    	+ ;
		   						"cabecerapedidos.cSufPed, "                                    	+ ;
		   						"cabecerapedidos.cCodCli, "                                    	+ ;
		   						"cabecerapedidos.dFecPed, "                                    	+ ;
		   						"lineaspedidos.cRef, "                                         	+ ;
		   						"lineaspedidos.cDetalle, "                                     	+ ;
		   						"lineaspedidos.nCanPed, "                                      	+ ;
		   						"lineaspedidos.nUniCaja, "                                     	+ ;
		   						"lineaspedidos.nUndKit, "                                      	+ ;
		   						"lineaspedidos.nMedUno, "                                      	+ ;
		   						"lineaspedidos.nMedDos, "                                      	+ ;
		   						"lineaspedidos.nMedTre "                                       	+ ;
		                     	"FROM " + cPatEmp() + "PedCliT AS cabecerapedidos "       	   	+ ;
		                     		"INNER JOIN " + cPatEmp() + "PedCliL AS lineaspedidos "  	+ ;
		                     		"ON cabecerapedidos.cSerPed = lineaspedidos.cSerPed AND "  	+ ;
		                     		"cabecerapedidos.nNumPed = lineaspedidos.nNumPed AND "     	+ ;
		                     		"cabecerapedidos.cSufPed = lineaspedidos.cSufPed " 		   	+ ;	   
		                     	"WHERE"
	...
	cSql 				+=      	" MONTH( dFecPed ) = " +  AllTrim( Str( Int( nMes ) ) )		+ ;
		                     		" AND YEAR( dFecPed ) = " +  AllTrim( Str( Int( nYear ) - 1 ) )
...
Function nTotalImportePedidosClientes( nMes, nYear, cCodCli )
	...
	local cSql  		:= "SELECT cSerPed, "                                  	+ ;
		   						"nNumPed, "                                    	+ ;
		   						"cSufPed, "                                    	+ ;
		   						"cCodCli, "                                    	+ ;
		   						"dFecPed "                                    	+ ;
		   						"FROM " + cPatEmp() + "PedCliT "       	   		+ ;
		                     		"WHERE"
	...
	USE ( cPatEmp() + "PEDCLIL.DBF" ) NEW VIA ( cDriver() ) ALIAS ( cCheckArea( "PEDCLIL", @cPedCliL ) )
   	SET ADSINDEX TO ( cPatEmp() + "PEDCLIL.CDX" ) ADDITIVE
   	SET TAG TO "NNUMPED"		                     		
   	...
   	while !( cStmHead )->( Eof() )
   		...
   		if ( cPedCliL )->( dbSeek( ( cStmHead )->cSerPed + Str( ( cStmHead )->nNumPed ) + ( cStmHead )->cSufPed ) )
   			...
   			while ( cPedCliL )->cSerPed + Str( ( cPedCliL )->nNumPed ) + ( cPedCliL )->cSufPed == ( cStmHead )->cSerPed + Str( ( cStmHead )->nNumPed ) + ( cStmHead )->cSufPed .and.;
   					!( cPedCliL )->( Eof() )
      ...
      case hb_isArray( aTmp )
		 ...
		 if !lDel
            ...
            hset( hBuffer, "pendiente_entregar", nTotNPedPrv( aTmp ) )
		 	...
		 	nEntregadas 	+= AlbaranesClientesLineasModel():nUnidadesEntregadas( 	aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cSerPed" ) ) ] +;
		 																	   		Str( aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "nNumPed" ) ) ] ) +;
		 																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cSufPed" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cRef" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cCodPr1" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cCodPr2" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cValPr1" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cValPr2" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cLote" ) ) ] )
		 	nEntregadas 	+= FacturasClientesLineasModel():nUnidadesEntregadas(  	aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cSerPed" ) ) ] +; 
		 																	   		Str( aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "nNumPed" ) ) ] ) +; 
		 																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cSufPed" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cRef" ) ) ],;
		  																	   		aTmp[ ( D():PedidosClientesLineas( nView ) )->( fieldpos( "cCodPr1" ) ) ],;
      ...
      case hb_isChar( aTmp )
         ...
         if !lDel
            ...
            hset( hBuffer, "pendiente_entregar", nTotNPedPrv( aTmp ) )
		 	...
		 	nEntregadas 	+= AlbaranesClientesLineasModel():nUnidadesEntregadas( 	( aTmp )->cSerPed + Str( ( aTmp )->nNumPed ) + ( aTmp )->cSufPed,;
		  																	   		( aTmp )->cRef,;
		  																	   		( aTmp )->cCodPr1,;
		  																	   		( aTmp )->cCodPr2,;
		  																	   		( aTmp )->cValPr1,;
		  																	   		( aTmp )->cValPr2,;
		  																	   		( aTmp )->cLote )
		 	nEntregadas 	+= FacturasClientesLineasModel():nUnidadesEntregadas( 	( aTmp )->cSerPed + Str( ( aTmp )->nNumPed ) + ( aTmp )->cSufPed,;
		  																	   		( aTmp )->cRef,;
		  																	   		( aTmp )->cCodPr1,;
		  																	   		( aTmp )->cCodPr2,;
		  																	   		( aTmp )->cValPr1,;
