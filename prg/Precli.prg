...
#define _MENUITEM_           "presupuestos_de_clientes"
   ...
   DEFINE DIALOG oDlg RESOURCE "LFACCLI" TITLE LblTitle( nMode ) + "líneas de presupuestos a clientes"
      ...
      */
      ...
      REDEFINE GET aGet[ _CVALPR1 ] ;
         ...
         aGet[ _CVALPR1 ]:bChange   := {|| aGet[ _CVALPR1 ]:Assign(), StocksModel():lPutStockActual( aTmp[ _CREF ], aTmp[ _CALMLIN ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ], oStkAct ) }
      ...
      REDEFINE GET aGet[ _CVALPR2 ] ;
         ...
         aGet[ _CVALPR2 ]:bChange   := {|| aGet[ _CVALPR2 ]:Assign(), StocksModel():lPutStockActual( aTmp[ _CREF ], aTmp[ _CALMLIN ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ], oStkAct ) }
   ...
   if ( D():Asociado( nView ) )->( dbSeek( cCodArt ) )
      ...
      while ( D():Asociado( nView ) )->cCodArt == cCodArt .and. !( D():Asociado( nView ) )->( eof() )
         ...
         if ( D():Articulos( nView ) )->( dbSeek( ( D():Asociado( nView ) )->cRefAsc ) )
            ...
            ( dbfTmpLin )->nCosDiv     := nCosto( nil, D():Articulos( nView ), dbfKit, , , , aTmpPre[ _CCODCLI ] )
            ...
            */
            ...
            ( dbfTmpLin )->nPreDiv := nRetPreArt( nTarLin, aTmpPre[ _CDIVPRE ], aTmpPre[ _LIVAINC ], D():Articulos( nView ), D():Get( "Divisas", nView ), dbfKit, D():Get( "TIva", nView ), , , oNewImp )
         ...
         if ( lChgCodArt )
            ...
            if ( D():Articulos( nView ) )->lLote
               ...
               if !empty( aGet[ _CLOTE ] )
                  ...
                  if empty( aGet[ _CLOTE ]:VarGet() ) .and. uFieldempresa( "lLoaUltLot" )
                  ...
                  if empty( aTmp[ _CLOTE ] ) .and. uFieldempresa( "lLoaUltLot" )
                     aTmp[ _CLOTE ] := cLote 
                  else
            ...
            */
            ...
            do case
               case uFieldEmpresa( "nCosVta" ) < 2
                  ...
                  if nCosPro == 0
                     nCosPro        := nCosto( aTmp[ _CREF ], D():Articulos( nView ), dbfKit, .f., , D():Get( "Divisas", nView ), aTmpPre[ _CCODCLI ] )
                  end if
            ...
            */
            ...
            if uFieldempresa( "lDtoCliLin" )
               ...
               if !empty( aGet ) .and. !empty( aGet[ _NDTO ] )
                  aGet[ _NDTO ]:cText( retFld( aTmpPre[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" ) )
                  aTmp[ _NDTO ]     := retFld( aTmpPre[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               else
                  aTmp[ _NDTO ]     := retFld( aTmpPre[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               end if         
            ...
            */
            ...
            if !Empty( hAtipica )
               ...
               if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
                  aGet[ _NPREDIV ]:cText( nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpPre[ _CDIVPRE ], aTmpPre[_LIVAINC], D():Articulos( nView ), dbfDiv, dbfKit, dbfIva, , , oNewImp ) )
               end if
...
STATIC FUNCTION ChangeTarifa( aTmp, aGet, aTmpPre )
   ...
   if !Empty( aTmpPre[ _CCODTAR ] )
      nPrePro     := RetPrcTar( aTmp[ _CREF ], aTmpPre[_CCODTAR], aTmp[_CCODPR1], aTmp[_CCODPR2], aTmp[_CVALPR1], aTmp[_CVALPR2], dbfTarPreL, aTmp[ _NTARLIN ] )
   end if
...
Static Function DataReport( oFr )
   ...
   */
   ...
   oFr:setUserDataSet( "Impuestos presupuesto",;
      ...
      if ( D():Articulos( nView ) )->( dbSeek( ( dbfTmpLin )->cRef ) )
         ...
         */
         ...
         if !Empty( hAtipica )
            ...
            if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
               ( dbfTmpLin )->nPreDiv  := nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpPre[ _CDIVPRE ], aTmpPre[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, dbfIva, , , oNewImp )
            end if
...
Return ( printReportPreCli( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
