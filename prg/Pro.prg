...
FUNCTION brwPropiedadActual( oGet, oSay, cPrp )
   ...
   DEFINE DIALOG oDlg RESOURCE "HELPENTRY" TITLE cTitle
      ...
      REDEFINE COMBOBOX oCbxOrd ;
         ...
         ID       	102 ;
         ITEMS    	aCbxOrd ;
         ON CHANGE   ( changeComboBrowse( aIdxOrd[ oCbxOrd:nAt ], oBrw, oGetNbr, oCbxOrd, cPrp ) ) ;
         OF       	oDlg
...
METHOD CreateData()
   ...
   */
   ...
   USE ( cPatSnd() + "PRO.DBF" ) NEW VIA ( cLocalDriver() ) SHARED ALIAS ( cCheckArea( "PRO", @tmpProT ) )
   SET ADSINDEX TO ( cPatSnd() + "PRO.CDX" ) ADDITIVE

   USE ( cPatSnd() + "TBLPRO.DBF" ) NEW VIA ( cLocalDriver() ) SHARED ALIAS ( cCheckArea( "TBLPRO", @tmpProL ) )
