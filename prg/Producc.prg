...
METHOD DefineAuxiliar()
   ...
   DEFINE DATABASE ::cAreaTmpLabel FILE ( ::cFileTmpLabel ) CLASS "AuxLbl" ALIAS "AuxLbl" PATH ( cPatTmp() ) VIA ( cLocalDriver() )COMMENT "material producido"
      ...
      FIELD NAME "cLote"      TYPE "C" LEN  64  DEC 0 COMMENT "Lote"                                     OF ::cAreaTmpLabel
         ...
         */
         ...
         //do case
            ...
            //case nDevice == IS_PRINTER
               ...
               oFr:PrintOptions:SetCopies( nCopies )
               //oFr:PrintOptions:SetPageNumbers("1-5")
               oFr:PrintOptions:SetShowDialog( .f. )
               oFr:Print()
...
Method lPrepareDataReportLbl( lDesign ) CLASS TProduccion
   ...
   DEFINE DATABASE ::cAreaTemporalLabel FILE ( ::cFileTemporalLabel ) CLASS "AuxTmpLbl" ALIAS "AuxTmpLbl" PATH ( cPatTmp() ) VIA ( cLocalDriver() )COMMENT "material producido"
      ...
      FIELD NAME "cLote"      TYPE "C" LEN  64  DEC 0 COMMENT "Lote"                                     OF ::cAreaTemporalLabel
...
CLASS PartesProducidonModel FROM TransaccionesComercialesLineasModel

   METHOD getTableName()                     INLINE ::getEmpresaTableName( "ProCab" )

   METHOD getNameOpe( cSerOrd, nNumOrd, cSufOrd )

END CLASS

//---------------------------------------------------------------------------//

METHOD getNameOpe( cSerOrd, nNumOrd, cSufOrd ) CLASS PartesProducidonModel

   local cNameOperacion    := ""
   local cStm              := "NameOpe"  
   local cSql

   cSql                    := "SELECT cabecera.cCodOpe AS cCodOpe, operacion.cDesOpe AS cDesOpe"                                                + ;
                              " FROM " + ::getTableName() + " AS cabecera"                                                                      + ;
                              " INNER JOIN " + ::getEmpresaTableName( "Operacio" ) + " AS operacion ON operacion.cCodOpe = cabecera.cCodOpe "   + ;
                              "  WHERE cSerOrd = " + quoted( cSerOrd )                                                                          + ;
                                 " AND nNumOrd = " + AllTrim( Str( nNumOrd ) )                                                                  + ;
                                 " AND cSufOrd = " + quoted( cSufOrd )                          
