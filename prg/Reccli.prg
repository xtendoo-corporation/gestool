...
#define _MENUITEM_               "recibos_de_clientes"
...
FUNCTION RecCli( oMenuItem, oWnd, aNumRec )
   ...
   */
   ...
   DEFINE BTNSHELL RESOURCE "PREV1" OF oWndBrw ;
      NOBORDER ;
      ACTION   ( InfPreCli():New( "Listado de previsión de cobros" ):Play() ) ;
      TOOLTIP  "Pre(v)isión";
      HOTKEY   "V";
      LEVEL    ACC_ZOOM
   ...
   if Len( aRecibosMatriz ) <= 0
      ...
      */
      ...
      REDEFINE GET aGet[ _CENTEMP] VAR aTmp[ _CENTEMP];
         ...
         PICTURE  "9999" ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         VALID    (  lCalcDC( aTmp[ _CENTEMP], aTmp[ _CSUCEMP], aTmp[ _CDIGEMP], aTmp[ _CCTAEMP], aGet[ _CDIGEMP], aTmp[ _CEPAISIBAN ] ),;
      ...
      */
      ...
      REDEFINE GET aGet[ _CENTCLI] VAR aTmp[ _CENTCLI];
         ...
         PICTURE  "9999" ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         VALID    (  lCalcDC( aTmp[ _CENTCLI], aTmp[ _CSUCCLI], aTmp[ _CDIGCLI], aTmp[ _CCTACLI], aGet[ _CDIGCLI], aTmp[ _CPAISIBAN] ),;
...
function SynRecCli( cPath )
   ...
   USE ( cPath + "FACCLIL.DBF" ) NEW VIA ( cDriver() ) ALIAS ( cCheckArea( "FacCliL", @cFacCliL ) )
   if !lAIS() ; ( cFacCliL )->( ordListAdd( cPath + "FACCLIL.CDX" ) ); else ; ordSetFocus( 1 ) ; end

   USE ( cPath + "FACCLIP.DBF" ) NEW VIA ( cDriver() ) ALIAS ( cCheckArea( "FacCliP", @cFacCliP ) )
...
RETURN ( PrintReportRecCli( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
...
Function ValCheck( aGet, aTmp )
   ...
   if !aTmp[ _LCOBRADO ]
      ...
      if lShowClienteRiesgoAlcanzado( aTmp[ _CCODCLI ] )
         aTmp[ _LCOBRADO ]    := !aTmp[ _LCOBRADO ]
         aGet[ _LCOBRADO ]:Refresh()
         RETURN .f.
      end if
