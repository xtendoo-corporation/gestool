...
#define IDFOUND            3
#define _MENUITEM_         "movimientos_de_almacen"
...
CLASS TRemMovAlm FROM TMasDet
   ...
   DATA  cBitmap           INIT  clrTopAlmacenes
   DATA  oAlmacenOrigen
   ...
   DATA  oBtnImportarInventarioPDA
   ...
   METHOD importarInventarioPDA()
      METHOD importJsonFile( cFileName )
      METHOD addLineInventorarioPDA()
   ...
   METHOD ImprimirSeries()

   METHOD StartPrintSerie()
...
METHOD New( cPath, cDriver, oWndParent, oMenuItem ) CLASS TRemMovAlm
   ...
   DEFAULT oMenuItem       := "movimientos_de_almacen"
...
METHOD DefineFiles( cPath, cDriver ) CLASS TRemMovAlm
   ...
   DEFINE DATABASE ::oDbf FILE "REMMOVT.DBF" CLASS "TRemMovT" ALIAS "RemMovT" PATH ( cPath ) VIA ( cDriver ) COMMENT "Movimientos de almacén"
      ...
      FIELD CALCULATE NAME "cNomAlmOrg"         LEN 20  DEC 0 PICTURE "@!"                      VAL ( oRetFld( ( ::oDbf:nArea )->cAlmOrg, ::oAlmacenOrigen, "cNomAlm" ) ) COMMENT "Nombre org."                COLSIZE 120 OF ::oDbf
      FIELD NAME "cAlmDes"             TYPE "C" LEN 16  DEC 0 PICTURE "@!"                                                                                     COMMENT "Alm. des."                             COLSIZE 60  OF ::oDbf
      FIELD CALCULATE NAME "cNomAlmDes"         LEN 20  DEC 0 PICTURE "@!"                      VAL ( oRetFld( ( ::oDbf:nArea )->cAlmDes, ::oAlmacenDestino, "cNomAlm" ) ) COMMENT "Nombre des."               COLSIZE 120 OF ::oDbf
      FIELD NAME "cCodDiv"             TYPE "C" LEN  3  DEC 0 PICTURE "@!"                                                                                     COMMENT "Div."                            HIDE              OF ::oDbf
      FIELD NAME "nVdvDiv"             TYPE "N" LEN 13  DEC 6 PICTURE "@E 999,999.999999"                                                                      COMMENT "Cambio de la divisa"             HIDE              OF ::oDbf
      ...
      INDEX TO "RemMovT.Cdx" TAG "dFecRem"   ON "Dtos( dFecRem ) + cTimRem"  COMMENT "Fecha"    NODELETED DESCENDING OF ::oDbf
...
METHOD lSave( nMode ) CLASS TRemMovAlm
   ...
   */
   ...
   /*
   Actualizamos el stock en prestashop-----------------------------------------
   */
...
METHOD ImprimirSeries() CLASS TRemMovAlm
   ...
   REDEFINE GET oFmtDoc VAR cFmtDoc ;
      ...
      BITMAP   "LUPA" ;
      ON HELP  ( BrwDocumento( oFmtDoc, oSayFmt, "RM" ) ) ;
      OF       oDlg
   ...
   REDEFINE BUTTON ;
      ID       IDOK ;
      OF       oDlg ;
      ACTION   (  ::StartPrintSerie( Substr( cFmtDoc, 1, 3 ), str( nDocIni, 9 ) + cSufIni, cSerFin + str( nDocFin, 9 ) + cSufFin, oDlg, nil, lCopiasPre, nNumCop, lInvOrden, nRango, dFecDesde, dFecHasta ),;
                  oDlg:end( IDOK ) )
   ...
   oDlg:AddFastKey( VK_F5, {|| ::StartPrintSerie( Substr( cFmtDoc, 1, 3 ), str( nDocIni, 9 ) + cSufIni, cSerFin + str( nDocFin, 9 ) + cSufFin, oDlg, nil, lCopiasPre, nNumCop, lInvOrden, nRango, dFecDesde, dFecHasta ), oDlg:end( IDOK ) } )
...
METHOD StartPrintSerie( cFmtDoc, cDocIni, cDocFin, oDlg, cPrinter, lCopiasPre, nNumCop, lInvOrden, nRango, dFecDesde, dFecHasta  ) CLASS TRemMovAlm
   ...
   if nRango == 1
      ...
      while str( ::oDbf:nNumRem ) + ::oDbf:cSufRem >= cDocIni .and. ;
            str( ::oDbf:nNumRem ) + ::oDbf:cSufRem <= cDocFin
...
METHOD ImportAlmacen( nMode, oDlg ) CLASS TRemMovAlm 
   ...
   DEFINE DIALOG ::oDlgImport RESOURCE "Importar_Almacen_ADS"
      ...
      REDEFINE CHECKBOX ::lStockActual ;
         ID       390 ;
         OF       ::oDlgImport ;

      ::oMtrStock      := TApoloMeter():ReDefine( 400, { | u | if( pCount() == 0, ::nMtrStock, ::nMtrStock := u ) }, 10, ::oDlgImport, .f., , , .t., Rgb( 255,255,255 ), , Rgb( 128,255,0 ) )
...
METHOD alreadyInclude( sStkAlm ) CLASS TRemMovAlm
   ...
   while ( !::oDetMovimientos:oDbfVir:eof() )
      ...
      if ::oDetMovimientos:oDbfVir:cRefMov  == hGet( sStkAlm, "articulo" )    .and.;
         ::oDetMovimientos:oDbfVir:cCodPr1  == hGet( sStkAlm, "propiedad1" )  .and.;
         ::oDetMovimientos:oDbfVir:cCodPr2  == hGet( sStkAlm, "propiedad2" )  .and.;
         ::oDetMovimientos:oDbfVir:cValPr1  == hGet( sStkAlm, "valor1" )      .and.;
         ::oDetMovimientos:oDbfVir:cValPr2  == hGet( sStkAlm, "lote" )
...
//---------------------------------------------------------------------------//
// Trata de insertar el articulo en la remesa de movimientos-------------------
...
METHOD ActualizaStockWeb() CLASS TRemMovAlm
...
METHOD importarInventarioPDA() CLASS TRemMovAlm
...
METHOD importJsonFile( cFileName ) CLASS TRemMovAlm
...
METHOD addLineInventorarioPDA( hLine ) CLASS TRemMovAlm
...
METHOD moveFileToProcessed( cFileName ) CLASS TRemMovAlm
