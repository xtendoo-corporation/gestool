...
#define _MENUITEM_               "sat_de_clientes"
            ...
            */
            ...
            if uFieldempresa( "lDtoCliLin" )
               ...
               if !empty( aGet ) .and. !empty( aGet[ _NDTO ] )
                  aGet[ _NDTO ]:cText( retFld( aTmpSat[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" ) )
                  aTmp[ _NDTO ]     := retFld( aTmpSat[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               else
                  aTmp[ _NDTO ]     := retFld( aTmpSat[ _CCODCLI ], D():Clientes( nView ), "nDtoEsp" )
               end if         
            ...
            */
            ...
            if !Empty( hAtipica )
               ...
               if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
                  aGet[ _NPREDIV ]:cText( nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpSat[ _CDIVSAT ], aTmpSat[_LIVAINC], D():Articulos( nView ), dbfDiv, dbfKit, dbfIva, , , oNewImp ) )
               end if
      ...
      if ( D():Articulos( nView ) )->( dbSeek( ( dbfTmpLin )->cRef ) )
         ...
         */
         ...
         if !Empty( hAtipica )
            ...
            if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
               ( dbfTmpLin )->nPreDiv  := nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTmpSat[ _CDIVSAT ], aTmpSat[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, dbfIva, , , oNewImp )
            end if
...
STATIC FUNCTION ChangeTarifa( aTmp, aGet, aTmpSat )
   ...
   if !Empty( aTmpSat[ _CCODTAR ] )
      nPrePro     := RetPrcTar( aTmp[ _CREF ], aTmpSat[_CCODTAR], aTmp[_CCODPR1], aTmp[_CCODPR2], aTmp[_CVALPR1], aTmp[_CVALPR2], dbfTarPreL, aTmp[ _NTARLIN ] )
   end if
...
Return ( printReportSATCli( IS_MAIL, 1, ImpresoraDefectoUsuario(), cCodigoDocumento ) )
