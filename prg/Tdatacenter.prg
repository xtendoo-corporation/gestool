...
CLASS TDataCenter
   ...
   DATA        aLgcIndices                INIT { .t., .t., .f., .f. }
   DATA        aChkIndices                INIT Array( 4 )

   DATA        aProgress                  INIT Array( 4 )
   DATA        nProgress                  INIT { 0, 0, 0, 0 }

   DATA        cMsg                       INIT ""
   DATA        oMsg

   DATA        lAutoStart                 INIT .f.
   ...
   METHOD SeedersSqlToAds()
      METHOD SeederSqlMovAlm()
         METHOD AddSeederMovAlm()
      METHOD SeederSqlAgenda()
      METHOD SeederSqlTransportistas()
         METHOD AddSeederTransportista( hHashHead )
      METHOD SeederSqlConfiguracionEmpresa()
      METHOD SeederSqlOdooIds()
      METHOD SeederSqlUsers()
         METHOD SeederSqlUsuarios()
         METHOD SeederSqlRoles()
         METHOD SeederSqlPermisos()
         METHOD SeederSqlDetallePermisos()
   ...
   METHOD allOptions()                          INLINE ( ::changeOptions( .t. ) )
   METHOD noneOptions()                         INLINE ( ::changeOptions( .f. ) )
   METHOD changeOptions( lOption )

   METHOD CloseAllTables()
...
METHOD CloseAllTables() CLASS TDataCenter
...
METHOD CreateDataDictionary()
   ...
   if msgYesNo( "La base de datos " + ::getDataDictionaryFile() + " no existe, ¿desea crearla?")
      ...
      //AdsDDCreateUser( , "ADSSYS", "", ::cDataDictionaryComment )
   ...
   */
   ...
   DEFINE DIALOG ::oDlg RESOURCE "ReindexADS" OF oWnd()
      ...
      */
      ...
      REDEFINE BUTTON ID 500        OF ::oDlg ACTION ( ::allOptions() )

      REDEFINE BUTTON ID 510        OF ::oDlg ACTION ( ::noneOptions() )
...
METHOD AddSeederMovAlm( hHashHead )
