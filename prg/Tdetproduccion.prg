...
CLASS TDetProduccion FROM TDetalleArticulos
   ...
   Method ActualizaSqlStock( cNumDoc )
   Method RollBackSqlStock( cNumDoc )
...
METHOD DefineFiles( cPath, cVia, lUniqueName, cFileName ) CLASS TDetProduccion
   ...
   DEFINE TABLE oDbf FILE ( cFileName ) CLASS ( cFileName ) ALIAS ( cFileName ) PATH ( cPath ) VIA ( cVia ) COMMENT "Materiales producidos"
      ...
      FIELD NAME "cLote"      TYPE "C" LEN 64  DEC 0 COMMENT "Lote"                                COLSIZE  80 OF oDbf
      ...
      FIELD NAME "lValidado"  TYPE "L" LEN  10 DEC 0 COMMENT "Lógico validado con consolidación"   HIDE        OF oDbf  
      ...
      INDEX TO ( cFileName )  TAG "cStkFast" ON "cCodArt + cAlmOrd + dtos( dFecOrd ) + cHorIni + cValPr1 + cValPr2 + cLote";
                                                                                                                     FOR "!lValidado" NODELETED DESCENDING OF oDbf
...
METHOD LoaArticulo( oGetArticulo, oGetNombre ) CLASS TDetProduccion
   ...
   local cLotArt     := ::oLote:VarGet()
   local lChgLotArt  := ( Empty( ::cOldLotArt ) .or. Rtrim( ::cOldLotArt ) != Rtrim( cLotArt ) )
   local nCosPro     := 0
   ...
   if Empty( cCodArt )
      ...
      */
      ...
      if ::oParent:oArt:Seek( cCodArt ) .or. ::oParent:oArt:Seek( Upper( cCodArt ) )
         ...
         if !uFieldEmpresa( "lNStkAct" )
            ::oStkAct:cText( StocksModel():nGlobalStockArticulo( cCodArt, ::oDbfVir:cAlmOrd ) )
         end if
      ...
      MENUITEM "&1. Rotor"
         ...
         MENU
            ...
            MENUITEM    "&3. Informe de stock";
               ...
               RESOURCE "Info16";
               ACTION   ( InfStock( ::oDbfVir:cCodArt ) );
...
RETURN ( if ( !uFieldEmpresa( "lNStkAct" ), ::oStkAct:cText( StocksModel():nGlobalStockArticulo( ::oDbfVir:cCodArt, ::oDbfVir:cAlmOrd ) ), ), .t. )
...
Method ActualizaSqlStock( cNumDoc ) CLASS TDetProduccion

   local nRec
   local nOrdAnt

   nRec     := ::oParent:oDetProduccion:oDbf:Recno()
   nOrdAnt  := ::oParent:oDetProduccion:oDbf:OrdSetFocus( "cNumOrd" )

   if ::oParent:oDetProduccion:oDbf:Seek( cNumDoc )

      while ::oParent:oDetProduccion:oDbf:cSerOrd + Str( ::oParent:oDetProduccion:oDbf:nNumOrd ) + ::oParent:oDetProduccion:oDbf:cSufOrd == cNumDoc .and. ;
            !::oParent:oDetProduccion:oDbf:Eof()

         //::oParent:oSqlStock:sumaStock( hStockBufferDetalleProduccion( ::oParent:oDetProduccion:oDbf ) )

         ::oParent:oDetProduccion:oDbf:Skip()

      end while

  end if

   ::oParent:oDetProduccion:oDbf:OrdSetFocus( nOrdAnt )
   ::oParent:oDetProduccion:oDbf:GoTo( nRec )

Return .t.

//---------------------------------------------------------------------------//

Method RollBackSqlStock( cNumDoc ) CLASS TDetProduccion
   ...
   if ::oParent:oDetProduccion:oDbf:Seek( cNumDoc )
      ...
      while ::oParent:oDetProduccion:oDbf:cSerOrd + Str( ::oParent:oDetProduccion:oDbf:nNumOrd ) + ::oParent:oDetProduccion:oDbf:cSufOrd == cNumDoc .and. ;
         ...
         //::oParent:oSqlStock:sumaRollBackStock( hStockBufferDetalleProduccion( ::oParent:oDetProduccion:oDbf ) )
...
FUNCTION hStockBufferDetalleProduccion( oDbf )
   ...
   do case
      case hb_isObject( oDbf )
         ...
         hset( hBuffer, "codigo_almacen_entrada", AllTrim( oDbf:cAlmOrd ) )
         hset( hBuffer, "codigo_almacen_salida", "" )
