...
CLASS TFastVentasArticulos FROM TFastReportInfGen 
   ...
   METHOD AddRectificativaCliente()
   METHOD AddTicket()

   METHOD AddArticulo()
      METHOD appendStockArticulo()
      METHOD appendBlakStockArticulo()
   ...
   METHOD processAllProducts()

   METHOD processAllProductsAlmacen()
   ...
   METHOD nTotalStockArticulo()           INLINE ( StocksModel():nGlobalStockArticulo( ::oDbf:cCodArt ) )
   ...
   METHOD isProductInReport()

   METHOD isProductAlmacenInReport()
   ...
   METHOD StockActualArticulo( cCodArt )              INLINE ( StocksModel():nGlobalStockArticulo( Padr( cCodArt, 18 ) ) )
   METHOD StockAlmacenArticulo( cCodArt, cCodAlm )    INLINE ( StocksModel():nGlobalStockArticulo( Padr( cCodArt, 18 ), Padr( cCodAlm, 16 ) ) )
   METHOD StockLoteArticulo( cCodArt, cLote )         INLINE ( StocksModel():nStockArticulo( Padr( cCodArt, 18 ), , , , , , Padr( cLote, 14 ) ) )
   ...
   METHOD nTotalVentasArticulos()

   METHOD nTotalUnidadesVendidas()

   METHOD getUnidadesVendidas( cCodArt, cCodAlm )
...
METHOD Create( uParam ) CLASS TFastVentasArticulos
   ...
   ::AddField( "cNomAlm",     "C",100, 0, {|| "@!" }, "Nombre del almacén"                      )
   ...
   ::AddField( "nRe",         "N",  6, 2, {|| "" },   "Porcentaje recargo "                     )
   ::AddField( "nReArt",      "N", 16, 6, {|| "" },   "Total RE artículo"                       )
   ...
   ::AddField( "cLote",       "C", 64, 0, {|| "" },   "Número de lote"                          )
   ...
   ::AddField( "dFecCobro",   "D",  8, 0, {|| "" },   "FechaCobro"                              )

   ::AddField( "lAnulado",    "L",  1, 0, {|| "" },   "Linea pedido anulado"                    )
...
METHOD BuildReportCorrespondences()
   ...
   ::hReport   := {  "Listado" =>;                     
                     ...
                     "Rectificativas de clientes" => ;
                        {  "Generate" =>  {||   ::AddRectificativaCliente( .t. ),;
                                                ::processAllClients(),;
                                                ::processAllProducts() },;
...
Method lValidRegister() CLASS TFastVentasArticulos
   ...
   if empty( ::oDbf:cCodArt ) .and. empty( ::oDbf:cNomArt )
      Return .f.
   end if
   ...
   if !( ::oTFastReportOptions:getOptionValue( "Incluir artículos sin ventas", .f. ) )
   ...
   if !( ::oTFastReportOptions:getOptionValue( "Incluir almacenes y artículos sin ventas", .f. ) )
...
METHOD BuildTree( oTree, lLoadFile ) CLASS TFastVentasArticulos
   ...
   aReports := {  {  "Title"     => "Listado",;                        
                  ...
                  { ;
                     { "Title"      => "SAT de clientes",;
                       ...
                       "Options"    => {  "Incluir clientes sin ventas" => {  "Options"   => .f.,;
                                                                              "Value"     => .f. },;
                                          "Incluir artículos sin ventas" => { "Options"   => .f.,;
                                                                              "Value"     => .f. } } },;  
                     ...
                     {  "Title"     => "Ventas",;
                        ...
                        "Options"   => {  "Incluir clientes sin ventas" => {  "Options"   => .f.,;
                                                                              "Value"     => .f. },;
                                          "Incluir almacenes y artículos sin ventas" => { "Options"   => .f.,;
                                                                              "Value"     => .f. },;
                                          "Incluir artículos sin ventas" => { "Options"   => .f.,;
                                                                              "Value"     => .f. } } },;
                  } ;
                  ...
                  {  "Title"     => "Movimientos almacén",;
                     "Image"     => 25,;
                     "Type"      => "Movimientosalmacen",;
                     "Directory" => "Articulos\MovimientosAlmacen",;
                     "File"      => "Movimientos.fr3" },;
                  {  "Title"     => "Existencias",;
                     "Image"     => 16,;
                     "Subnode"   =>;
                  { ;
...
METHOD DataReport() CLASS TFastVentasArticulos
   ...
   */
   ...
   ::oFastReport:SetMasterDetail(   "Informe", "Tipos de " + cImp(),                {|| ::oDbf:TipoIva } )
...
METHOD AddSATClientes() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():SATClientesLineas( ::nView ) )->( eof() )
      ...
      if ( D():SATClientes( ::nView ) )->( dbseek( D():SATClientesLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():SATClientesLineas( ::nView ) )->cDetalle ), ( D():SATClientesLineas( ::nView ) )->cDetalle, ( D():SATClientesLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:cNomTrn    := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():SATClientes(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():SATClientes( ::nView ) )->cCodUsr )
...
METHOD AddPresupuestoClientes() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():PresupuestosClientesLineas( ::nView ) )->( eof() )
      ...
      if ( D():PresupuestosClientes( ::nView ) )->( dbseek( D():PresupuestosClientesLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():PresupuestosClientesLineas( ::nView ) )->cDetalle ), ( D():PresupuestosClientesLineas( ::nView ) )->cDetalle, ( D():PresupuestosClientesLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:cNomTrn    := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():PresupuestosClientes(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():PresupuestosClientes( ::nView ) )->cCodUsr )
...
METHOD AddPedidoClientes() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():PedidosClientesLineas( ::nView ) )->( eof() )
      ...
      if ( D():PedidosClientes( ::nView ) )->( dbseek( D():PedidosClientesLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():PedidosClientesLineas( ::nView ) )->cDetalle ), ( D():PedidosClientesLineas( ::nView ) )->cDetalle, ( D():PedidosClientesLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:lAnulado   := ( D():PedidosClientesLineas( ::nView ) )->lAnulado
         ...
         ::oDbf:cNomTrn    := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():PedidosClientes(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():PedidosClientes( ::nView ) )->cCodUsr )
...
METHOD AddAlbaranCliente( lFacturados ) CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():AlbaranesClientesLineas( ::nView ) )->( eof() )
      ...
      if D():gotoIdAlbaranesClientes( D():AlbaranesClientesLineasId( ::nView ), ::nView )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():AlbaranesClientesLineas( ::nView ) )->cDetalle ), ( D():AlbaranesClientesLineas( ::nView ) )->cDetalle, ( D():AlbaranesClientesLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:cNomTrn    := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():AlbaranesClientes(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():AlbaranesClientes( ::nView ) )->cCodUsr )
...
METHOD AddFacturaCliente() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():FacturasClientesLineas( ::nView ) )->( eof() )
      ...
      if D():gotoIdFacturasClientes( D():FacturasClientesLineasId( ::nView ), ::nView ) 
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():FacturasClientesLineas( ::nView ) )->cDetalle ), ( D():FacturasClientesLineas( ::nView ) )->cDetalle, ( D():FacturasClientesLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:nRe        := ( D():FacturasClientesLineas( ::nView ) )->nReq
         ...
         ::oDbf:nReArt     := nReqLFacCli( ( D():FacturasClientesLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv )
         ...
         ::oDbf:cCodAge    := if( Empty( ( D():FacturasClientesLineas( ::nView ) )->cCodAge ), ( D():FacturasClientes( ::nView ) )->cCodAge, ( D():FacturasClientesLineas( ::nView ) )->cCodAge )
         ::oDbf:cCodTrn    := ( D():FacturasClientes( ::nView ) )->cCodTrn
         ::oDbf:cNomTrn    := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():FacturasClientes(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():FacturasClientes( ::nView ) )->cCodUsr )
         ...
         ::oDbf:dFecCobro  := FacturasClientesCobrosModel():dFechaCobro( ( D():FacturasClientesLineas( ::nView ) )->cSerie, ( D():FacturasClientesLineas( ::nView ) )->nNumFac, ( D():FacturasClientesLineas( ::nView ) )->cSufFac )
...
METHOD AddRectificativaCliente() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():FacturasRectificativasLineas( ::nView ) )->( eof() )
      ...
      if D():gotoIdFacturasRectificativas( D():FacturasRectificativasLineasId( ::nView ), ::nView )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():FacturasRectificativasLineas( ::nView ) )->cDetalle ), ( D():FacturasRectificativasLineas( ::nView ) )->cDetalle, ( D():FacturasRectificativasLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:cNomTrn    := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():FacturasRectificativas(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():FacturasRectificativas( ::nView ) )->cCodUsr )
      ...
      if D():gotoIdTiketsLineas( D():TiketsId( ::nView ), ::nView ) 
         ...
         while ( D():TiketsId( ::nView ) ) == ( D():TiketsLineasId( ::nView ) ) .and. !( D():TiketsLineas( ::nView ) )->( eof() )
            ...
            if ( !empty( ( D():TiketsLineas( ::nView ) )->cCbaTil ) )
               ...
               ::oDbf:cCodTrn    := ""
               ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():Tikets( ::nView ) )->cCcjTik )
...
METHOD listadoArticulo() CLASS TFastVentasArticulos
   ...
   */
   ...
   while !( D():Articulos( ::nView ) )->( eof() ) .and. !::lBreak
      ...
      ::oDbf:nUniArt  := ::nTotalUnidadesVendidas()
      ::oDbf:nTotArt  := ::nTotalVentasArticulos()

      hTotPdt         := StocksModel():nTotStockPendiente( ( D():Articulos( ::nView ) )->Codigo )

      ::oDbf:nPdtRec  := hGet( hTotPdt, "nTotPdtRec" )
      ::oDbf:nPdtEnt  := hGet( hTotPdt, "nTotPdtEnt" )
...
METHOD AddArticulo() CLASS TFastVentasArticulos
   ...
   while !( D():Articulos( ::nView ) )->( eof() ) .and. !::lBreak
      ...
      aStockArticulo       := DBHScatter( StocksModel():getSqlBrwStock( ( D():Articulos( ::nView ) )->Codigo, ::cAlmacenDefecto, ::dFinInf ) )
                                                   

      if !empty( aStockArticulo )
         ::appendStockArticulo( aStockArticulo )
      else
         ::appendBlakStockArticulo()
      end if 
...
METHOD appendStockArticulo( aStockArticulo )
   ...
   for each hStock in aStockArticulo
      ...
      if !empty( hGet( hStock, "articulo" ) )

         ::oDbf:Blank()

         ::oDbf:cCodArt    := hGet( hStock, "articulo" )
         ::oDbf:cCodAlm    := hGet( hStock, "almacen" )
         ::oDbf:cCodPr1    := hGet( hStock, "propiedad1" )
         ::oDbf:cCodPr2    := hGet( hStock, "propiedad2" )
         ::oDbf:cValPr1    := hGet( hStock, "valor1" )
         ::oDbf:cValPr2    := hGet( hStock, "valor2" )
         ::oDbf:cLote      := hGet( hStock, "lote" )
         //::oDbf:dFecCad    := hGet( hStock, "" )  //:dFechaCaducidad
         ::oDbf:nUniArt    := hGet( hStock, "unidades" )
         ::oDbf:nBultos    := hGet( hStock, "bultos" )
         ::oDbf:nCajas     := hGet( hStock, "cajas" )
         //::oDbf:nPdtRec    := hGet( hStock, "pdtrecibir" )
         //::oDbf:nPdtEnt    := hGet( hStock, "pdtentrega" )
...
METHOD appendBlakStockArticulo()
   ...
   if len( aAlmacenes ) != 0
      ...
      for each hAlmacen in aAlmacenes
         ...
         ::oDbf:cCodArt    := ( D():Articulos( ::nView ) )->Codigo
         ::oDbf:cCodAlm    := hGet( hAlmacen, "cCodAlm" )
         ::oDbf:nUniArt    := 0
         ::oDbf:nBultos    := 0
         ::oDbf:nCajas     := 0
...
METHOD AddProducido() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():PartesProduccionMaterial( ::nView ) )->( Eof() )
      ...
      if lChkSer( ( D():PartesProduccionMaterial( ::nView ) )->cSerOrd, ::aSer )
         ...
         ::oDbf:cLote      := ( D():PartesProduccionMaterial( ::nView ) )->cLote
...
METHOD AddConsumido() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():PartesProduccionMateriaPrima( ::nView ) )->( Eof() )
      ...
      if lChkSer( ( D():PartesProduccionMateriaPrima( ::nView ) )->cSerOrd, ::aSer )
         ...
         ::oDbf:cLote      := ( D():PartesProduccionMateriaPrima( ::nView ) )->cLote
...
METHOD AddMovimientoAlmacen() CLASS TFastVentasArticulos
   ...
   oRowSet              := MovimientosAlmacenLineasModel():getInfoForReport( Self )   
   ...
   while !(::lBreak ) .and. !( ( oRowSet )->( Eof() ) )
      ...
      ::oDbf:cCodArt    := ( oRowSet )->cRefMov
      ::oDbf:cNomArt    := retFld( ( oRowSet )->cRefMov, D():Articulos( ::nView ), "Nombre", "Codigo" )

      ::oDbf:cCodFam    := RetFld( ( oRowSet )->cRefMov, D():Articulos( ::nView ), "Familia", "Codigo" )
      ::oDbf:cNomFam    := RetFld( ::oDbf:cCodFam, D():Familias( ::nView ) )
     
      ::oDbf:cCodTip    := RetFld( ( oRowSet )->cRefMov, ( D():Articulos( ::nView ) ), "cCodTip", "Codigo" )
      ::oDbf:cCodTemp   := RetFld( ( oRowSet )->cRefMov, ( D():Articulos( ::nView ) ), "cCodTemp", "Codigo" )
      ::oDbf:cCodFab    := RetFld( ( oRowSet )->cRefMov, ( D():Articulos( ::nView ) ), "cCodFab", "Codigo" )
      ::oDbf:cCodCate   := RetFld( ( oRowSet )->cRefMov, ( D():Articulos( ::nView ) ), "cCodCate", "Codigo" )
      ::oDbf:cDesUbi    := RetFld( ( oRowSet )->cRefMov, ( D():Articulos( ::nView ) ), "cDesUbi", "Codigo" )
      ::oDbf:cCodEnv    := RetFld( ( oRowSet )->cRefMov, ( D():Articulos( ::nView ) ), "cCodFra", "Codigo" )                    

      ::oDbf:cCodAlm    := ( oRowSet )->cAliMov
      ::oDbf:cAlmOrg    := ( oRowSet )->cAloMov

      ::oDbf:nBultos    := ( oRowSet )->nBultos
      ::oDbf:nCajas     := ( oRowSet )->nCajMov
      
      if lCalCaj()
         ::oDbf:nUniArt := notCaja( ( oRowSet )->nCajMov ) * ( oRowSet )->nUndMov
      else
         ::oDbf:nUniArt := ( oRowSet )->nUndMov
      end if
      ::oDbf:nPreArt    := ( oRowSet )->nPreDiv

      ::oDbf:nBrtArt    := 0
      
      ::oDbf:nTotArt    := ::oDbf:nUniArt * ( oRowSet )->nPreDiv

      ::oDbf:cCodPr1    := ( oRowSet )->cCodPr1
      ::oDbf:cCodPr2    := ( oRowSet )->cCodPr2
      ::oDbf:cValPr1    := ( oRowSet )->cValPr1
      ::oDbf:cValPr2    := ( oRowSet )->cValPr2

      ::oDbf:cClsDoc    := MOV_ALM
      
      ::oDbf:cTipDoc    := cTextoMovimiento( oRowSet )

      ::oDbf:cSerDoc    := ""
      ::oDbf:cNumDoc    := alltrim( Str( ( oRowSet )->nNumRem ) )
      ::oDbf:cSufDoc    := ( oRowSet )->cSufRem

      ::oDbf:cIdeDoc    := Str( ( oRowSet )->nNumRem ) + ( oRowSet )->cSufRem

      ::oDbf:nNumLin    := ( oRowSet )->nNumLin

      ::oDbf:dFecDoc    := ( oRowSet )->dFecMov
      ::oDbf:nMesDoc    := month( ( oRowSet )->dFecMov )
      ::oDbf:nAnoDoc    := year( ( oRowSet )->dFecMov )

      ::oDbf:cHorDoc    := SubStr( ( oRowSet )->cTimMov, 1, 2 )
      ::oDbf:cMinDoc    := SubStr( ( oRowSet )->cTimMov, 4, 2 )
      ::oDbf:cSecDoc    := SubStr( ( oRowSet )->cTimMov, 7, 2 )
      ::oDbf:cTimDoc    := ( oRowSet )->cTimMov

      ::oDbf:cLote      := ( oRowSet )->cLote
      ::oDbf:dFecCad    := ( oRowSet )->dFecCad
...
METHOD AddPedidoProveedor() CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():PedidosProveedoresLineas( ::nView ) )->( eof() )
      ...
      if ( D():PedidosProveedores( ::nView ) )->( dbseek( D():PedidosProveedoresLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():PedidosProveedoresLineas( ::nView ) )->cDetalle ), ( D():PedidosProveedoresLineas( ::nView ) )->cDetalle, ( D():PedidosProveedoresLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:lAnulado   := ( D():PedidosProveedoresLineas( ::nView ) )->lAnulado
         ...
         ::oDbf:cPrvHab    := ( D():PedidosProveedores( ::nView ) )->cCodPrv
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():PedidosProveedores( ::nView ) )->cCodUsr )

         ::oDbf:nRecibi    := AlbaranesProveedoresLineasModel():nUnidadesRecibidas( ( D():PedidosProveedoresLineas( ::nView ) )->cSerPed + Str( ( D():PedidosProveedoresLineas( ::nView ) )->nNumPed ) + ( D():PedidosProveedoresLineas( ::nView ) )->cSufPed,;
                                                                              ( D():PedidosProveedoresLineas( ::nView ) )->cRef,;
                                                                              ( D():PedidosProveedoresLineas( ::nView ) )->cCodPr1,;
                                                                              ( D():PedidosProveedoresLineas( ::nView ) )->cCodPr2,;
                                                                              ( D():PedidosProveedoresLineas( ::nView ) )->cValPr1,;  
                                                                              ( D():PedidosProveedoresLineas( ::nView ) )->cValPr2,;
                                                                              ( D():PedidosProveedoresLineas( ::nView ) )->cLote )
...
METHOD AddAlbaranProveedor( lFacturados ) CLASS TFastVentasArticulos
   ...
   while !::lBreak .and. !( D():AlbaranesProveedoresLineas( ::nView ) )->( eof() )
      ...
      if ( D():AlbaranesProveedores( ::nView ) )->( dbseek( D():AlbaranesProveedoresLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():AlbaranesProveedoresLineas( ::nView ) )->cDetalle ), ( D():AlbaranesProveedoresLineas( ::nView ) )->cDetalle, ( D():AlbaranesProveedoresLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():AlbaranesProveedores( ::nView ) )->cCodUsr )
   ...
   */
   ...
   while !::lBreak .and. !( D():FacturasProveedoresLineas( ::nView ) )->( eof() )
      ...
      if ( D():FacturasProveedores( ::nView ) )->( dbseek( D():FacturasProveedoresLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():FacturasProveedoresLineas( ::nView ) )->cDetalle ), ( D():FacturasProveedoresLineas( ::nView ) )->cDetalle, ( D():FacturasProveedoresLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:nIva       := ( D():FacturasProveedoresLineas( ::nView ) )->nIva
         ::oDbf:nRe        := ( D():FacturasProveedoresLineas( ::nView ) )->nReq
         ...
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():FacturasProveedores( ::nView ) )->cCodUsr )
   ...
   */
   ...
   while !::lBreak .and. !( D():FacturasRectificativasProveedoresLineas( ::nView ) )->( eof() )
      ...
      if ( D():FacturasRectificativasProveedores( ::nView ) )->( dbseek( D():FacturasRectificativasProveedoresLineasId( ::nView ) ) )
         ...
         ::oDbf:cNomArt    := if( !Empty( ( D():FacturasRectificativasProveedoresLineas( ::nView ) )->cDetalle ), ( D():FacturasRectificativasProveedoresLineas( ::nView ) )->cDetalle, ( D():FacturasRectificativasProveedoresLineas( ::nView ) )->mLngDes )
         ...
         ::oDbf:cCodUsr    := UsuariosModel():getNombreWhereCodigo( ( D():FacturasRectificativasProveedores( ::nView ) )->cCodUsr )
...
METHOD processAllClients() CLASS TFastVentasArticulos
   ...
   while !( D():Clientes( ::nView ) )->( Eof() ) .and. !::lBreak
      ...
      if !( ::isClientInReport( ( D():Clientes( ::nView ) )->Cod ) )
         ...
         ::oDbf:cNomTrn       := TransportistasModel():getField( "cNomTrn", "cCodTrn", ( D():Clientes(::nView ) )->cCodTrn )
         ::oDbf:cCodUsr       := UsuariosModel():getNombreWhereCodigo( ( D():Clientes( ::nView ) )->cCodUsr )
...
METHOD processAllProducts() CLASS TFastVentasArticulos
   ...
   while !( D():Articulos( ::nView ) )->( Eof() ) .and. !::lBreak
      ...
      if !( ::isProductInReport( ( D():Articulos( ::nView ) )->Codigo ) )
         ...
         ::oDbf:cNomArt       := ( D():Articulos( ::nView ) )->Nombre
         ::oDbf:cCodFam       := ( D():Articulos( ::nView ) )->Familia
         ::oDbf:cNomFam       := RetFld( ( D():Articulos( ::nView ) )->Familia, D():Familias( ::nView ) )
         ::oDbf:TipoIva       := ( D():Articulos( ::nView ) )->TipoIva
         ::oDbf:cCodTip       := ( D():Articulos( ::nView ) )->cCodTip
         ::oDbf:cCodEst       := ( D():Articulos( ::nView ) )->Codigo
         ::oDbf:cCodTemp      := ( D():Articulos( ::nView ) )->Codigo
         ::oDbf:cCodFab       := ( D():Articulos( ::nView ) )->Codigo
         ::oDbf:cCodUsr       := UsuariosModel():getNombreWhereCodigo( ( D():Clientes( ::nView ) )->cCodUsr )
         ::oDbf:cCodEnv       := ( D():Articulos( ::nView ) )->Codigo
         ::oDbf:cCodCate      := ( D():Articulos( ::nView ) )->Codigo
         ::oDbf:cPrvHab       := ( D():Articulos( ::nView ) )->cPrvHab
...
METHOD processAllProductsAlmacen() CLASS TFastVentasArticulos
   ...
   ( D():Articulos( ::nView ) )->( OrdSetFocus( "Codigo" ) )
   nOrdAnt := ( D():Almacen( ::nView ) )->( OrdSetFocus( "cCodAlm" ) )

   ( D():Articulos( ::nView ) )->( dbgotop() )
   
   while !( D():Articulos( ::nView ) )->( Eof() ) .and. !::lBreak

      ( D():Almacen( ::nView ) )->( dbgotop() )   

      while !( D():Almacen( ::nView ) )->( Eof() ) .and. !::lBreak         

         if !( ::isProductAlmacenInReport( ( D():Articulos( ::nView ) )->Codigo, ( D():Almacen( ::nView ) )->cCodAlm ) )
            ...
            ::oDbf:cCodCate      := ( D():Articulos( ::nView ) )->Codigo
            ::oDbf:cPrvHab       := ( D():Articulos( ::nView ) )->cPrvHab
            ::oDbf:cCodAlm       := ( D():Almacen( ::nView ) )->cCodAlm
            ::oDbf:cNomAlm       := ( D():Almacen( ::nView ) )->cNomAlm
   ...
   D():setStatusArticulos( ::nView )
   nOrdAnt := ( D():Almacen( ::nView ) )->( OrdSetFocus( nOrdAnt ) )
...
METHOD isProductInReport( cCodArt ) CLASS TFastVentasArticulos

RETURN ( ::oDbf:SeekInOrd( cCodArt, "cCodArt" ) )

//---------------------------------------------------------------------------//

METHOD isProductAlmacenInReport( cCodArt, cCodAlm ) CLASS TFastVentasArticulos

RETURN ( ::oDbf:SeekInOrd( cCodArt + cCodAlm, "cCodAlm" ) )
...
METHOD nTotalUnidadesVendidas() CLASS TFastVentasArticulos
   ...
   if ( D():AlbaranesClientesLineas( ::nView ) )->( dbseek(  ::oDbf:cCodArt  ) )

      while ( D():AlbaranesClientesLineas( ::nView ) )->cRef ==  ::oDbf:cCodArt  .and. ;
            !( D():AlbaranesClientesLineas( ::nView ) )->( eof() )

         if ( ( D():AlbaranesClientesLineas( ::nView ) )->dFecAlb >= ::dIniInf .and.;
            ( D():AlbaranesClientesLineas( ::nView ) )->dFecAlb <= ::dFinInf ) .and.;
            !( D():AlbaranesClientesLineas( ::nView ) )->lFacturado
            nTotal   += nTotNAlbCli( D():AlbaranesClientesLineas( ::nView ) ) 
         end if
   ...
   if ( D():FacturasClientesLineas( ::nView ) )->( dbseek(  ::oDbf:cCodArt  ) )

      while ( D():FacturasClientesLineas( ::nView ) )->cRef ==  ::oDbf:cCodArt  .and. ;
            !( D():FacturasClientesLineas( ::nView ) )->( eof() )

         if ( D():FacturasClientesLineas( ::nView ) )->dFecFac >= ::dIniInf .and.;
            ( D():FacturasClientesLineas( ::nView ) )->dFecFac <= ::dFinInf
   ...
   if ( D():FacturasRectificativasLineas( ::nView ) )->( dbseek(  ::oDbf:cCodArt  ) )

      while ( D():FacturasRectificativasLineas( ::nView ) )->cRef ==  ::oDbf:cCodArt  .and. ;
            !( D():FacturasRectificativasLineas( ::nView ) )->( eof() )

         if ( D():FacturasRectificativasLineas( ::nView ) )->dFecFac >= ::dIniInf .and.;
            ( D():FacturasRectificativasLineas( ::nView ) )->dFecFac <= ::dFinInf
   ...
   if ( D():TiketsLineas( ::nView ) )->( dbseek(  ::oDbf:cCodArt  ) )

      while ( D():TiketsLineas( ::nView ) )->cCbaTil ==  ::oDbf:cCodArt  .and. ;
            !( D():TiketsLineas( ::nView ) )->( eof() )

         if ( D():TiketsLineas( ::nView ) )->dFecTik >= ::dIniInf .and.;
            ( D():TiketsLineas( ::nView ) )->dFecTik <= ::dFinInf
...
METHOD nTotalVentasArticulos() CLASS TFastVentasArticulos
   ...
   if ( D():AlbaranesClientesLineas( ::nView ) )->( dbseek( ::oDbf:cCodArt ) )
      ...
      while ( D():AlbaranesClientesLineas( ::nView ) )->cRef == ::oDbf:cCodArt .and. ;
         ...
         if ( ( D():AlbaranesClientesLineas( ::nView ) )->dFecAlb >= ::dIniInf .and.;
            ( D():AlbaranesClientesLineas( ::nView ) )->dFecAlb <= ::dFinInf ) .and.;
            !( D():AlbaranesClientesLineas( ::nView ) )->lFacturado
            nTotal   += nImpLAlbCli( ( D():AlbaranesClientes( ::nView ) ), ( D():AlbaranesClientesLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv, , , .t., .t. )
            nTotal   += nIvaLAlbCli( ( D():AlbaranesClientesLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv )
         end if
   ...
   if ( D():FacturasClientesLineas( ::nView ) )->( dbseek( ::oDbf:cCodArt ) )
      ...
      while ( D():FacturasClientesLineas( ::nView ) )->cRef == ::oDbf:cCodArt .and. ;
         ...
         if ( D():FacturasClientesLineas( ::nView ) )->dFecFac >= ::dIniInf .and.;
            ...
            nTotal    += nImpLFacCli( ( D():FacturasClientes( ::nView ) ), ( D():FacturasClientesLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv, , , .t., .t.  )
            nTotal    += nIvaLFacCli( ( D():FacturasClientesLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv )
   ...
   if ( D():FacturasRectificativasLineas( ::nView ) )->( dbseek( ::oDbf:cCodArt ) )
      ...
      while ( D():FacturasRectificativasLineas( ::nView ) )->cRef == ::oDbf:cCodArt .and. ;
         ...
         if ( D():FacturasRectificativasLineas( ::nView ) )->dFecFac >= ::dIniInf .and.;
            ...
            nTotal    += nImpLFacRec( ( D():FacturasRectificativas( ::nView ) ), ( D():FacturasRectificativasLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv, , , .t., .t.  )
            nTotal    += nIvaLFacRec( ( D():FacturasRectificativasLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv )
   ...
   if ( D():TiketsLineas( ::nView ) )->( dbseek( ::oDbf:cCodArt ) )
      ...
      while ( D():TiketsLineas( ::nView ) )->cCbaTil == ::oDbf:cCodArt .and. ;
         ...
         if ( D():TiketsLineas( ::nView ) )->dFecTik >= ::dIniInf .and.;
            ...
            nTotal      += nImpLTpv( ( D():Tikets( ::nView ) ), ( D():TiketsLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv, nil, 1 )
            nTotal      += nIvaLTpv( ( D():Tikets( ::nView ) ), ( D():TiketsLineas( ::nView ) ), ::nDecOut, ::nDerOut, ::nValDiv, 1 )
...
METHOD GetInformacionEntrada( cCodArt, cLote, cDatoRequerido ) 
   ...
   local cDato := ::GetDatoMovimientosAlamcen( cCodArt, cLote )

RETURN if ( !empty( cDato ), cDato, ::GetDatoAlbaranProveedor( cCodArt, cLote, cDatoRequerido ) )

//---------------------------------------------------------------------------//

METHOD GetDatoAlbaranProveedor( idArticulo, cLote, cCampoRequerido ) CLASS TFastVentasArticulos
...
METHOD GetDatoMovimientosAlamcen( cCodArt, cLote ) CLASS TFastVentasArticulos

   local nFieldPosition
   local Resultado            := ""

   //Resultado         := MovimientosAlmacenLineasRepository():getPrimeraFechaCompra( cCodArt, cLote )
...
Function nCostoFechaCompra( cCodArt, dFecha )

   local nCosto         := 0
   local hCostoAlbaran
   local hCostoFactura

   hCostoAlbaran        := AlbaranesProveedoresLineasModel():getCostoFecha( cCodArt, sTod( dFecha ) )
   hCostoFactura        := FacturasProveedoresLineasModel():getCostoFecha( cCodArt, sTod( dFecha ) )

   if hGet( hCostoFactura, "Fecha" ) >= hGet( hCostoAlbaran, "Fecha" )
      nCosto   := hGet( hCostoFactura, "Costo" )
   else
      nCosto   := hGet( hCostoAlbaran, "Costo" )
   end if
