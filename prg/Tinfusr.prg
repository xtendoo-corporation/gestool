...
METHOD AnuUsrFields()
   ...
   ::AddField ( "cCodUsr", "C",  4, 0, {|| "@!" },         "Cod. Usr.",       .t., "Codigo usuario",          4 )
   ::AddField ( "cNomUsr", "C", 50, 0, {|| "@!" },         "Usuario",         .t., "Usuario",                20 )
...
METHOD AcuCreate()
   ...
   ::AddField ( "cCodUsr", "C",  4, 0, {|| "@!" },           "Cod. Usr.",        .t., "Codigo usuario"     ,  4, .f. )
   ::AddField ( "cNomUsr", "C", 50, 0, {|| "@!" },           "Usuario",          .t., "Usuario"            , 20, .f. )
...
METHOD AddPre( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oPreCliT:cCodUsr )
      ...
      ::oDbf:nImpTrn    += nTrnUPreCli( ::oPreCliL:cAlias, ::nDecOut, ::nValDiv )
      ::oDbf:nPntVer    += nPntUPreCli( ::oPreCliL:cAlias, ::nDecOut, ::nValDiv )
      ::oDbf:nImpTot    += nImpLPreCli( ::oPreCliL:cAlias, ::nDecOut, ::nDerOut, ::nValDiv, , , .t., .t.  )
...
METHOD AddPed( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oPedCliT:cCodUsr )
...
METHOD AddAlb( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oAlbCliT:cCodUsr )
...
METHOD AddFac( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oFacCliT:cCodUsr )
...
METHOD AddFacRec( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oFacRecT:cCodUsr )
...
METHOD AddTik( cCodArt, nPrecio, lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oTikCliT:cCcjTik )
...
METHOD IncluyeCero()
   ...
   //Repaso de todos los usuarios------------------------------------------

   ::oDbfUsr:GoTop()
   while !::oDbfUsr:Eof()

      if ( ::lAllUsr .or. ( ::oDbfUsr:cCodUsr >= ::cUsrOrg .AND. ::oDbfUsr:cCodUsr <= ::cUsrDes ) ) .AND.;
         !::oDbf:Seek( ::oDbfUsr:cCodUse )

         ::oDbf:Append()
         ::oDbf:Blank()
         ::oDbf:cCodUsr    := ::oDbfUsr:cCodUse
         ::oDbf:cNomUsr    := ::oDbfUsr:cNbrUse
         ::oDbf:Save()
...
METHOD NewGroup( lDesPrp )
   ...
   if lDesPrp
      ::AddGroup( {|| ::oDbf:cCodUsr + ::oDbf:cCodArt + ::oDbf:cCodPr1 + ::oDbf:cCodPr2 + ::oDbf:cValPr1 + ::oDbf:cValPr2 + ::oDbf:cLote },;
