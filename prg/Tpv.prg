...
#define _MENUITEM_                  "terminal_punto_de_venta"
...
#define _LCOMENT                  87  
#define _LVALIDADO                88
#define _LDELETE                  89
#define _PARUUID                  90
#define _NPVPANT                  91     //   C      40     0
#define _CNUMALB                  92     //   C      40     0
...
STATIC FUNCTION OpenFiles( cPatEmp, lExt, lTactil )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPatDat() + "CajL.Dbf" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "CAJL", @dbfCajL ) )
      SET ADSINDEX TO ( cPatDat() + "CajL.Cdx" ) ADDITIVE
...
STATIC FUNCTION cOpenStatement()
   ...
   /*if lAIS() .and. !Auth():isSuperAdmin()

      cStatement        := "SELECT * FROM " + cPatEmp() + "TikeT WHERE cNcjTik='" + Application():CodigoCaja() + "'"
...
FUNCTION FrontTpv( oMenuItem, oWnd, cCodCli, cCodArt, lEntCon, lExtTpv, hDocument )
   ...
   */
   ...
   DEFINE SHELL oWndBrw FROM 0, 0 TO 22, 80 ;
      ...
      //DELETE   ( DeleteRegister( oWndBrw:oBrw, D():Tikets( nView ), {|| TpvDelRec() } ) );
      ...
      oWndBrw:oBrw:bClrStd      := {|| { CLR_BLACK, if( ( D():Tikets( nView ) )->lDelete, Rgb( 255, 0, 0 ), GetSysColor( COLOR_WINDOW ) ) } }
   ...
   /*DEFINE BTNSHELL RESOURCE "UNDO_" OF oWndBrw ;
      NOBORDER ;
      ACTION   ( RecuperarTicket() );
      TOOLTIP  "Recuperar";
      FROM     oDel ;
      CLOSED ;
      LEVEL    ACC_DELE*/
...
if !lExtTpv
      ...
      DEFINE BTNSHELL RESOURCE "SCROLL_DELETE_" OF oWndBrw ;
         NOBORDER ;
   ...
   DEFINE BTNSHELL oImp RESOURCE "IMP" GROUP OF oWndBrw ;
      ...
      lGenTikCli( oWndBrw:oBrw, oImp, IS_PRINTER ) ;
...
if lExtTpv
   ...
   DEFINE BTNSHELL oPrv RESOURCE "PREV1" GROUP OF oWndBrw ;
      NOBORDER ;
      ...
      lGenTikCli( oWndBrw:oBrw, oPrv, IS_SCREEN ) ;
   ...
   DEFINE BTNSHELL RESOURCE "BMPCONTA" OF oWndBrw ;
      NOBORDER ;
      ACTION   ( DlgCntTicket( oWndBrw, oNewImp, nView ) );
   ...
   DEFINE BTNSHELL RESOURCE "gc_spy_" GROUP OF oWndBrw ;
      NOBORDER ;
      ACTION   ( InformeAuditoria( ( D():Tikets( nView ) )->uuid, TIK_CLI ) ) ;
      TOOLTIP  "Auditoría"
   end if
...
FUNCTION TpvDelRec()
   ...
   */
   ...
   /*
   Volvemos los albaranes a no facturados-----------------------------------
   */

   if Len( aDelAlb ) > 0
      aEval( aDelAlb, {|cAlbaran| AlbaranesClientesModel():UpdateFacturado( cAlbaran, 1 ),; 
                                  AlbaranesClientesLineasModel():UpdateAllFacturado( cAlbaran, .f. ) } )
   end if
   ...
   */
   ...
   // Anotamos la acción en la auditoría---------------------------------------

   AuditoriaModel():addRegister( ( D():Tikets( nView ) )->uuid,;
   ...
   */
   ...
   DEFINE DIALOG oDlgTpv RESOURCE cResource TITLE cTitDoc
      ...
      */
      ...
      REDEFINE GET aGet[ _CREFTIK ] VAR aTmp[ _CREFTIK ];
         ID       300 ;
         OF       oDlgTpv
      ...
      REDEFINE GET aGet[ _CCCJTIK ] VAR aTmp[ _CCCJTIK ];
         ID       125 ;
         WHEN     ( .f. ) ;
         VALID    ( oGetTxt[ 10 ]:cText( UsuariosModel():getNombreWhereCodigo( aTmp[ _CCCJTIK ] ) ), .t.  ) ;
         OF       oDlgTpv
...
Static Function deleteLineTicket( aTmp, oBrwDet )
   ...
   oAuditoria:AddTemporal( { "parent_uuid" => aTmp[ _UUID ] ,;
                             "description" => "Linea eliminada: " + AllTrim( Trans( ( dbfTmpL )->nUntTil, MasUnd() ) ) + ;
                                              " unidades de " + AllTrim( ( dbfTmpL )->cCbaTil ) + " - " + AllTrim( ( dbfTmpL )->cNomTil ) + ;
                                              " con precio:  " + AllTrim( Trans( ( dbfTmpL )->nPvpTil, cPorDiv ) ) ,;
...
Static Function StartEdtRec( aTmp, aGet, nMode, oDlgTpv, oBrw, oBrwDet, hDocument, cCodArt )
   ...
   if empty( oOfficeBar )
      ...
      oGrupo            := TDotNetGroup():New( oCarpeta, 66, "Albaranes", .f. )
         oBtnAssDev     := TDotNetButton():New( 60, oGrupo, "gc_documents_empty_user_32", "Agrupar albaranes",    1, {|| AgruparAlbaranes( aTmp, aGet, oBrwDet ), oBrwDet:Refresh() }, , {|| nMode == APPD_MODE }, .f., .f., .f. )
      ...
      if Auth():isSuperAdmin()
         ...
         oGrupo               := TDotNetGroup():New( oCarpeta, 66,  "Auditoría", .f. )
            oBoton            := TDotNetButton():New( 60, oGrupo,    "gc_spy_32",            "Informe",           1, {|| InformeAuditoria( aTmp[ _UUID ], TIK_CLI ) }, , , .f., .f., .f. )
      ...
      */
      ...
      BEGIN SEQUENCE
         ...
         */
         ...
         // Antes de guardar, si venimos de agrupar albaranes le cambiamos el estado

         if hb_isArray( aAlbaranes ) .and. Len( aAlbaranes ) > 0

            aEval( aAlbaranes, {|hAlbaran| if( hGet( hAlbaran, "LSNDDOC" ), ( AlbaranesClientesModel():UpdateFacturado( hGet( hAlbaran, "CSERALB" ) + Str( hGet( hAlbaran, "NNUMALB" ) ) + hGet( hAlbaran, "CSUFALB" ), 3 ),; 
                                                                              AlbaranesClientesLineasModel():UpdateAllFacturado( hGet( hAlbaran, "CSERALB" ) + Str( hGet( hAlbaran, "NNUMALB" ) ) + hGet( hAlbaran, "CSUFALB" ), .t. ) ), ) } )
...
Static Function EdtCobTik( oWndBrw, lBig )
   ...
   */
   ...
   case ( D():Tikets( nView ) )->cTipTik == SAVFAC // Como factura
      ...
      aTmp[ _NCOBTIK ]  := nTotFacCli( cNumDoc, dbfFacCliT, dbfFacCliL, dbfIva, dbfDiv, dbfFacCliP, nil, nil, .f. )
...
Static function BeginTrans( aTmp, aGet, nMode, lNewFile )
   ...
   */
   ...
   if !empty( aGetTxt )
      ...
      aGetTxt[ 10]      := UsuariosModel():getNombreWhereCodigo( aTmp[ _CCCJTIK ] )
   end if
...
Static Function BeginTrans( aTmp, aGet, nMode )
   ...
   */
   ...
   /*
   Inicializamos el Temporal de auditoria--------------------------------------
   */
...
Static Function SetDlgMode( oDlg, aTmp, aGet, nMode, oBrw, oBtn, oBtnCancel ) // , nTop, nLeft, nHeight, nWidth )
   ...
   if empty( aTmp[ _NPVPTIL ] ) .or. oUser():lAdministrador() .or. ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) )
...
STATIC FUNCTION SavLine( aTmp, aGet, dbfTmpL, oBrw, aTik, oGetTotal, lTwo, nMode, oBtn, cPorDiv, cPicEur, lMsgVta, lNotVta, cCodArt, oStock )
   ...
   */
   ...
   if lNotVta .or. lMsgVta
      ...
      nStockActual   := StocksModel():nStockArticulo( aTmp[ _CCBATIL ], aTik[ _CALMTIK ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aTmp[ _CVALPR1 ], aTmp[ _CVALPR2 ], aTmp[ _CLOTE ] )
   ...
   */
   ...
   if nMode == APPD_MODE
      ...
      if aXbYStr[ 1 ] != 0 .and. aXbYStr[ 2 ] != 0
         ...
         */
         ...
         if lIsCode( aTmp, dbfTmpL, oBrw )
            ...
            /*
            Guardamos en la auditoría------------------------------------------------
            */

            oAuditoria:AddTemporal( {  "parent_uuid" => aTik[ _UUID ] ,;
                                             "description" => "Linea añadida: " + AllTrim( Trans( aTmp[ _NUNTTIL ], MasUnd() ) ) + ;
                                                              " unidades de " + AllTrim( aTmp[ _CCBATIL ] ) + " - " + AllTrim( aTmp[ _CNOMTIL ] ) + ;
                                                              " con precio:  " + AllTrim( Trans( aTmp[ _NPVPTIL ], cPorDiv ) ) ,;
                                             "tipodocumento" => TIK_CLI } )
      ...
      */
      ...
      oAuditoria:AddTemporal( {  "parent_uuid" => aTik[ _UUID ] ,;
                                 "description" => "Linea modificada: " + AllTrim( Trans( aTmp[ _NUNTTIL ], MasUnd() ) ) + ;
                                                  " unidades de " + AllTrim( aTmp[ _CCBATIL ] ) + " - " + AllTrim( aTmp[ _CNOMTIL ] ) + ;
                                                  " con precio:  " + AllTrim( Trans( aTmp[ _NPVPTIL ], cPorDiv ) ) + ; 
                                                  " con unidades anteriores:  " + AllTrim( Trans( aTmp[ _NUNDANT ], MasUnd() ) ) + ; 
                                                  " con precio anterior:  " + AllTrim( Trans( aTmp[ _NPVPANT ], cPorDiv ) ) ,;
                                 "tipodocumento" => TIK_CLI } )
   ...
   */
   ...
   if aSeekProp( @cCodArt, @cValPr1, @cValPr2, dbfArticulo, dbfTblPro )
         ...
         */
         ...
         if !empty( hAtipica )
            ...
            if hhaskey( hAtipica, "nTarifaFamilia" ) .and. hAtipica[ "nTarifaFamilia" ] > 0
               ...
               aGet[ _NPVPTIL ]:cText( nRetPreArt( hAtipica[ "nTarifaFamilia" ], aTik[ _CDIVTIK ], if( aTik[ _NREGIVA ] <= 1, .t., .f. ), dbfArticulo, dbfDiv, dbfKit, dbfIva, .t. ) )
      ...
      */
      ...
      if empty( aTmp[ _NPVPTIL ] ) .or. ( RolesModel():getRolCambiarPrecios( Auth():rolUuid() ) )
         if( !empty( aGet[ _NPVPTIL ] ), aGet[ _NPVPTIL ]:HardEnable(), )
         if( !empty( aGet[ _NDTODIV ] ), aGet[ _NDTODIV ]:HardEnable(), )
         if( !empty( aGet[ _NDTOLIN ] ), aGet[ _NDTOLIN ]:HardEnable(), )
      else
...
Static Function SavTik2Tik( aTmp, aGet, nMode, nSave, nNumDev )
   ...
   */
   ...
   while !( dbfTmpL )->( eof() )
      ...
      if ( dbfTmpL )->ParUuid != aTmp[ _UUID ]
         ( dbfTmpL )->ParUuid := aTmp[ _UUID ]
      end if
   ...
   */
   ...
   while !( dbfTmpP )->( eof() )
      ...
      if ( dbfTmpP )->ParUuid != aTmp[ _UUID ]
         ( dbfTmpP )->ParUuid := aTmp[ _UUID ]
      end if
   ...
   */
   ...
   do case
      case nMode == APPD_MODE
         ...
         AuditoriaModel():addRegister( aTmp[ _UUID ],;
                                       "Simplificada añadida " + aTmp[ _CSERTIK ] + "/" + AllTrim( aTmp[ _CNUMTIK ] ) + "/" + aTmp[ _CSUFTIK ],;
                                       TIK_CLI )
...
Static Function nChkPagTik( cNumTik, cTikT, dbfTikL, dbfTikP, dbfIva, dbfDiv )
   ...
   if !( D():Tikets( nView ) )->lPgdTik
      ...
      nCob        := nTotCobTik( cNumTik, dbfTikP, dbfDiv )          //( D():Tikets( nView ) )->nCobTik 
...
Static Function GetPesoBalanza( aGet, oBtn )
   ...
   if oBal:OpenPort()
      ...
      aGet[ _NUNTTIL ]:cText( oBal:GetPeso( aGet[ _NPVPTIL ]:VarGet() ) )
...
Static Function AgruparAlbaranes( aTmp, aGet, oBrwDet )
   ...
   aAlbaranes           := AlbaranesClientesModel():ListNoFacturados( aTmp[_CCLITIK], aTmp[_CCODOBR] )
   ...
   DEFINE DIALOG  oDlg ;
      RESOURCE    "SET_ALBARAN_SIMPLIFICADA" ;
      ...
      REDEFINE SAY oTitle1 PROMPT rTrim( aTmp[_CNOMTIK] );
         ID       501 ;
         OF       oDlg
      ...
      */
      ...
      REDEFINE BUTTON ;
         ID       516 ;
         OF       oDlg ;
         ACTION   (  aEval( aAlbaranes, {|h| hSet( h, "LSNDDOC", .t. ) } ), oBrwDet:Refresh() )

      REDEFINE BUTTON ;
         ID       517 ;
         OF       oDlg ;
         ACTION   (  aEval( aAlbaranes, {|h| hSet( h, "LSNDDOC", .f. ) } ), oBrwDet:Refresh() )

      REDEFINE BUTTON ;
         ID       518 ;
         OF       oDlg ;
         ACTION   ( ZooAlbCli( hGet( oBrwDet:aArrayData[ oBrwDet:nArrayAt ], "CSERALB" ) + str( hGet( oBrwDet:aArrayData[ oBrwDet:nArrayAt ], "NNUMALB" ) ) + hGet( oBrwDet:aArrayData[ oBrwDet:nArrayAt ], "CSUFALB" ) ) )
...
Static Function insertAlbaran( hAlbaran, aTmp )
   ...
   /*
   Inserto linea de Cabecera de albarán---------------------------------------
   */
   ...
   if lNumAlb() .or. lNumObr() .or. lSuAlb()
      ...
      ( dbfTmpL )->cTipTil       := SAVTIK
      ( dbfTmpL )->cCbaTil       := Space( 18 )
      ( dbfTmpL )->cNomTil       := cDesAlb
      ( dbfTmpL )->cAlmLin       := aTmp[ _CALMTIK ]
      ( dbfTmpL )->lImpLin       := .t.
      ( dbfTmpL )->lControl      := .t.
...
Function nombreUsuarioTicket( dbfTmpT )
   ...
   DEFAULT dbfTmpT   := D():Tikets( nView )
