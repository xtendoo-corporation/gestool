...
CLASS TpvCobros  
   ...
   CLASSDATA aBigResourceFormaPago  INIT aBigResourceFormaPago()
   ...
   DATA aBtnFPago                   INIT Array(4)

   DATA aTicketsPendientes

   DATA aTicketsLiquidados

   DATA oBrwPendientes

   DATA nUltimoCambio

   DATA cCodigoClienteSeleccionado
   ...
   */
   ...
   METHOD nSenderView()                INLINE ( ::oSender:nView )

   METHOD CheckVentaCredito( nEstado )

   METHOD LiquidacionCliente()

   METHOD loadHashTicketsPendientes()

   METHOD LiquidaRecibos()

   METHOD LiquidaTicket( hTicket, nImporte )

   METHOD ImprimirTicketsLiquidados()

   METHOD UltimoCambio()

   METHOD LoadBtnFPago()

   METHOD FormaPagoSeleccionada()
   ...
   */
   ...
   DEFINE DIALOG ::oDlg ;
      ...
      */
      ...
      with object ( ::oBrwFormasPago:AddCol() )
         :cHeader                      := "Forma"
         :bEditValue                   := {|| Capitalize( ::oBrwFormasPago:aRow[ "Descripcion" ] ) }
         :nWidth                       := 90
      end with
...
METHOD OnClickAceptar()
   ...
   if ::CreaCobro( ::nGetEntregado )
      ...
      if ( ::oSender:nTipoDocumento != documentoAlbaran ) .and. ( ::Total() != 0 )

         do case
            case ::Entregado() == 0

               lEnd  := ::CheckVentaCredito( estadoPendiente )
            ...
            case ::Entregado() < ::Total()
               ...
               lEnd  := ::CheckVentaCredito( estadoParcial )
...
METHOD CheckVentaCredito( nEstado ) CLASS TpvCobros
...
METHOD LoadBtnFPago() CLASS TpvCobros
...
METHOD OnClickFormadePago( oBoton ) CLASS TpvCobros
   ...
   aEval( ::aBtnFPago, {| o | o:lPressed  := .f., o:SetColor( CLR_BLACK, Rgb( 240, 240, 240 ) ) } )
   
   oBoton:SetColor( CLR_BLACK, Rgb( 255, 205, 67 ) )
   oBoton:lPressed      := .t.
...
METHOD FormaPagoSeleccionada() CLASS TpvCobros
...
METHOD LiquidacionCliente( cCodCli, cNomcli ) CLASS TpvCobros
   ...
   DEFINE DIALOG ::oDlgLiq ;
      RESOURCE       ( "LIQCLIENTE" ) ;
      TITLE          ( ::oSender:cTipoDocumento() );
      FONT           ( ::oSender:oFntDlg )

   REDEFINE BITMAP   oBmpTitle ;
      ID             500 ;
      RESOURCE       "hand_money_64" ;
      TRANSPARENT ;
      OF             ::oDlgLiq

   REDEFINE SAY      oTittle ;
      VAR            cTittle ;
      ID             510 ;
      OF             ::oDlgLiq
   ...
   ::oBrwPendientes:bClrSel         := {|| { CLR_BLACK, Rgb( 229, 229, 229 ) } }
   ::oBrwPendientes:bClrSelFocus    := {|| { CLR_BLACK, Rgb( 167, 205, 240 ) } }
   ...
   */
   ...
   TButtonBmp():ReDefine( IDOK, {|| ::LiquidaRecibos() }, ::oDlgLiq,,, .f.,,,, .f., "gc_check_32" )

   TButtonBmp():ReDefine( IDCANCEL, {|| ::oDlgLiq:End( IDCANCEL ) }, ::oDlgLiq,,, .f.,,,, .f., "gc_door_open2_32" )

   ::oDlgLiq:bStart  := {|| ::oBrwPendientes:MakeTotals() }

   ACTIVATE DIALOG ::oDlgLiq CENTER
...
METHOD loadHashTicketsPendientes() CLASS TpvCobros
   ...
   while !( cConsulta )->( Eof() )
      ...
      aAdd( ::aTicketsPendientes,;
            {  "cSerTik" => ( cConsulta )->cSerTik ,;
               "cNumTik" => ( cConsulta )->cNumTik ,;
               "cSufTik" => ( cConsulta )->cSufTik ,;
               "numero" => ( cConsulta )->cSerTik + ( cConsulta )->cNumTik + ( cConsulta )->cSufTik ,;
               "numeroformato" => ( cConsulta )->cSerTik + "/" + AllTrim( ( cConsulta )->cNumTik ) + "/" + ( cConsulta )->cSufTik ,;
               "fecha" => ( cConsulta )->dFecTik ,;
               "importe" => ( cConsulta )->nTotTik ,;
               "pendiente" => ( ( cConsulta )->nTotTik - nTotCobTik( ( cConsulta )->cSerTik + ( cConsulta )->cNumTik + ( cConsulta )->cSufTik, ::oSender:oTiketCobro:cAlias, ::oSender:oDivisas:cAlias ) ) } )
...
METHOD LiquidaRecibos() CLASS TpvCobros
...
METHOD LiquidaTicket( hTicket, nImporte ) CLASS TpvCobros
...
METHOD ImprimirTicketsLiquidados() CLASS TpvCobros
...
METHOD UltimoCambio( nImporte ) CLASS TpvCobros
