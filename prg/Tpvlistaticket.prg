...
METHOD lResource() CLASS TpvListaTicket
   ...
   DEFINE DIALOG ::oDlg RESOURCE ( ::cResource ) TITLE ( "Lista de tickets: Tickets abiertos" )
      ...
      ::oBrwListaTicket:bClrStd           := {|| { CLR_BLACK, if( ::oSender:oTiketCabecera:lDelete, Rgb( 255, 0, 0 ), GetSysColor( COLOR_WINDOW ) ) } }
      ...
      with object ( ::oBrwListaTicket:AddCol() )
         :cHeader          := "Ubicación"
         :bEditValue       := {|| ::oSender:cUbicacion() + CRLF + if( ::oSender:oTiketCabecera:lDelete, "Eliminado", "" ) }
         :nWidth           := 190
      end with
      ...
      */
      ...
      if Auth():isSuperAdmin()
         ...
         oGrupo               := TDotNetGroup():New( oCarpeta, 66,  "Auditoría", .f. )
            oBoton            := TDotNetButton():New( 60, oGrupo,    "gc_spy_32",                  "Informe",            1, {|| InformeAuditoria( ::oSender:oTiketCabecera:uuid, TIK_CLI ) }, , , .f., .f., .f. )
