...
CLASS TTpvRestaurante FROM TMasDet
   ...
   DATA oDetImpComandas    AS OBJECT
...
METHOD DefineFiles( cPath, cDriver ) CLASS TTpvRestaurante
   ...
   DEFINE TABLE ::oDbf FILE "SalaVta.Dbf" CLASS "SalaVta" ALIAS "SalaVta" PATH ( cPath ) VIA ( cDriver ) COMMENT "Sala de ventas"
      ...
      FIELD NAME "Uuid"          TYPE "C" LEN 40  DEC 0 COMMENT "Identificador únido"                                   OF ::oDbf
   ...
   DEFINE DIALOG oDlg RESOURCE "SalaVentas" TITLE LblTitle( nMode ) + "sala de ventas"
      ...
      */
      ...
      REDEFINE BUTTON ;
         ...
         OF       oDlg ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( ::oDetImpComandas:App() )

      REDEFINE BUTTON ;
         ID       501 ;
         OF       oDlg ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( ::oDetImpComandas:Edt() )

      REDEFINE BUTTON ;
         ID       502 ;
         OF       oDlg ;
         WHEN     ( nMode != ZOOM_MODE ) ;
         ACTION   ( ::oDetImpComandas:Del() )
      ...
      */
      ...
      ::oBrwImpComandas:bClrSel            := {|| { CLR_BLACK, Rgb( 229, 229, 229 ) } }
      ::oBrwImpComandas:bClrSelFocus       := {|| { CLR_BLACK, Rgb( 167, 205, 240 ) } }
...
CLASS TDetImpComandas
   ...
   METHOD App()            INLINE ( WinAppRec( ::oSender:oBrwImpComandas, ::bEdtDet ) )
   METHOD Edt()            INLINE ( WinEdtRec( ::oSender:oBrwImpComandas, ::bEdtDet ) )
   METHOD Del()            INLINE ( DbDelRec( ::oSender:oBrwImpComandas ) )

   METHOD EndResource( aTmp, aGet, nMode )

END CLASS

//--------------------------------------------------------------------------//

METHOD New( oSender ) CLASS TDetImpComandas
...
METHOD OpenFiles() CLASS TDetImpComandas
...
METHOD CloseFiles() CLASS TDetImpComandas
...
METHOD Begintrans() CLASS TDetImpComandas
...
METHOD KillTrans() CLASS TDetImpComandas
...
METHOD Resource( aTmp, aGet, dbfTmpLin, oBrwImpComandas, bWhen, bValid, nMode ) CLASS TDetImpComandas
   ...
   DEFINE DIALOG ::oDlg RESOURCE "lCajas" TITLE LblTitle( nMode ) + "impresoras de comandas"
      ...
      */
      ...
      REDEFINE GET aGet[ ( dbfTmpLin )->( FieldPos( "cPrnCom" ) ) ] ;
         ...
         ID       140 ;
         IDTEXT   141 ;
         VALID    ( cDocumento( aGet[ ( dbfTmpLin )->( FieldPos( "cPrnCom" ) ) ], aGet[ ( dbfTmpLin )->( FieldPos( "cPrnCom" ) ) ]:oHelpText, D():Documentos( ::nView ) ) );
         BITMAP   "LUPA" ;
      ...
      */      
      ...
      REDEFINE GET aGet[ ( dbfTmpLin )->( FieldPos( "cPrnAnu" ) ) ] ;
         ...
         ID       150 ;
         IDTEXT   151 ;
         VALID    ( cDocumento( aGet[ ( dbfTmpLin )->( FieldPos( "cPrnAnu" ) ) ], aGet[ ( dbfTmpLin )->( FieldPos( "cPrnAnu" ) ) ]:oHelpText, D():Documentos( ::nView ) ) );
         BITMAP   "LUPA" ;
      ...
      REDEFINE BUTTON ;
         ID       500 ;
         OF       ::oDlg ;
         ACTION   ( ::EndResource( aTmp, aGet, dbfTmpLin, nMode ) )
...
METHOD EndResource( aTmp, aGet, dbfTmpLin, nMode ) CLASS TDetImpComandas
...
FUNCTION SynSalaVenta( cPath )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPath + "SalaVta.Dbf" ) NEW VIA ( cDriver() ) EXCLUSIVE ALIAS ( cCheckArea( "SalaVta", @dbfSalaVta ) )
      SET ADSINDEX TO ( cPath + "SalaVta.Cdx" ) ADDITIVE
...
CLASS SalaVentaModel FROM ADSBaseModel

   METHOD getTableName()                     INLINE ::getEmpresaTableName( "SalaVta" )

   METHOD getUuidFromCodigo( cCodigo )       INLINE ( ::getField( "Uuid", "cCodigo", cCodigo ) ) 
