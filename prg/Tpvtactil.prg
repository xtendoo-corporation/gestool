...
CLASS TpvTactil
   ...
   DATA oGetCodigoBarras
   DATA cGetCodigoBarras
   ...
   DATA cDescripcionLineaComentario
   DATA nUnidadesLineaComentario
   DATA nImporteLineaComentario
   DATA nIvaLineaComentario
   ...
   METHOD KeyChar( cKey )
   METHOD KeyCharCodigosBarras( cKey )
   METHOD oDlgKeyDown( o, nKey, nFlag )
   METHOD oDlgKeyDownCodigosBarras( o, nKey, nFlag )
   ...
   METHOD AgregarLineas()
      ...
      METHOD AgregarLineaComentario()
         METHOD ValidarAgregarLineaComentario( oGetDescripcion, oDlg )
         METHOD GuardarAgregarLineaComentario()
   ...
   */
   ...
   METHOD nDtoLTpv()                   INLINE ( nDtoLTpv( ::oTiketLinea ) )
   ...
   METHOD GetNombreUsuario()           INLINE ( UsuariosModel():getNombreWhereCodigo( ::oTiketCabecera:cCcjTik ) )
   ...
   METHOD OpenCajon()               	INLINE ( Application():openDirectCajon() )
   METHOD OpenCajonDirect()            INLINE ( Application():openDirectCajon() )
   ...
   */
   ...
   METHOD startActivateBrwBig()

   METHOD SelBrwBigUser( nOpt, oDlg )

   METHOD LiquidacionCliente()

   METHOD InicializaDocumento()
...
METHOD New( oMenuItem, oWnd ) CLASS TpvTactil
   ...
   DEFAULT oMenuItem          := "terminal_punto_de_venta_tactil"
   DEFAULT oWnd               := oWnd()
...
METHOD OpenFiles() CLASS TpvTactil
   ...
   DATABASE NEW ::oCajaLinea                                PATH ( cPatDat() )   FILE "CajL.DBF"            VIA ( cDriver() ) SHARED INDEX "CAJL.CDX"
...
METHOD Resource() CLASS TpvTactil
   ...
   */
   ...
   DEFINE DIALOG ::oDlg RESOURCE ( ::cResource ) TITLE "TPV TACTIL ( Usuario: " + AllTrim( Auth():Nombre() ) + " )"
   ...
   */
   ...
   if ::l1024()
      ...
      REDEFINE GET ::oGetCodigoBarras VAR ::cGetCodigoBarras;
         ID       900 ;
         WHEN     .f. ;
         FONT     ::oFntEur ;
         OF       ::oDlg
    
      ::oGetCodigoBarras:OnKeyDown   := {|| 0 } // {| o, nKey, nFlag | ::oDlgKeyDown( o, nKey, nFlag ) }
   ...
   */
   ...
   ::oDlg:OnKeyDown           := {| o, nKey, nFlag | if( !uFieldEmpresa( "lCBarTpv" ), ::oDlgKeyDown( o, nKey, nFlag ), ::oDlgKeyDownCodigosBarras( o, nKey, nFlag ) ) } // {|| msgStop( "OnKeyDown" ), ::oGetUnidades:SetFocus(), 1 }
   ...
   if ::l1024()
      ...
      if Empty( ::oOfficeBar )
         ...
         */
         ...
         if !RolesModel():getRolNoMostrarCobrosRapidos( Auth():rolUuid() )
            ...
            oGrupo                     := TDotNetGroup():New( oCarpeta, 66, "Rápidos", .f., , "gc_cash_register_32" )
               ::oBtnImportesExactos   := TDotNetButton():New( 60, oGrupo, "gc_gauge_32",                   "Cobros rápidos",    1, {|| ::OnClickImportesExactos() }, , , .f., .f., .f. )
         ...
         oGrupo                  := TDotNetGroup():New( oCarpeta, 66, "Comentario", .f., , "gc_user_message_32" )
            TDotNetButton():New( 60, oGrupo, "gc_user_message_32", "Línea comentario", 1, {|| ::AgregarLineaComentario() }, , , .f., .f., .f. )         
         ...
         oGrupo                     := TDotNetGroup():New( oCarpeta, 66, "Liquidación", .f., , "hand_money_32" )
            TDotNetButton():New( 60, oGrupo, "hand_money_32",     "Liquidación cliente",                             1, {|| ::LiquidacionCliente() }, , , .f., .f., .f. )
      ...
      */
      ...
      if Empty( ::oOfficeBar )
         ...
         oCarpeta                := TCarpeta():New( ::oOfficeBar, "táctil" )

         /*
         Hacemos un case para contemplar todas pas opciones que existen con las opciones de llevar, recoger y encargo
         */

         /*::oGrpSalones           := TDotNetGroup():New( oCarpeta, 106, "", .f., , "gc_cup_32" )
            ::oBtnSala           := TDotNetButton():New( 50, ::oGrpSalones, "gc_cup_32",              "",               1, {|| MsgInfo( "Boton 1" ) }, , , .f., .f., .f. )
            ::oBtnGeneral        := TDotNetButton():New( 50, ::oGrpSalones, "gc_cash_register_32",    "",               2, {|| MsgInfo( "Boton 2" ) }, , , .f., .f., .f. )*/
...
METHOD CambiarPrecio()
   ...
   if ( ::valUnidadesPrecios() != 0 )
      ...
      if ( ::oTemporalLinea:nPvpTil == 0 ) .or. ( ApoloMsgNoYes( "¿Desea cambiar el precio del artículo seleccionado?", "Confirme", .t. ) )
         ...
         ::oAuditoria:AddTemporal( {  "parent_uuid" =>  ::oTiketCabecera:uuid ,;
                                    "description" =>  "Linea modificada: " + AllTrim( ::oTemporalLinea:cCbaTil ) + " - " + AllTrim( ::oTemporalLinea:cNomTil ) + ;
                                                      " con nuevo precio:  " + AllTrim( Trans( nPrecioNuevo, ::cPictureTotal ) ) + ;
                                                      " con anterior precio:  " + AllTrim( Trans( nPrecioAnterior, ::cPictureTotal ) ),;
                                    "tipodocumento" => TIK_CLI } )
...
METHOD SetDocumentoPagado()
   ...
   do case
      case ::oTpvCobros:nEstado == nPendiente
         ::setTicketPendiente()
...
METHOD OnClickIniciarSesion()
   ...
   /*
   Comprobamos que tenga permiso para ello-------------------------------------
   */
...
METHOD AgregarLineaComentario()
   ...
   DEFINE DIALOG oDlg RESOURCE "LineaComentario" TITLE "Línea de comentario"

      REDEFINE GET oDescripcionLineaComentario ;
         VAR      ::cDescripcionLineaComentario ;
         ID       100 ;
         OF       oDlg
      ...
      REDEFINE BUTTONBMP ;
         ...
         OF       oDlg ;
         BITMAP   "gc_keyboard_32" ;
         ACTION   ( VirtualKey( .f., oDescripcionLineaComentario ) )

      REDEFINE GET oIvaLineaComentario ;
         VAR      ::nIvaLineaComentario ; 
         PICTURE  "@E 999.99" ;
         WHEN     ( .f. );
         ID       200 ;
         OF       oDlg
      ...
      REDEFINE BUTTONBMP ; 
         ...
         OF       oDlg ;
         BITMAP   "Lupa_32" ;
         ACTION   ( BigBrwIva( oIvaLineaComentario, ::oTipoIVA:cAlias ) )

      REDEFINE GET oUnidadesLineaComentario ;
         VAR      ::nUnidadesLineaComentario ;
         PICTURE  ::cPictureUnidades ;
         ID       120 ;
         OF       oDlg
      ...
      REDEFINE BUTTONBMP ; 
         ...
         OF       oDlg ;
         BITMAP   "gc_calculator_32" ;
         ACTION   ( Calculadora( 0, oUnidadesLineaComentario ) ) 

      REDEFINE GET oImporteLineaComentario ;
         VAR      ::nImporteLineaComentario ;
         PICTURE  ::cPictureTotal ;
         ID       140 ;
         OF       oDlg
      ...
      REDEFINE BUTTONBMP ;
         ...
         OF       oDlg ; 
         BITMAP   "gc_calculator_32" ;
         ACTION   ( Calculadora( 0, oImporteLineaComentario ) )

      REDEFINE BUTTONBMP ;
         BITMAP   "gc_check_32" ;
         ID       IDOK ;
         OF       oDlg ;
         ACTION   ( ::ValidarAgregarLineaComentario( oDescripcionLineaComentario, oUnidadesLineaComentario, oDlg ) )
...
METHOD ValidarAgregarLineaComentario( oDescripcionLineaComentario, oUnidadesLineaComentario, oDlg ) CLASS TpvTactil
...
METHOD GuardarAgregarLineaComentario() CLASS TpvTactil
...
METHOD KeyCharCodigosBarras( cKey ) CLASS TpvTactil
...
METHOD oDlgKeyDownCodigosBarras( o, nKey, nFlag )
   ...
   do case
      ...
      case ( nKey == 48 )  // Teclado numérico 0
         ::KeyCharCodigosBarras( Chr( 48 ) )

      case ( nKey == 49 )  // Teclado numérico 1
         ::KeyCharCodigosBarras( Chr( 49 ) )

      case ( nKey == 50 )  // Teclado numérico 2
         ::KeyCharCodigosBarras( Chr( 50 ) )

      case ( nKey == 51 )  // Teclado numérico 3
         ::KeyCharCodigosBarras( Chr( 51 ) )

      case ( nKey == 52 ) // Teclado numérico 4
         ::KeyCharCodigosBarras( Chr( 52 ) )

      case ( nKey == 53 ) // Teclado numérico 5
         ::KeyCharCodigosBarras( Chr( 53 ) )

      case ( nKey == 54 ) // Teclado numérico 6
         ::KeyCharCodigosBarras( Chr( 54 ) )

      case ( nKey == 55 ) // Teclado numérico 7
         ::KeyCharCodigosBarras( Chr( 55 ) )

      case ( nKey == 56 ) // Teclado numérico 8
         ::KeyCharCodigosBarras( Chr( 56 ) )

      case ( nKey == 57 ) // Teclado numérico 9
...
METHOD AgregarLineas( cCodigoArticulo, cCodigoMenu, cCodigoOrden ) CLASS TpvTactil
   ...
   /*
   Comprobamos que no esté entregado el ticket---------------------------------
   */
      ...
      if ::oTemporalLinea:Seek( Str( nNumeroLinea ) )
         ...
         while ( ::oTemporalLinea:nNumLin == nNumeroLinea ) .and. !( ::oTemporalLinea:eof() )
            ...
            if !::oTemporalLinea:lKitChl
               ...
               ::oAuditoria:AddTemporal( {  "parent_uuid" => ::oTiketCabecera:uuid ,;
                                          "description" => "Linea modificada: " + AllTrim( Trans( ::oTemporalLinea:nUntTil, ::cPictureUnidades ) ) + ;
                                                  " unidades de " + AllTrim( ::oTemporalLinea:cCbaTil ) + " - " + AllTrim( ::oTemporalLinea:cNomTil ) + ;
                                                  " con unidades anteriores:  " + AllTrim( Trans( nUnidadesAnterior, ::cPictureUnidades ) ),;
                                          "tipodocumento" => TIK_CLI } )
...
METHOD OnClickUsuarios() CLASS TpvTactil
   ...
   RECOVER USING oError
      ...
      msgStop( "Error al montar la salas de venta1" + CRLF + ErrorMessage( oError ) )
...
METHOD OnClickCobro() CLASS TpvTactil
   ...
   if ::oTpvCobros:lCobro()
      ...
      ::oAuditoria:AddTemporal( {   "parent_uuid" =>  ::oTiketCabecera:uuid ,;
                                    "description" =>  "Simplificada cobrada",;
                                    "tipodocumento" => TIK_CLI } )
...
METHOD OnClickEntregaNota() CLASS TpvTactil
   ...
   ::oAuditoria:AddTemporal( {   "parent_uuid" =>  ::oTiketCabecera:uuid ,;
                                 "description" =>  "Simplificada entregada",;
                                 "tipodocumento" => TIK_CLI } )
...
METHOD GuardaDocumento( lZap ) CLASS TpvTactil
   ...
   BEGIN SEQUENCE
      ...
      if ::lBlankTicket()
         ...
         //Anotamos en la auditoría el añadir el ticket----------------------

         AuditoriaModel():addRegister( ::oTiketCabecera:uuid,;
                                       "Simplificada añadida " + ::oTiketCabecera:cSerTik + "/" + AllTrim( ::oTiketCabecera:cNumTik ) + "/" + ::oTiketCabecera:cSufTik,;
                                       TIK_CLI )
      ...
      //Guardamos los valores de la auditoría----------------------------------
...
METHOD CargaDocumento( cNumeroTicket ) CLASS TpvTactil
   ...
   if ::oTiketCabecera:Seek( cNumeroTicket ) .and. ::oTiketCabecera:RecLock()
      ...
      if ::oTiketLinea:Seek( cNumeroTicket )
         ...
         while ( ::cNumeroTicketLinea() == cNumeroTicket ) .and. !( ::oTiketLinea:Eof() )
            ...
            if !( uFieldEmpresa( "lShowLin" ) .and. ::oTiketLinea:lDelTil )
               ...
               /*::oTiketLinea:Load()
               ::oTiketLinea:nUndAnt    := ::oTiketLinea:nUntTil
               ::oTiketLinea:nPvpAnt    := ::oTiketLinea:nPvpTil
               ::oTiketLinea:Save()*/
      ...
      // Inicializo la temporal de auditoría-----------------------------------
...
METHOD OnClickSalaVenta( nSelectOption ) CLASS TpvTactil
   ...
   */
   ...
   RECOVER USING oError
      ...
      msgStop( "Error al montar la salas de venta2" + CRLF + ErrorMessage( oError ) )
...
METHOD OnClickParaLlevar() CLASS TpvTactil
   ...
   */
   ...
   RECOVER USING oError
      ...
      msgStop( "Error al montar la salas de venta3" + CRLF + ErrorMessage( oError ) )
...
METHOD OnClickEncargar() CLASS TpvTactil
   ...
   */
   ...
   RECOVER USING oError
      ...
      msgStop( "Error al montar la salas de venta4" + CRLF + ErrorMessage( oError ) )
...
METHOD OnClickCambiaUbicacion() CLASS TpvTactil
   ...
   */
   ...
   RECOVER USING oError
      ...
      msgStop( "Error al montar la salas de venta5" + CRLF + ErrorMessage( oError ) )
...
METHOD SetAliasDocumento( cTexto ) CLASS TpvTactil
   ...
   if ::validateAliasDocumento( cNombreUbicacion )
      ...
      apoloMsgStop( "El nombre elegido no es válido", "Elija uno diferente" )      
...
METHOD ProcesaComandas( lCopia )
   ...
   /*
   Comprobamos que el usuario pueda imprimir comanda---------------------------
   */
   ...
   /*
   Comprobamos no sea general sin guardarlo------------------------------------
   */   
   ...
   do case
      ...
      otherwise
         ...
         */
         ...
         ::oFastReport:AddVariable(     "Lineas de tickets",   "Importe descuento línea del factura",          "CallHbFunc( 'oTpvTactil', [ 'nDtoLTpv()' ] )" )
...
METHOD OnClickEliminarLinea()
   ...
   if RolesModel():getRolNoConfirmacionEliminacion( Auth():rolUuid() ) .or. ApoloMsgNoYes( "¿Desea eliminar el registro en curso?", "Confirme supresión", .t. )
      ::disableDialog()
      ::eliminarLinea()
      ::enableDialog()
   end if
...
METHOD eliminarLinea()
   ...
   ::oAuditoria:AddTemporal( {   "parent_uuid" => ::oTiketCabecera:uuid ,;
                                 "description" => "Linea eliminada: " + AllTrim( Trans( ::oTemporalLinea:nUntTil, ::cPictureUnidades ) ) + ;
                                                  " unidades de " + AllTrim( ::oTemporalLinea:cCbaTil ) + " - " + AllTrim( ::oTemporalLinea:cNomTil ) + ;
                                                  " con precio:  " + AllTrim( Trans( ::oTemporalLinea:nPvpTil, ::cPictureTotal ) ) ,;
                                 "tipodocumento" => TIK_CLI } )
...
METHOD ColorLinea( oDbf )
   ...
   do case 
      ...
      case oDbf:FieldGetByName( "lComent" )
         aColor   := { CLR_BLACK, Rgb( 251, 146, 51 ) }
      otherwise
...
METHOD InitDocumento( nUbicacion )
   ...
   //Inicializamos la variable de auditoría---------------------------------
...
METHOD BrwBigUser( dbfUsr, dbfCaj ) CLASS TpvTactil
   ...
   DEFINE DIALOG oDlg RESOURCE "SelUsuarios"
      ...
      ::oImgenUsr:AddMasked( TBitmap():Define( "gc_businessman2_50" ),   Rgb( 255, 0, 255 ) )
      ::oImgenUsr:AddMasked( TBitmap():Define( "gc_user2_50" ),          Rgb( 255, 0, 255 ) )
...
METHOD startActivateBrwBig() CLASS TpvTactil
...
METHOD SelBrwBigUser( nOpt, oDlg ) CLASS TpvTactil
   ...
   //::oTiketCabecera:cCcjTik   := cCodigoUsuario
...
METHOD LiquidacionCliente()
   ...
   if Empty( ::oTiketCabecera:cCliTik )
      apoloMsgStop( "Seleccione un cliente para hacer la liquidación" )
      return nil
   end if
...
METHOD InicializaDocumento() CLASS TpvTactil
   ...
   ::cargaValoresDefecto( ubiGeneral )

   // ::initValoresDefecto()

   ::getComensales( ubiGeneral )
