...
CLASS TComercioProduct FROM TComercioConector
   ...
   METHOD getRootImage( hProductImage )                        INLINE ( cPatOut() + hget( hProductImage, "cPrefijoNombre" ) + ".jpg" )
...
METHOD buildProduct( idProduct, lCleanProducts ) CLASS TComercioProduct
   ...
   // Recopilar Array de stocks-----------------------------------------------

   aStockArticulo             := StocksModel():aStockArticulo( idProduct, ::TComercioConfig():getStore() )

   aAdd( aStockArticulo, { "articulo" => idProduct,;
                           "almacen" => ::TComercioConfig():getStore(),;
                           "lote" => Space( 14 ),;
                           "propiedad1" => Space( 20 ),;
                           "propiedad2" => Space( 20 ),;
                           "valor1" => Space( 20 ),;
                           "valor2" => Space( 20 ),;
                           "unidades" => nTotStockArticulo } )
...
METHOD buildHashProduct( idProduct, aImagesArticulos, aStockArticulo, aLangsArticulos ) CLASS TComercioProduct
   ...
   aAdd( ::aProducts,   {  "id"                    => idProduct,;
                           "uuid"                  => ArticulosModel():getUuid( idProduct ),;
...
METHOD insertStockProduct( idProductPrestashop, hStock ) CLASS TComercioProduct
   ...
   isStockByProperty             := !empty( hget( hStock, "propiedad1" ) )    .or.;
                                    !empty( hget( hStock, "valor1" ) )        .or.;
                                    !empty( hget( hStock, "propiedad2" ) )    .or.;
