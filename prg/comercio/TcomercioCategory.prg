...
CLASS TComercioCategory FROM TComercioConector
   ...
   METHOD uploadImageCategory( cImage )                     INLINE ( ::oFtp():CreateFile( cImage, ::cDirectoryCategories() + "/" ) )
...
METHOD buildCategory( id, rootCategory ) CLASS TComercioCategory
   ...
   if ( D():Familias( ::getView() ) )->( dbseekinord( id, "cCodFam" ) )  
      ...
      aAdd( ::aCategoriesProduct,   {  "id"                 => id,;
                                       ...
                                       "aTypeImages"        => {},;
                                       "langs"              => aLangs,;
                                       "cType"              => alltrim( ( D():Familias( ::getView() ) )->cType ) } )
...
METHOD insertCategory( hCategory ) CLASS TComercioCategory
   ...
   //nLevelDepth          := if( hget( hCategory, "root_category" ) != nil, hget( hCategory, "root_category" ), nLevelInTree( Padr( hget( hCategory, "id" ), 16 ) ) )
...
METHOD getCategoryLangs( id, categoryName, categoryLongName, meta_title, meta_description, meta_keywords )
   ...
   for each hLang in ( hget( hCategoryProduct, "langs" ) )
      ...
      cCommand          := "INSERT IGNORE INTO " + ::cPrefixTable( "category_lang" ) + " ( "  + ;
                           ...
                           "VALUES ( "                                                 + ;
                              ...
                              quoted( hget( hLang, 'meta_title' ) ) + ", "             + ;
                              quoted( hget( hLang, 'meta_keywords' ) ) + ", "          + ;
