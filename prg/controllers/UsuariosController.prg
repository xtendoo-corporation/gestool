...
METHOD changeComboEmpresa( lInit )

   DEFAULT lINIT := .f.

   /*
   Cargamos las delegaciones de la empresa seleccionada------------------------
   */
...
CLASS SQLUsuariosModel FROM SQLBaseModel
   ...
   METHOD getUuidWhereNombre( nombre )                      INLINE ( ::getField( 'uuid', 'nombre', nombre ) )
   ...
   METHOD getNombreUsuarioWhereNetName( cNetName )          INLINE ( ::getField( 'nombre', 'last_pcname', cNetName ) )

   METHOD setUsuarioPcEnUso( cPcName, UuidUsuario )         INLINE ( ::updateFieldWhereUuid(  UuidUsuario, 'last_pcname', cPcName ) )

   METHOD setUsuarioEmpresaEnUso( cCodEmp, UuidUsuario )    INLINE ( ::updateFieldWhereUuid(  UuidUsuario, 'last_empresa', cCodEmp ) )

   METHOD getUsuarioEmpresa( uuid )                         INLINE ( ::getField( 'last_empresa', 'uuid', uuid ) )
...
METHOD getColumns() CLASS SQLUsuariosModel
   ...
   hset( ::hColumns, "last_pcname",    {  "create"    => "VARCHAR( 100 )"                          ,;
                                          "default"   => {|| space( 100 ) } }                      )

   hset( ::hColumns, "last_empresa",   {  "create"    => "VARCHAR( 4 )"                            ,;
                                          "default"   => {|| space( 4 ) } }                        )
...
METHOD getNombreWhereCodigo( cCodigo ) CLASS SQLUsuariosModel
...
METHOD getUsuariosToJson() CLASS SQLUsuariosModel

   local cSql  := "SELECT Codigo, Nombre "                            + ;
                     "FROM " + ::getTableName() 
...
METHOD SeederToADS() CLASS SQLUsuariosModel
...
/*Function lGetUsuario( oGetUsuario, dbfUsr )
   ...
   local oUsuarioSelector

   if !lRecogerUsuario()
      Return .t.
   end if

   oUsuarioSelector     := UsuariosController():New()
   ...
   DEFINE DIALOG oDlg RESOURCE "GetUsuario"
      ...
      oUsuarioSelector:oGetSelector:Bind( bSETGET( cCodigoUsuario ) )
      oCodigoUsuario        :=  oUsuarioSelector:oGetSelector:Activate( 100, 110, oDlg )
      oCodigoUsuario:bWhen  := {|| .t. }
   ...
   if !Empty( oUsuarioSelector )
      oUsuarioSelector:End()
   end if
