...
CLASS MovimientosAlmacenLineasRepository FROM SQLBaseRepository
   ...
   METHOD getPrimeraFechaCompra( cCodigoArticulo, cLote )

   METHOD getRowSetMovimientosForStock( hParams )

   METHOD getSqlSentenceMovForStock()
...
METHOD getSQLSentenceIdFromBuffer( hBuffer )
   ...
   cSql  := "SELECT id FROM " + ::getTableName()                                                            + " " + ;
...
METHOD getSQLSentenceFechaHoraConsolidacion( cCodigoArticulo, cCodigoAlmacen, cCodigoPrimeraPropiedad, cCodigoSegundaPropiedad, cValorPrimeraPropiedad, cValorSegundaPropiedad, cLote, dFechaFinLimite )
   ...
   if !empty( dFechaFinLimite )
      //cSql     +=    "AND cabecera.fecha_hora <= " + toSQLString( dFechaFinLimite ) + " "  ANTERIOR
...
METHOD getSQLSentenceMovForStock( hParams ) CLASS MovimientosAlmacenLineasRepository
   ...
   cSentence         += "SELECT "                                                                              + ;
                        ...
                        "INNER JOIN movimientos_almacen cabecera ON lineas.parent_uuid = cabecera.uuid "       + ;
                        "WHERE empresa_codigo = " + quoted( cCodEmp() ) + " "                                  + ;
                           "AND lineas.codigo_articulo = movimientos_almacen_lineas.codigo_articulo "          + ;
                           "AND cabecera.tipo_movimiento = 4 "                                                 
   if hhaskey( hParams, "almacen" ) .and. !empty( hget( hParams, "almacen" ) )
...
METHOD getRowSetMovimientosForStock( hParams ) CLASS MovimientosAlmacenLineasRepository
...
METHOD getPrimeraFechaCompra( cCodigoArticulo, cLote )
   ...
   cSentence   := "SELECT "                                                                                 + ;
                     ...
                     "WHERE empresa_codigo = " + quoted( cCodEmp() ) + " "                                  + ;
                        "AND lineas.codigo_articulo = " + quoted( cCodigoArticulo ) + " "                   + ;
                        "AND cabecera.tipo_movimiento = 4 "                                                 + ;
                        "AND lineas.lote = " + quoted( cLote, .t. ) + " "                                   + ;
                     "ORDER BY cabecera.fecha_hora DESC " + ;
                        "LIMIT 1"
