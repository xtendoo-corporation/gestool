...
CREATE CLASS TVF_ClaveTipoFacturaType FROM TVF_Tags
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_ClaveTipoFacturaType 
...
METHOD Set( uData ) CLASS TVF_ClaveTipoFacturaType 
...
METHOD HasData() CLASS TVF_ClaveTipoFacturaType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_ClaveTipoFacturaType 
...
METHOD Get() CLASS TVF_ClaveTipoFacturaType 
Return ( ::__ClaveTipoFacturaType )

METHOD Check( uData ) CLASS TVF_ClaveTipoFacturaType 
    ...
    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
        ...
        ::__oVerifactu:__oReturn:Log := 'El dato TipoFactura : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato TipoFactura : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { 'F1','F2','R1','R2','R3','R4','R5','F3' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_ClaveTipoFacturaType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__ClaveTipoFacturaType ) )
