...
CREATE CLASS TVF_CuponType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __CuponType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_CuponType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'CuponType' )
...
METHOD Set( uData ) CLASS TVF_CuponType 
...
METHOD HasData() CLASS TVF_CuponType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_CuponType 
...
METHOD Get() CLASS TVF_CuponType 
Return ( ::__CuponType )

METHOD Check( uData ) CLASS TVF_CuponType 

    hb_default( @uData, ::__CuponType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Cupon : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_CuponType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__CuponType ) )
