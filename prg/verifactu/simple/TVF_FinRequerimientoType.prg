// CLASS: TFinRequerimientoType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_FinRequerimientoType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __FinRequerimientoType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_FinRequerimientoType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'FinRequerimientoType' )
...
METHOD Set( uData ) CLASS TVF_FinRequerimientoType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__FinRequerimientoType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_FinRequerimientoType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_FinRequerimientoType 
...
METHOD Get() CLASS TVF_FinRequerimientoType 
Return ( ::__FinRequerimientoType )

METHOD Check( uData ) CLASS TVF_FinRequerimientoType 

    hb_default( @uData, ::__FinRequerimientoType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato FinRequerimiento : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_FinRequerimientoType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__FinRequerimientoType ) )
