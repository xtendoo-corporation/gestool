// CLASS: TGeneradoPorType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_GeneradoPorType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __GeneradoPorType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_GeneradoPorType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'GeneradoPorType' )
...
METHOD Set( uData ) CLASS TVF_GeneradoPorType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__GeneradoPorType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_GeneradoPorType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_GeneradoPorType 
...
METHOD Get() CLASS TVF_GeneradoPorType 
Return ( ::__GeneradoPorType )

METHOD Check( uData ) CLASS TVF_GeneradoPorType 

    hb_default( @uData, ::__GeneradoPorType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato GeneradoPor : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { 'E','D','T' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_GeneradoPorType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__GeneradoPorType ) )
