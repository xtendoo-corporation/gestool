...
CREATE CLASS TVF_ImpuestoType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __ImpuestoType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_ImpuestoType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'ImpuestoType' )
...
METHOD Set( uData ) CLASS TVF_ImpuestoType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__ImpuestoType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_ImpuestoType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_ImpuestoType 
...
METHOD Get() CLASS TVF_ImpuestoType 
Return ( ::__ImpuestoType )

METHOD Check( uData ) CLASS TVF_ImpuestoType 

    hb_default( @uData, ::__ImpuestoType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Impuesto : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { '01','02','03','05' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_ImpuestoType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__ImpuestoType ) )
