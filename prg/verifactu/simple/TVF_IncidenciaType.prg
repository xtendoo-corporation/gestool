// CLASS: TIncidenciaType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_IncidenciaType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __IncidenciaType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_IncidenciaType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'IncidenciaType' )
...
METHOD Set( uData ) CLASS TVF_IncidenciaType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__IncidenciaType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_IncidenciaType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_IncidenciaType 
...
METHOD Get() CLASS TVF_IncidenciaType 
Return ( ::__IncidenciaType )

METHOD Check( uData ) CLASS TVF_IncidenciaType 

    hb_default( @uData, ::__IncidenciaType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Incidencia : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_IncidenciaType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__IncidenciaType ) )
