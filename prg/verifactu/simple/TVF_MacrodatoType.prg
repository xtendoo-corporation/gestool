// CLASS: TMacrodatoType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_MacrodatoType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __MacrodatoType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_MacrodatoType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'MacrodatoType' )
...
METHOD Set( uData ) CLASS TVF_MacrodatoType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__MacrodatoType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_MacrodatoType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_MacrodatoType 
...
METHOD Get() CLASS TVF_MacrodatoType 
Return ( ::__MacrodatoType )

METHOD Check( uData ) CLASS TVF_MacrodatoType 

    hb_default( @uData, ::__MacrodatoType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Macrodato : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_MacrodatoType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__MacrodatoType ) )
