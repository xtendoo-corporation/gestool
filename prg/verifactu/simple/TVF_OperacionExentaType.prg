// CLASS: TOperacionExentaType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_OperacionExentaType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __OperacionExentaType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_OperacionExentaType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'OperacionExentaType' )
...
METHOD Set( uData ) CLASS TVF_OperacionExentaType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__OperacionExentaType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_OperacionExentaType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_OperacionExentaType 
...
METHOD Get() CLASS TVF_OperacionExentaType 
Return ( ::__OperacionExentaType )

METHOD Check( uData ) CLASS TVF_OperacionExentaType 

    hb_default( @uData, ::__OperacionExentaType )

    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
            
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato OperacionExenta : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato OperacionExenta : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { 'E1','E2','E3','E4','E5','E6' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_OperacionExentaType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__OperacionExentaType ) )
