// CLASS: TRechazoPrevioAnulacionType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_RechazoPrevioAnulacionType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __RechazoPrevioAnulacionType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_RechazoPrevioAnulacionType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'RechazoPrevioAnulacionType' )
...
METHOD Set( uData ) CLASS TVF_RechazoPrevioAnulacionType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__RechazoPrevioAnulacionType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_RechazoPrevioAnulacionType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_RechazoPrevioAnulacionType 
...
METHOD Get() CLASS TVF_RechazoPrevioAnulacionType 
Return ( ::__RechazoPrevioAnulacionType )

METHOD Check( uData ) CLASS TVF_RechazoPrevioAnulacionType 

    hb_default( @uData, ::__RechazoPrevioAnulacionType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato RechazoPrevio : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_RechazoPrevioAnulacionType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__RechazoPrevioAnulacionType ) )
