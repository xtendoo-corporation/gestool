// CLASS: TRechazoPrevioType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_RechazoPrevioType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __RechazoPrevioType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_RechazoPrevioType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'RechazoPrevioType' )
...
METHOD Set( uData ) CLASS TVF_RechazoPrevioType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__RechazoPrevioType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_RechazoPrevioType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_RechazoPrevioType 
...
METHOD Get() CLASS TVF_RechazoPrevioType 
Return ( ::__RechazoPrevioType )

METHOD Check( uData ) CLASS TVF_RechazoPrevioType 

    hb_default( @uData, ::__RechazoPrevioType )
...
METHOD BuildXml( oFather) CLASS TVF_RechazoPrevioType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__RechazoPrevioType ) )
