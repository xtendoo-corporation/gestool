...
CREATE CLASS TVF_SiNoType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __SiNoType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_SiNoType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'SiNoType' )
...
METHOD Set( uData ) CLASS TVF_SiNoType 
...
METHOD HasData() CLASS TVF_SiNoType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_SiNoType 
...
METHOD Get() CLASS TVF_SiNoType 
Return ( ::__SiNoType )

METHOD Check( uData ) CLASS TVF_SiNoType 

    hb_default( @uData, ::__SiNoType )

    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
            
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato IndicadorMultiplesOT : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato IndicadorMultiplesOT : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_SiNoType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__SiNoType ) )
