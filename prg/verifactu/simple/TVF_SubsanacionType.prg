// CLASS: TSubsanacionType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_SubsanacionType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __SubsanacionType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_SubsanacionType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'SubsanacionType' )
...
METHOD Set( uData ) CLASS TVF_SubsanacionType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__SubsanacionType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_SubsanacionType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_SubsanacionType 
...
METHOD Get() CLASS TVF_SubsanacionType 
Return ( ::__SubsanacionType )

METHOD Check( uData ) CLASS TVF_SubsanacionType 

    hb_default( @uData, ::__SubsanacionType )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Subsanacion : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_SubsanacionType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__SubsanacionType ) )
