// CLASS: TTipoHuellaType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_TipoHuellaType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __TipoHuellaType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_TipoHuellaType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'TipoHuellaType' )
...
METHOD Set( uData ) CLASS TVF_TipoHuellaType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__TipoHuellaType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_TipoHuellaType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_TipoHuellaType 
...
METHOD Get() CLASS TVF_TipoHuellaType 
Return ( ::__TipoHuellaType )

METHOD Check( uData ) CLASS TVF_TipoHuellaType 

    hb_default( @uData, ::__TipoHuellaType )

    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
            
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato TipoHuella : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato TipoHuella : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { '01' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_TipoHuellaType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__TipoHuellaType ) )
