// CLASS: TTipoOperacionType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_TipoOperacionType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __TipoOperacionType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_TipoOperacionType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'TipoOperacionType' )
...
METHOD Set( uData ) CLASS TVF_TipoOperacionType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__TipoOperacionType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_TipoOperacionType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_TipoOperacionType 
...
METHOD Get() CLASS TVF_TipoOperacionType 
Return ( ::__TipoOperacionType )

METHOD Check( uData ) CLASS TVF_TipoOperacionType 

    hb_default( @uData, ::__TipoOperacionType )

    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
            
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato TipoOperacion : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato TipoOperacion : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { 'Alta','Anulacion' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_TipoOperacionType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__TipoOperacionType ) )
