// CLASS: TTipoPeriodoType 
#include 'hbclass.ch'
#include "Verifactu.inc"
            
CREATE CLASS TVF_TipoPeriodoType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __TipoPeriodoType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_TipoPeriodoType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'TipoPeriodoType' )
...
METHOD Set( uData ) CLASS TVF_TipoPeriodoType 

    uData := ::Convert( uData )

    If ::Check( uData )
            
        ::__TipoPeriodoType := uData
        ::__lDataSet := .T.
...
METHOD HasData() CLASS TVF_TipoPeriodoType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_TipoPeriodoType 
...
METHOD Get() CLASS TVF_TipoPeriodoType 
Return ( ::__TipoPeriodoType )

METHOD Check( uData ) CLASS TVF_TipoPeriodoType 

    hb_default( @uData, ::__TipoPeriodoType )

    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
            
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Periodo : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Periodo : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { '01','02','03','04','05','06','07','08','09','10','11','12' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_TipoPeriodoType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__TipoPeriodoType ) )
