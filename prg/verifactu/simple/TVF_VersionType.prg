...
CREATE CLASS TVF_VersionType FROM TVF_Tags
    ...
    PROTECTED:
        ...
        DATA __VersionType AS STRING INIT ""
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_VersionType 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'VersionType' )
...
METHOD Set( uData ) CLASS TVF_VersionType 
...
METHOD HasData() CLASS TVF_VersionType 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_VersionType 
...
METHOD Get() CLASS TVF_VersionType 
Return ( ::__VersionType )

METHOD Check( uData ) CLASS TVF_VersionType 

    hb_default( @uData, ::__VersionType )

    If ::__lRequired .And. Len(Alltrim(( uData ))) == 0
            
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato IDVersion : ' + hb_ValToStr( uData ) + ' es requerido ' + ::__cTag 
        Return ( .F. )
    ...
    If .Not. HB_ISSTRING( uData )
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato IDVersion : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
            
    Endif


    If hb_AScan( { '1.0' } , uData ) == 0
...
METHOD BuildXml( oFather) CLASS TVF_VersionType 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__VersionType ) )
