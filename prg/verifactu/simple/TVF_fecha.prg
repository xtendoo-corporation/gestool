...
CREATE CLASS TVF_fecha FROM TVF_Tags
...
METHOD New( oVerifactu, lRequired, cTag, oFather ) CLASS TVF_fecha 

    hb_default( @lRequired, .F. )
    hb_default( @cTag, 'fecha' )
...
METHOD Set( uData ) CLASS TVF_fecha 
...
METHOD HasData() CLASS TVF_fecha 
Return( ::__lDataSet )


METHOD Convert( uData ) CLASS TVF_fecha 
...
METHOD Get() CLASS TVF_fecha 
Return ( ::__fecha )

METHOD Check( uData ) CLASS TVF_fecha 

    hb_default( @uData, ::__fecha )


    If .Not. HB_ISSTRING( uData )

        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Hasta : ' + hb_ValToStr( uData ) + ' no es del tipo string para ' + ::__cTag
        Return ( .F. )
    ...
    If Len( uData ) > 10
        ...
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Hasta : ' + hb_ValToStr( uData ) + ' excede los 10 caracteres para ' + ::__cTag
        Return ( .F. )
                
    Endif
    If .Not. hb_RegExLike( '\d{2,2}-\d{2,2}-\d{4,4}', uData,  ) 
                    
        ::__oVerifactu:__oReturn:Success := .F.
        ::__oVerifactu:__oReturn:Log := 'El dato Hasta : ' + hb_ValToStr( uData ) + ' no cumple el patr├│n \d{2,2}-\d{2,2}-\d{4,4} para ' + ::__cTag
        Return ( .F. )
...
METHOD BuildXml( oFather) CLASS TVF_fecha 
    
    If ::HasData()
    
        oFather:NewChild( 'sumI:' + ::__cTag, hb_ValToStr( ::__fecha ) )
